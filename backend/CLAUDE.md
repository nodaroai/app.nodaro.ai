...
Cloud-only proprietary features (currently: voice-changer-pro, surround-continuation, film-studio pipeline doctrine) ship from a private npm package instead of living in this repo — a closed-source counterpart to `ee/` for code that must stay private in implementation, not just license.
