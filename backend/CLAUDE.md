...
| Combine-videos smart cut | `smartCutEnabled` + `smartCutFramesPrev/Next` (route/worker/payload-builder/MCP `smart_cut*`): per boundary, `findSmartCutBoundary` (`providers/video/smart-cut.ts`, reuses smart-loop-cut's probe + sharp-PSNR at a 192px compare scale) searches the last N frames of clip k and the first M of clip k+1 on the NORMALIZED files, ends k ON the best-matching frame and starts k+1 right AFTER its match (duplicate plays once → continuous motion; built for i2v continuation clips). Results feed the per-clip `trimEdgeFrames` plan, REPLACING the fixed `trimStart/EndFrames` at boundaries (outer edges always 0); a failed boundary search falls back to the fixed trims for that boundary only. |
