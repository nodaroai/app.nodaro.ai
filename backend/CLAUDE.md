...
- `types.ts` — hand-synced structural mirror of the private repo's `src/contract.ts` (no shared package, no automated sync). Additive-only: new optional capabilities (`engines?()`, `prompts?()` were added this way, no version bump — older loaders ignore unknown members); a BREAKING change bumps `CONTRACT_VERSION` in BOTH repos; the loader refuses to register a plugin whose `contractVersion` doesn't match.
