...
**Error telemetry → `app_reports` (surfacing at `/admin/app-reports`):** two complementary layers, both best-effort (a telemetry failure can never break the path it observes) and observe-only (response bytes never change except the 500 sanitize rewrite):
- **HTTP net (`http-errors.ts`):** every 5xx — an uncaught route throw (`registerErrorTelemetry`'s `onError` hook, registered in `app.ts`), a `sendInternalError` call, or a hand-composed body the net genericized — files `kind: "internal-error"` (route pattern, userId, raw message + stack). The same onSend hook also observes structured product failures: 402 `insufficient_credits` → `kind: "insufficient-credits"` (info, throttle-keyed per user), 400 `validation_error` → `kind: "validation-reject"` (warning — wrong parameters that never reach a provider; from our own UI these are the recurring stale-enum/Zod-reject bug class), 503 `price_not_configured` → `kind: "price-not-configured"` (error). All throttled at one report / 5 min per key.
- **Failure sweeps (`lib/app-report-sweep.ts`, cron every 15 min):** classifies recent failed `jobs` rows — content-policy rejections → `kind: "model-rejection"`, everything else → `kind: "job-failure"` — which covers EVERY provider lane (KIE, Replicate, direct ElevenLabs, direct Anthropic/Gemini/OpenAI, ffmpeg, Remotion), since async provider failures all end as a failed jobs row. A second sweep reports failed/timed-out `workflow_executions` that produced NO failed job (orchestrator-level errors: payload-builder throws, credit walls at reservation, timeouts) as `kind: "execution-failure"`, keyed by the migration-328 `app_reports.execution_id` column. Idempotency: partial UNIQUE (kind, job_id) and (kind, execution_id) indexes; insertAppReport treats 23505 as a no-op.
...
- `load.ts` — the only file that imports `@nodaroai/cloud-plugins` (private repo `nodaroai/nodaro-cloud-plugins`), passed to `import()` as a runtime string so `tsc` never resolves it. No-ops unless `hasCredits()`. On cloud, dynamic-imports the package and, per plugin in its `plugins[]` array, registers `registerRoutes()` (only when called with an `app`), merges `handlers()`, additively applies `staticCreditCosts()` into `credits.ts`'s `STATIC_CREDIT_COSTS` (never overwrites a core-defined price), collects `engines()` (in-process engine surfaces called directly by core handlers — e.g. surround's `buildSurroundComposite`/`harmonizeSurround`), and registers `prompts()` into `ee/pipelines/llms/prompt-registry.ts` (film-studio doctrine strings, looked up synchronously via `getPipelinePrompt(key)`). Called from `app.ts` (with `app`), `workers/video-worker.ts` and `ee/workers/pipeline-worker.ts` (without — handlers/engines/prompts only; the pipeline worker MUST call it at module top-level before its BullMQ `Worker` is constructed, so prompts are registered before any stage runs).
- `types.ts` — hand-synced structural mirror of the private repo's `src/contract.ts` (no shared package, no automated sync). Additive-only: new optional capabilities (`engines?()`, `prompts?()`, `recipes?()` were added this way, no version bump — older loaders ignore unknown members); a BREAKING change bumps `CONTRACT_VERSION` in BOTH repos; the loader refuses to register a plugin whose `contractVersion` doesn't match.
- `recipe-registry.ts` — leaf accessor (`setPluginRecipes`/`getPluginRecipes`) for the merged `recipes()` table, so the `get_recipe` MCP tool (`lib/mcp/tools/recipes.ts`) serves plugin recipes beside `backend/skills/recipes/` without importing `load.ts`'s toolkit graph; hidden from first-party (Copilot) sessions.
...
**Daemons — long-lived plugin processes (`daemons()`, types in `daemon-contract.ts`):** for work no request or queue job can hold (a live connection kept open on a user's behalf), a plugin contributes `PluginDaemon`s — `{ name, registerInternalRoutes?, start({ signal }), health? }`. Only the daemon host collects them: `src/plugin-daemons.ts` calls `loadPrivatePlugins({ daemons: true, toolkit: buildToolkit({ role: "daemon" }) })`; the API server and the workers never construct one. The host (`lib/plugin-daemons/host.ts`) mounts each daemon's routes under `/<name>` on an internal listener (`PLUGIN_DAEMONS_PORT`) whose door is the HOST's: every route but `GET /health` requires `X-Internal-Orchestrator-Secret`, checked with the shared `lib/constant-time.ts` comparison in an `onRequest` hook before routing (an unknown path is a 401, not a route oracle). A daemon whose `start()` rejects or resolves while its signal is live takes the process down non-zero; SIGTERM aborts and drains ≤ 8 s; with no daemons at all (version lag, degraded load) the host stays up on `/health` instead of crash-looping. The API side calls in through `tk.daemons.request` (`lib/plugin-daemons/client.ts` supplies `PLUGIN_DAEMONS_URL` + the secret, refuses a path that climbs out of `/<daemon>`, and returns `reachable: false` instead of throwing). Exactly ONE host per environment — its own service (start command `node /app/backend/dist/plugin-daemons.js`, one replica) or `PLUGIN_DAEMONS_IN_CONTAINER=true` in start.sh, never both; the variables the two services must share are listed in `docs/deployment.md` → "Plugin daemon host (Cloud)". Two content-free toolkit members came with it: `tk.redis.lease` (`lib/redis-lease.ts` — SET NX PX + Lua compare-and-extend/delete, keys under `plugin:lease:`) and `tk.accountSecrets` (`lib/plugin-account-secrets.ts` over migration 440 `plugin_account_secrets` — the 435 posture, every call scoped to the plugin it names AND `getRuntimeEnv()`, every metadata call states its owner scope — `userId`, or the explicit `allOwners: true` for the daemon/admin — and throws on neither, no metadata read ever selects the ciphertext, and every envelope is sealed with a per-environment HKDF subkey of the instance key and bound to its row as GCM associated data — `instance-cipher.ts :: boundCipher` — so a copied ciphertext or another environment's code cannot open it). The host also strips the internal secret from each request after checking it, filters `/health` detail to counters, and reserves the daemon name `health`; in-container it binds loopback. **`accountSecrets.upsert/open` — the store's only encrypt/decrypt sites — exist ONLY in the `role: "daemon"` toolkit;** `private-plugins/__tests__/toolkit-daemon-role.test.ts` fails the build if any file other than `plugin-daemons.ts` asks for that role.
...
**Video-worker jobs and the `pre-task` sentinel (2026-09-15, widened 2026-09-22):** the video worker stamps `provider_kind="pre-task"` at pickup on EVERY row, and the reconcile cron fails + refunds a `pre-task` row whose stamp is 30 min old. The worker wraps the handler **at its dispatch site** — the one place `video-worker.ts` looks a handler up by job name and runs it — through `withPreTaskHeartbeat` (`workers/pre-task-heartbeat.ts`), which refreshes the stamp every 60 s while the handler runs and stops at a cap so a hung handler still ages into the sweep. The default cap is `PRE_TASK_HEARTBEAT_MAX_MS = NODE_TIMEOUT_MS` (90 min) — an empirical margin over today's handlers (relay polls ≤ 85 min, other ffmpeg spawns at the 10-min default; silence-detect, whose media proxy may fetch a multi-gigabyte original, declares its own budget since Track 0.19), not a derived bound. **A handler whose legitimate run can outlive it declares its own budget via `HandlerFn.livenessBudgetMs(job)`: the sum of the kill budgets of its own bounded steps, never a guess.** apply-edl on a direct lane (`POST /v1/apply-edl`, the MCP verb — no orchestrator ceiling) declares `applyEdlRenderBudgetMs(edl)`: each referenced source's fetch (charged at `DOWNLOAD_MAX_MS`, see below) + audio probe, the canvas probes, and the SAME per-chunk ffmpeg kill budget the slice runner hands `runFfmpeg` (`chunkRenderTimeoutMs`) over the SAME chunk plan (`resolveChunksForOutput`), plus — for a chunked video render — each audio slice and the single join/encode/mux (`audioMuxTimeoutMs`) — guard tests pin the sum. **What no cap bounds, stated rather than padded:** time WAITING for an ffmpeg slot (a residual for every ffmpeg handler; apply-edl's multi-hour slot holds are its main cause), and storage I/O (the R2 client has no request timeout — apply-edl's checkpoints, the 404-fallback download, the deliverable upload after the render); both ride in the slack between a real render and its kill budgets plus the 30 min after the last beat. The DAG lane agrees since Track 0.11: the handler declares its budget through `lib/job-budget.ts :: declaredJobBudgetMs`, and the orchestrator holds the node to that same number within one orchestrator attempt (see **Budget-aware caps** under Timeouts, incl. how a re-pick re-attaches a live render). Coverage is total by construction: no merge order, no later `Object.assign`, no second map can leave a job type out, so no handler, core or plugin, needs its own heartbeat for this. `__tests__/video-worker-heartbeat-wiring.test.ts` pins only what a text guard can — the looked-up handler is wrapped before it is invoked and nothing calls the bare lookup; the beats themselves are proven by `pre-task-heartbeat.test.ts` and the processor-level liveness cases in `video-worker.test.ts`. It used to wrap only the plugin loader's map, which left a core ffmpeg long-runner (an hour-long multicam apply-edl cut) with the original exposure. The refresh (`refreshPreTaskSentinel`) is a CAS on `provider_kind="pre-task"` + `status="processing"` and never writes `provider_kind`: a sentinel a plugin cleared (gvp/evp, owned by the 90-min orchestrator sweep) stays null, and a real provider kind is never overwritten. Do NOT "fix" a long run by raising `STALE_THRESHOLD_MS["pre-task"]` — it is the dead/hung backstop for every video-worker job type. History: staging Pro 3D Render job 99ede351 was failed at minute 31 with its worker alive (its only heartbeat renewed a Redis stage lease).
...
### Timeouts
- Per-node: 90 minutes (`NODE_TIMEOUT_MS` processing, `POLL_ABSOLUTE_TIMEOUT_MS` incl. queue wait)
- Per-workflow: 120 minutes (`WORKFLOW_TIMEOUT_MS`)
- Stale-execution backstop: 4 hours (`STALE_EXECUTION_THRESHOLD_MS`, `lib/job-budget.ts` — boot sweep + 90-s cron)
- Job polling interval: 3 seconds (`JOB_POLL_INTERVAL_MS`)

**Budget-aware caps (podcast Track 0.11, product decision 2026-09-24).** A node whose job DECLARES a budget (apply-edl and audio-sync today) gets node/poll timeouts = its budget, and the workflow cap grows by the same excess; everything else keeps the limits above byte-for-byte (same numbers, same timeout messages, context untouched). `excess = max(0, budget − NODE_TIMEOUT_MS)`; processing = `NODE_TIMEOUT_MS + excess`, poll = `POLL_ABSOLUTE_TIMEOUT_MS + excess`, workflow cap = `WORKFLOW_TIMEOUT_MS + Σ excess` over the run's budgeted dispatches (`OrchestratorContext.budgetExcessMs` — a SUM: sequential renders add up and parallel ones can serialize on the shared ffmpeg slots), stale backstop = 4 h + the same Σ. **One budget function, no second formula:** `lib/job-budget.ts :: declaredJobBudgetMs(jobName, data)` is the registry both hung-detectors read — the video worker's handler declares `livenessBudgetMs` THROUGH it (so the pre-task heartbeat beats exactly that long) and `node-executor.ts` sizes the node from the same call on the same payload. apply-edl's budget/chunk planning lives in the pure leaf `providers/video/apply-edl-budget.ts` (imports only `@nodaro/shared` + `ffmpeg-timeouts.ts`) so the engine never loads the ffmpeg runtime; `lib/__tests__/job-budget-leaf.test.ts` fails if it does, and `workers/handlers/__tests__/ffmpeg.test.ts` fails if a handler's declared budget ever differs from the registry's. A new budgeted job type = one registry entry + the handler's one-line delegation, and its job name must equal its node type (rows carry the node type in `job_type` until pickup renames it — guarded in `node-executor-budget-ceilings.test.ts`). **Clocks that wait from OUTSIDE the orchestrator** read the execution's excess from rows (`lib/execution-budget.ts :: executionBudgetExcessMs` — Σ over the execution's budgeted `jobs` rows via the same registry, plus each component node's inner execution, recursively) and only once their base limit is spent (`BudgetedDeadline`), so an unbudgeted run pays no extra query: the component route's inner wait (`routes/component-execute.ts`), the parent's component node wait (`COMPONENT_TIMEOUT_MS` in `node-executor.ts`; on completion it adds the inner excess to the parent's cap), and both stale sweeps. Inline sub-workflows share the parent's context, so their long nodes count for free; published-app runs are ordinary orchestrator runs (the reserve is per EDL minute, so a longer cap has no billing effect). Unchanged by design: sync-HTTP nodes' `AbortSignal.timeout(NODE_TIMEOUT_MS)` (nothing there declares a budget), `sweepNeverStartedJobs` (30 min `pending` = queue wait, not the node's own run), the component-wrapper sweep (acts only once the inner run is terminal). **The editor's client-side component executor waits as long as the server allows** (decided 2026-09-24): its own 30-min wait, and — once that is spent — the server's limit for the run from `GET /v1/component/execute/:jobId/wait-limit` (`POLL_ABSOLUTE_TIMEOUT_MS` + `executionBudgetExcessMs` of the inner execution — the route's own background wait), re-read each time it is reached (`frontend/…/workflow-editor/component-wait.ts`). Three cases: a long render already dispatched → base + excess; one in the run but NOT dispatched yet (`pendingBudgetedNodes` — `executionMayDispatchBudgetedJob`: an unsettled `node_states` entry of a budgeted type in a `pending`/`running` execution, recursing into dispatched component wrappers; unknown, e.g. a run not picked up yet, answers true) → at least the server's 90-min base, asked again there (the excess cannot see an undispatched node, so without this a render starting at minute 40 was abandoned at 30); nothing budgeted → 30 min exactly as before. A failed lookup is retried on the next poll ticks (3 in a row) before the known limit stands. Residual, pre-existing: the server's own waits (the route's background wait, a parent's component node) read only the excess at their base, so a render first dispatched after that base still ends them. A reload mid-render re-attaches a single-node apply-edl poll for 6 h (`LONG_RUNNING_NODE_TYPES` in `frontend/src/lib/single-node-restore.ts`). Residual: a component node nested INSIDE an inline sub-workflow is invisible to the row readers (its wrapper is linked to no execution column) — the orchestrator's in-memory cap still counts it. **An orchestrator re-pick RE-ATTACHES a live render (decided 2026-09-24):** when the orchestrator is re-picked mid-render (deploy drain → `moveToDelayed`, SIGKILL + stall re-pick, event-loop block), its resume calls `cancelInFlightChildJobs(id, { adoptLiveBudgetedRenders: true })`, which ADOPTS a single-shot row whose job declares a budget, is `processing`, and carries a `pre-task` stamp younger than `STALE_THRESHOLD_MS["pre-task"]` (`isLiveRenderRow` — the reconcile cron's own staleness rule, so the two readers never disagree about liveness). The node executor polls it under the SAME budget on the row's ORIGINAL clocks (`AdoptedJobClocks`: poll-absolute from `created_at`, processing from `started_at`) — a resume never grants a fresh budget, which is what bounds a re-pick loop; a render adopted after its budget is spent times out on the first tick. A stale/absent/cleared stamp, a `pending` row, a fan-out iteration or an unbudgeted job keeps today's cancel + refund + re-dispatch. Race: the cancel of a budgeted row is CAS-pinned to the stamp it judged dead, so a beat or a video-worker pickup landing in between makes the CAS miss and the re-read adopts the row — never a live render AND a re-dispatch. The component-timeout cleanup calls the function WITHOUT the option (the parent gave up; cancelling is the point). Residual: a worker that dies AFTER adoption and is never re-picked by BullMQ (`maxStalledCount` exhausted) is failed + refunded by the 30-min `pre-task` sweep and the node fails rather than re-rendering. Pre-existing and unchanged: the workflow-level cap and the execution's `started_at` restart at each re-pick. **The 3-hour output cap (decided 2026-09-24):** `APPLY_EDL_MAX_OUTPUT_MS` (180 min, in the budget leaf) — `validateEffectiveEdl` refuses a longer edit at every ingress (the REST route runs it as a preHandler AHEAD of the credit guard, so it is a 400 `invalid_edl` whose message names the issue, never a 402), and `applyEdlJobBudgetMs` declares no budget for an over-cap payload. It bounds the budget by output LENGTH. **The chunk kill budget tracks real work (decided 2026-09-24, measured on the pinned ffmpeg):** `chunkRenderTimeoutMs(edl, segs, reads, canvas) = max(2 min, 3 × (1.6·pf·output + 0.5·pf·videoSpan + 0.05·audioSpan))` (audio-only slices: 0.1·output instead of the picture encode rate) — `pf = canvasPixelFactor(canvas)` (1080p30 = 1, 1080p60 = 2, 4K30 = 4, floored at 1, capped at 4), and the spans are the SOURCE seconds each input decodes from its seek point to its last read (`chunkDecodeSpanSec`). Render time grows with the source a chunk decodes, not only its output: 30 one-second windows spread over 179 min took 1,340–1,518 s — the old flat 20-min floor KILLED that real render — while a dense 1080p chunk costs ~1.2 s per output second and a 4K one ~5.6. Every measured shape keeps ≥ 3.2× headroom (`apply-edl-budget.test.ts` re-checks the measured table). **The formula is linear only because graphs are capped at 30 segments** — picture (`VIDEO_FILTERGRAPH_MAX_SEGMENTS`) AND sound (`AUDIO_FILTERGRAPH_MAX_SEGMENTS`, decided 2026-09-24): each segment is its own branch off the shared input, so an audio graph's cost grows with segments² × the source span it decodes. 180 one-minute cuts of a 3-hour master in ONE pass (the old ≤200-segment threshold) took 88.7 min — over the 81 min this formula gives it — and six 30-segment slices of the same edit take 16–23 s each; 100 one-second windows spread over 3 hours took 21 min in one slice (the old 20-min floor killed it) and 27–50 CPU-s per 30-segment slice. Never widen a graph without re-measuring. **A crossfade run has no hard cut to close a chunk at** (the node's Default crossfade makes a whole edit one run), so `planChunks` makes one INSIDE one of its segments (plan B2, `splitInsideCrossfadeRun`): the head keeps the crossfade in, the tail has no transition and the next crossfade blends out of it, and the tail reads its picture from the whole segment's start (`PlanSegment.splitLeadMs`) so its frames keep that segment's frame phase; the head is the chunk's last segment and, when it lies wholly inside the crossfade into it (it can outlast that crossfade by under a frame), it renders as a crossfade over ALL its frames (`PlanSegment.splitTailMs`) — the frames one pass blends — where any other wholly-covered segment adds no picture. A split render matches one pass frame for frame (e2e on frame-counter sources at 300/100/200 ms dissolves), except that a boundary landing EXACTLY on a half frame may round the other way (float order): ≤ 1 frame, drift-free, consistent within the render. A full chunk inside a run that goes on closes at the LATEST hard cut or cuttable run segment within the cap (`closeInsideRun`), so a graph exceeds 30 only when no segment of its run within reach can take a cut. The cut is taken only where the D17 per-boundary clamp leaves BOTH crossfades exactly as they were (checked with `boundaryOverlapSecs` itself), in whole ms so the plan does not depend on the canvas (the dispatch-time budget cannot know it). Only a run whose segments are ALL too short for their crossfades (e.g. 1 s clips under 5 s crossfades, clamped to 0.9 s each side) stays one wider graph; its cost grows faster than linearly with width (300 × 200 ms all-crossfade audio, unsplit, ran past a 120 s limit with 16 of 45 s rendered), so a slice wider than its cap keeps production's former limit as a FLOOR — `WIDE_SLICE_FLOOR_MS`: max(20 min, 6 × output) on its own output (decided 2026-09-24). The dispatch-time LIVENESS budget (heartbeat, `isLiveRenderRow`, every budget-derived ceiling) cannot know the canvas, so it charges every chunk at `LIVENESS_CANVAS` (4K30): always ≥ the kill budgets, looser for 1080p. Measured for 180-min edits: 1080p chunk kill budgets sum to ~19 h at any cut density (a hung ffmpeg is killed at ~3× its expected time); the liveness ceiling is ~76–84 h for hard-cut edits (~97–109 h for a Default-crossfade 3-hour edit: 3,600 × 3 s at 300 ms, 10,800 × 1 s at 100 ms) (was ~18.5 h for one segment and ~120–160 h for a 10,800-cut tighten under the flat floor). **Big-media downloads are opt-in (Track 0.19, decided 2026-09-25):** by default `downloadFile` keeps ONE flat `DOWNLOAD_TIMEOUT_MS` (120 s) for response + body — for the ~60 callers that fetch provider results, images and user-supplied URLs it is their only bound. A caller fetching big media passes `{ limits: BIG_MEDIA_DOWNLOAD_LIMITS }` — today exactly apply-edl's source originals and the media proxy's source, both user-supplied URLs, so the limits are also an abuse guard: 120 s for the response; once 120 s have passed since the start (never stricter than the flat default — a slow source it delivered still arrives), every `DOWNLOAD_RATE_WINDOW_MS` (60 s) window must deliver `DOWNLOAD_MIN_BYTES_PER_WINDOW` (15 MB ≈ 2 Mbit/s — a dead or drip-fed transfer fails within ~2–3 min); a known-size body gets its size at 2 MB/s, nothing runs past `DOWNLOAD_MAX_MS` (60 min), and nothing writes past `BIG_MEDIA_MAX_BYTES` (64 GB; a body whose Content-Length is over the cap fails at the headers); every timeout starts "Download timeout:" and names its limit. The body is requested uncompressed (`Accept-Encoding: identity`) and a compressed one is REFUSED — the rate window counts decoded bytes, so gzip of zeros (~1000:1) would carry a drip-feed past it to the 60-min ceiling. A declared budget counts `DOWNLOAD_MAX_MS` for each big-media download (apply-edl per source; audio-sync's and silence-detect's proxy source fetch) and `DOWNLOAD_TIMEOUT_MS` for an ordinary one; a one-source video apply-edl now budgets just over 90 min, so it runs on its declared ceilings. A new big-media caller gets these limits only if its transfer can keep ~2 Mbit/s. Residual: a 30-segment 4K chunk needed ~4 GB RSS (OOM-killed in an 8 GB VM shared with other renders) — 4 concurrent 4K ffmpegs could exhaust a worker's memory. **Cancellation:** `applyEdl` calls `throwIfJobCancelled()` at every chunk boundary (each source, picture chunk, audio slice, and before the join), so a cancelled multi-hour render stops within one chunk and releases its ffmpeg slot (the video worker runs every handler inside `runWithJobCancellation`), and deletes the R2 checkpoints it uploaded (a cancelled job is never resumed; any other failure keeps them for the same-jobId BullMQ retry).
...
### `jobs.job_type` at pickup — an OVERWRITE, not a backfill (INVARIANT)

`jobs.job_type` is written twice with different meanings. At INSERT it is the *admitted* type: `node.type` for an orchestrated DAG row (`insertInternalJob("orchestrator", …)` in `node-executor.ts`), the route's own name for a plugin/pipeline row, and nothing at all for the synchronous routes. Then the video worker's pickup CAS (`job_type: job.name` in `workers/video-worker.ts`) **unconditionally overwrites** it with the BullMQ queue name — every comment that used to call this a "backfill" was wrong; it replaces a non-null value, it is not `?? job.name`, and it applies to the video queue only (`render-worker.ts`'s pickup UPDATE leaves the column alone). **The gallery depends on the rewrite:** `routes/gallery.ts` and its MCP twin `lib/mcp/tools/gallery.ts` allowlist rows with `.in("job_type", …)` in queue-name vocabulary, so DAG rows (`modify-image`, `upscale-image`, `generate-video` — in no allowlist) become visible only once pickup renames them to `image-to-image` / `edit-image` / `image-to-video`; a production sample had 225 of 400 completed DAG rows depending on it, and breaking it removes that history from every gallery surface with no error anywhere. Consequences to work with rather than around: **private-plugin lanes must accept BOTH spellings** — `tk.jobs.readJob(...).job_type` returns the queue name after pickup, so a `row.job_type !== "<admitted type>"` guard is safe only for a lane that inserts and enqueues the same string (generate-video-pro's deliberate lockstep), and a lane that admits X while enqueueing Y must match either (assuming otherwise left the Scene3D advanced-preview lane dark for a day, plugins PR #467); and **run-history filters key off `input_data.type`, never the column** (`routes/jobs.ts`), because that value is written once at insert and never rewritten. Guarded from both sides: `workers/__tests__/video-worker.test.ts` ("pickup OVERWRITES job_type…") asserts the CAS writes `job.name` over a differing pre-existing value, and `routes/__tests__/gallery.test.ts` ("job_type allowlist is QUEUE-NAME vocabulary") pins the three renames.
...
|-------|------|------------|-------|
| `gemini-3-flash` | Economy | chat-completions | `thinkingDefaultOn`, reasoning floor 8192 |
| `gemini-3.6-flash` | Economy | chat-completions | efforts low/high (KIE) / none–high (direct); direct lane (Advanced mode); backs 5 feature defaults — stays KIE-first; `thinkingDefaultOn`, reasoning floor 8192 (measured, #1588) |
| `gemini-3.7-flash` | Economy | chat-completions | efforts low/high (KIE) / none–high (direct); direct lane (Advanced mode); image-only modality caps (VA decision deferred — see llm-models.ts); `thinkingDefaultOn`, reasoning floor 8192 |
| `gemini-3.8-flash` | Economy | chat-completions | efforts low/high (KIE) / none–high (direct); direct lane (Advanced mode); image-only modality caps (VA decision deferred, same rationale as 3.7); KIE output cap measured 16k+ (unlike 3.6/3.7's 8192 intersection) → `maxOutputTokens: 16384`; `thinkingDefaultOn`, reasoning floor 16384 |
...
| `gemini-3.1-pro` | Premium | chat-completions | `thinkingDefaultOn`, reasoning floor 16384 |
...
| `gpt-6-astra` | Premium | responses | efforts low/medium/high/xhigh; `supportsTemperature: false`; `thinkingDefaultOn` |
...
`gpt-6-astra` (KIE `codex/v1/responses`, body model `gpt-6-astra`) and `gemini-3.8-flash` (KIE
slug `gemini-3-8-flash-openai`) were live-verified 2026-09-06. Astra: `text.format` json_schema
is enforced (server echoes `strict: true`); `reasoning.effort` enum low/medium/high/xhigh; with
no reasoning param the echo shows `medium` → `thinkingDefaultOn`; `temperature` is silently
ignored (echo stays at the default) → `supportsTemperature: false`. Its rate row in
`lib/pricing/llm-cost.ts` is the FLAT band only — OpenAI reports cached tokens INSIDE the prompt
total, so the cached-read / cache-write multipliers must not be fed (see the warning in that
file); that rate row IS `provider_cost` for non-streamed astra calls, which are served collapsed
over SSE (see API Formats below) and so carry no `credits_consumed`. 3.8: `response_format`
json_schema enforced, `reasoning_effort` low/high, and `max_tokens 20000` was honored
(14,892 completion tokens, `finish_reason: stop`) — hence its 16384 cap where 3.6/3.7 keep the
8192 KIE-safe intersection.

### API Formats
- **chat-completions** — `gemini-3-flash`, `gemini-3.6-flash` (slug `gemini-3-6-flash-openai`), `gemini-3.7-flash` (slug `gemini-3-7-flash-openai`), `gemini-3.8-flash` (slug `gemini-3-8-flash-openai`), `gemini-3.1-pro`, `gpt-5.2` — `POST {KIE_API_BASE}/{model.kieSlugOrModel}/v1/chat/completions` (the model's own KIE slug prefixes the path — there's no single shared chat-completions URL)
...
- **responses** — `gpt-5.4`, `gpt-5.5`, the GPT-5.6 family (`gpt-5.6-luna`/`gpt-5.6-terra`/`gpt-5.6-sol`), `gpt-6-astra`, and `grok-4.6` — `POST {KIE_API_BASE}/{family}/v1/responses` with an `input` array + `developer` role. The path family is derived from the registry `vendor` (`openai` → `codex`, `xai` → `grok`; see `kieResponsesUrl`) — the wrong family is a hard error, not an alias. Grok's endpoint additionally rejects the plain-string `input` form (500) — always the array form, which is the only shape the builder emits.
  - `gpt-6-astra` alone on this format is served **collapsed** — `llmComplete` opens the streaming wire and collapses the SSE into one response (`callKieResponsesCollapsed`, one bounded retry), because KIE's non-stream lane 500s about two calls in three for this model (measured 2026-09-06: 2/6 non-stream vs 5/6 streaming, same body; the other responses models are fine non-stream). The registry flag `kieCollapseStream` is the switch — per model, capability-driven, never a model-name match — and provider cost on that path is the rate-table estimate, since KIE's SSE does not reliably carry `credits_consumed`. Two invariants keep the collapsed lane as HONEST as the non-stream one it replaces, because it returns the stream parser's object instead of going through `buildResponse`: a stream that ends with no text at all (dies after `response.created` — no error frame, so nothing else throws) is a thrown failure eligible for that retry, never an empty-string success handed to the caller; and `minPromptTokens` (the media fail-open guard) is re-applied by hand after the retry resolves, deliberately outside it so an un-ingested answer doesn't spend a second billed call.
...
`deriveParams()` also floors the output-token cap whenever reasoning shares it — at `xhigh`/`max`, and on EVERY call to a `thinkingDefaultOn` model — to `reasoningOutputFloor(model)` (`@nodaro/shared`): 32768 unless the model declares a lower `reasoningOutputFloor`, which it must when a lane serving it is not known to take 32768 (the Gemini entries: 8192 for the KIE-capped flash models, 16384 for 3.8 / 3.1 Pro). It applies even over an explicit caller `maxTokens` — a node's 1,100 on gemini-3.6-flash was issue #1588.

**A cap stop is a failure on every lane, never an answer (#1588).** No dialect reports "stopped at the output cap" as an error — it is a 200 with `finish_reason: "length"`, `stop_reason: "max_tokens"`, `finishReason: "MAX_TOKENS"` or `status: "incomplete"` + `max_output_tokens`, and the text is a fragment. Every adapter reads its dialect's signal and throws `LlmOutputTruncatedError` (`lib/llm-errors.ts`, a usage-carrying `LlmStreamResponseError`: never transport-retried, NEVER re-asked on the other serving lane — `laneFallbackAllowed`, because the cap is the request's, not the lane's, and a fallback would bill a second full-length reply cut at the same place — and `llmCompleteStructured` fails fast instead of paying for a correction retry). The routes' existing catch then fails the job and refunds, and the orchestrator stops the run instead of passing the fragment downstream; llm-chat, ai-writer and prompt-helper also keep the incurred cost on the failed row (`billedProviderCost`), since the old "completed" fragment carried it. A caller for which a cut-off verdict is recoverable catches the class explicitly (`reduce-strategies/pick-best-llm.ts` keeps its first-survivor fallback). In the editor, a failed Generate Text stream puts back the last good result — `extractNodeOutput` reads `generatedText` whatever the status. The stop reason is a REQUIRED argument of every response builder (`buildResponse`'s `ReplyEnd`, `parseSseStream`'s `cap`, the Gemini lane's `toResponse`), so a new lane cannot build a response without stating it. Behavioural cases per lane: `lib/__tests__/llm-output-cap.test.ts` + `lib/gemini/__tests__/output-cap.test.ts` (the #1588 replay: KIE `{code: 500}` → direct fallback → `MAX_TOKENS`). The one lane without a live case is `callKieMessages`, unreachable while `KIE_CLAUDE_NONSTREAM_VERIFIED` is false.
...
- **`KIE_CLAUDE_TOOLS_VERIFIED = true`** — forced tool_choice DOES reach the model, but KIE's proxy does NOT return a `tool_use` block: the tool call arrives re-serialized as a `<tool_calls>` text pseudo-tag whose JSON is malformed by construction (each tool object's closing brace missing — live-captured 2026-07-14; the 2026-07-13 "verification" only checked the call arrived and broke every structured call routed via KIE, e.g. the generate-video-pro planner). `callKieMessages` decodes the tag via `extractKieToolCallInput` (balance-scan of the well-formed `"input"` object) and THROWS when a structured response has no decodable payload, so `llmComplete` falls back to the direct SDK instead of burning the parse+retry loop. Structured NON-STREAM calls stay on the `preferKie` path.
...
### Direct Google lane (`directGeminiModel` / `preferDirect` + `GEMINI_API_KEY`)

The Google-side twin of the direct-Anthropic fallback. Two registry fields in `packages/shared/src/llm-models.ts` drive it, and nothing in `llm-client.ts` matches on a vendor name: `directGeminiModel` names the model's id on Google's own API (stated, not derived — Google carries `-preview` suffixes, so `gemini-3.1-pro` → `gemini-3.1-pro-preview`), and `preferDirect` decides lane ORDER. With `GEMINI_API_KEY` set, a `preferDirect` model goes Google-first with KIE as the fallback; any other model with a `directGeminiModel` stays KIE-first and uses Google only when KIE fails. With the key unset the direct lane doesn't exist and routing is byte-identical to before. `preferDirect` and `preferKie` are the two halves of one idea and are mutually exclusive — guarded by a registry test.

**Lane choice is a COST decision, not just reliability.** The two lanes bill the same model at materially different unit rates (figures live in `lib/pricing/llm-cost.ts`, the sanctioned home — keep them out of `packages/*`, which is published to npm, and out of `docs/`; `tools/check-pricing-leaks.mjs` enforces the package half). Only `gemini-3.1-pro` is direct-first today — premium tier, lowest volume, and the place the capability wins actually land. `gemini-3.6-flash` backs five `LLM_FEATURE_DEFAULTS` plus the video-analysis fast tier, so it stays KIE-first; a routing test pins that so it can't drift onto the expensive lane silently. `calculateLlmCost(model, usage, lane)` carries a per-lane rate band (plus Gemini 3.1 Pro's >200k-prompt step-up), so `provider_cost` reflects whoever actually served the call. KNOWN GAP: the direct-Anthropic fallback still costs at the KIE row — pre-existing, and the mechanism to close it now exists.

**Media is the load-bearing difference.** The KIE chat-completions lane works only because *KIE* dereferences our R2 URLs server-side — that's why every `LlmContentBlock` is a bare URL. Google will not: a `Part` may carry inline base64, a Files API URI, a Cloud Storage URI, or a YouTube link, and nothing else. `lib/gemini/media.ts` bridges it — `safeFetch` the URL, then inline (≤6 MB raw, well under the ~20 MB request ceiling once base64 inflates it 4/3) or stream to a temp file and upload via the Files API, polling to `ACTIVE` because referencing a still-`PROCESSING` video fails the generate call. Handles are cached per URL (12h, inside the 48h retention) so a multi-window video-analysis run uploads a large clip once. YouTube URLs pass through untouched. A failed media fetch THROWS rather than dropping the reference — a model silently answering about a video it never saw is worse than a hard failure.

**Inline video bytes (`video_base64`) — the one block with no URL.** For a caller that must analyse a SPECIFIC, IMMUTABLE payload it already holds, a URL is the wrong identity: publishing the user's media to reach the model is a privacy cost with no upside, and a mutable/signed URL can change, expire or 403 between the request being built and the model reading it — the analysis would then describe something else, with a 200. So the block carries the bytes: `{ type: "video_base64", mediaType: "video/mp4", data, fps? }`. `lib/gemini/media.ts` validates it (media type EXACTLY `video/mp4`, canonical base64, real MP4 `ftyp` magic, ≤ 6 MiB RAW — the same `INLINE_MAX_BYTES` used for inlining a fetched asset) and passes the bytes through verbatim as `inlineData`, with `fps` riding as `videoMetadata` exactly like the URL block. Bigger clips stay on the URL `video` block and its Files API path. **It is the only lane-pinned block in the union**: `llmComplete`/`llmStream` reject the whole request unless it carries `requireLane: "direct"` (`assertInlineVideoLane`, before any lane is chosen — so before any provider request, spend or metering), and `assertLanePinnable` then rejects a Claude/GPT model on the same pin. Requiring the pin is what makes it total — an unpinned Gemini call is still eligible for a KIE leg (primary or fallback), and no KIE format can carry bytes at all, so "it happened to route direct" is not a guarantee. All four non-Gemini builders throw on the block as defense in depth. `toolkit.completeStructuredMultimodal` already defaults the pin; `private-plugins/llm-metered.ts` deliberately does NOT, so a caller using it directly passes `requireLane: "direct"` itself.

Effort maps to `thinkingConfig.thinkingLevel` (`minimal|low|medium|high`; our `xhigh`/`max` collapse onto `high` since Google exposes nothing above it). `thoughtsTokenCount` is folded into `outputTokens` because Google bills thinking at the output rate.

**`LlmRequest.requireLane` — pin, no fallback.** Overrides every registry preference above and disables both fallback legs. `assertLanePinnable` hard-fails a pin the environment can't honour (no `directGeminiModel`, or no `GEMINI_API_KEY`) with an operator-actionable message rather than quietly serving the other lane. **Video-analysis uses this**: `toolkit.completeStructuredMultimodal` defaults to `requireLane: "direct"`, because that is the analysis lane by construction (see the `PluginLlmMultimodalRequest` docstring). The reason it must not fall back is that KIE reaches Gemini through the `image_url` URL-smuggling hack instead of real media parts, and drops record-shaped schema fields — so a fallback run returns differently-grounded analysis with no signal, which is strictly worse than an outage. `completeStructured` (text-only, shared by gvp/evp planners and film-studio doctrine) deliberately does NOT default-pin. **Operational consequence: `GEMINI_API_KEY` is REQUIRED on any deployment serving video-analysis** — a guard test pins that every `VIDEO_ANALYSIS_TIERS` model has both a direct lane and a direct rate row, so the config can't drift into a hard outage.

### Gemini enforced `responseSchema` cannot express `z.record` (house-wide structured-output rule)
...
**The direct lane's own limit is array caps.** Google compiles `responseJsonSchema` into a constrained decoder with a state budget, and `maxItems` multiplies it by its bound: a schema nesting a few caps is refused with a bare `400 INVALID_ARGUMENT` (~2 s, no usage, no keyword named, identical on every retry — measured 2026-09-18 on `gemini-3.7-flash`; it failed every scene3d job carrying a video reference). `lib/gemini/response-schema.ts` withholds `maxItems` from the wire schema for EVERY structured call on this lane, streamed or not; the cap is not lost, because `llmCompleteStructured` re-validates the answer against the caller's Zod. `minItems`, `min/maxLength`, `minimum/maximum` and `additionalProperties` were each varied alone and stay. Write schemas normally — do not strip caps per schema (a plugin's schema is also its fingerprint and its contract on the other lanes), and extend the withheld set only with a repro.

### `supportsTemperature: false` — registry-derived strip
Models with `supportsTemperature: false` (`claude-opus-4.7`, `gpt-5.5`, the GPT-5.6 family, `gpt-6-astra`, `grok-4.6`, `claude-sonnet-5`, `claude-opus-4.8`) have `temperature` stripped from every wire body in `deriveParams()` — never sent, regardless of what the caller passed. The same registry flag also derives `ee/pipelines/llms/call-llm.ts`'s `TEMPERATURE_UNSUPPORTED_MODELS` set (`LLM_MODELS.filter(m => m.supportsTemperature === false).flatMap(m => [m.kieSlugOrModel, ...(m.directFallbackModel ? [m.directFallbackModel] : [])])` — covers both the KIE slug and the direct-Anthropic model id). The film-studio pipeline's LLM wrapper is a SEPARATE call path from `llm-client.ts` and used to hand-list this set; it's now single-sourced from the registry so a new reasoning model can't ship covered on one call path but forgotten on the other.
...
| TTS v3 vs v2 | ElevenLabs v3 supports `[audio tags]`. As of the TTS-direct migration, ALL models (v3/turbo/multilingual) route through the direct ElevenLabs API (`workers/handlers/audio-ai.ts::handleTextToSpeech` → `directElevenLabsTTS`) — never KIE. Worker `stripAudioTags()` removes `[…]` before sending for non-v3. KIE's `KieAudioProvider.textToSpeech` + `KIE_TTS_MODELS` remain registered (unused in production) only for historical actual-cost audit reconciliation in `ee/routes/admin-credit-audit.ts`. Because the call is synchronous, TTS jobs never set `provider_call_started_at` and are invisible to the reconcile cron (no `makeOnTaskCreated` wiring). |
| MCP server | Per-request `McpServer` at `POST/GET /mcp` (`backend/src/lib/mcp/`). 4 tool families (verbs/jobs/workflows/components/apps/models/gallery) gated by scope; widgets returned alongside text from generation tools. **Workflow tools (`tools/workflows.ts`):** 9 tools — `list_workflows`/`get_workflow`/`get_workflow_json`/`export_workflow` (`workflows:read`), `create_workflow`/`delete_workflow`/`update_workflow_json`/`import_workflow` (`workflows:write`), `run_workflow` (`workflows:execute`). All except `export_workflow` are scoped to the user's auto-created "mcp" project via `ensureMcpProject(session)` (`tools/_mcp-project.ts`, caches `session.mcpProjectId`) — they validate `project_id` matches before any read/write/run. **In-app sessions** (the Workflow Copilot) pin the session to the open workflow's real project instead: `buildMcpServer({ projectScope: { projectId } })` verifies the project belongs to the user, then sets `session.scopedProjectId`, which `ensureMcpProject` returns first. INVARIANT: `project_id` is a narrowing filter only — every block in `tools/*.ts` that reads/compares `project_id` also filters `user_id` (`__tests__/project-scope-guard.test.ts` fails the build otherwise). **In-process invocation:** `lib/mcp/invoke.ts` (`createMcpInvoker(server).listTools()/callTool()`) is the one sanctioned way to call tools without a transport (SDK private handler map, `InMemoryTransport` fallback); the test helper re-exports it. **Handle map:** `lib/mcp/generated/node-handles.ts` (`NODE_HANDLES`, inputs/outputs per node type) is GENERATED by `gen:skills` from the frontend `NODE_DEFINITIONS` and drift-checked by `gen:skills:check` — never hand-edit. **Skill sections:** `backend/skills/workflow-editor.md` carries `<!-- SECTION:name -->` markers in its hand-written prose; `getWorkflowEditorSections([...])` in `tools/skill-loaders.ts` composes a subset (the Copilot skips `update-contract`/`result-fields`, which assume `update_workflow_json`). `export_workflow` can export ANY of the caller's workflows (template mode strips generated fields via `stripExportContent`; `with_assets=true` bundles characters/objects/locations — logic mirrors `routes/workflows.ts` since MCP has no user JWT). `import_workflow` parses + Zod-validates the bundle, re-creates bundled assets under the caller, remaps `*DbId` node fields, and always lands the workflow in the mcp project. `update_workflow_json` accepts any subset of `nodes`+`edges` (together), `settings`, and `thumbnail_url` (thumbnail-only updates are allowed; mirrors the all-optional REST `PATCH /v1/workflows/:id`) and supports optimistic concurrency via `expected_updated_at`/`expected_version`. **`buildMcpServer` is async** (`Promise<McpServer>`). **v3.0: per-user dynamic `app_<slug>` / `component_<slug>` MCP tools were dropped** (`tools/dynamic.ts` removed — they didn't scale and competed with the verb tools). Apps and saved components are now reached via static discovery tools: `list_apps` / `get_app_inputs` / `run_app` (`tools/apps.ts`) and `list_components` / `get_component_inputs` / `run_component` (`tools/components.ts`). The `published_apps.last_run_at` recency index (migration 096) still powers default ordering in `list_apps`. **Schema:** `published_apps` uses `creator_id` (NOT `owner_user_id`); the MCP app tools filter on BOTH `is_active = true` AND `deleted_at IS NULL` (`tools/apps.ts`); migration 096 adds `last_run_at` + per-user recency index. Workflow widget (`widgets/workflow.ts`) is a vertical pill list with live `node:<id>:<status>` updates bridged from `executionEvents` → MCP `ui/message` via `progress-emitter.ts`. Widget runtime JS is `createElement`+`textContent` only — no raw HTML assignment; snapshot tests guard. v3.0: `/v1/oauth/app-info` returns `kind` (from migration 094); consent UI renders `<McpConsentNotice>` orange warning when `kind=dynamic_mcp` (self-claimed name via RFC 7591 DCR). Public docs at `docs/mcp/`; marketing landing at `/mcp`. |
...
| apply-edl render invariants (Track 0.14) | `providers/video/apply-edl.ts`. (1) **Picture on one cumulative frame grid:** each cut gets exactly `round(cumEnd·F) − round(cumStart·F)` frames from its GLOBAL output position (`chunkStartSec`), and EVERY video chunk ends with `gridHold` (unbounded clone pad → keep exactly its grid count → timestamps rebuilt from the frame index), so no chunk seam can ever carry drift into the next. A chunk containing a crossfade is on the SAME grid (Track 0.16, `xfPlan`): each segment keeps its grid frames on the overlap-compressed timeline, and EVERY frame interval comes from one arithmetic — the running prefix in `chunkOutputSec`'s exact operation order — so the plan, `gridHold` and the next chunk's `chunkStartSec` agree even at an exact half-frame tie (two float orders of one sum round apart there), every xfade blends `accEndF − startF` whole frames at a frame-counted offset (a nominal-seconds offset cut real frames after short segments), a blend under one frame becomes a cut, and joins after a cut are `concat,setpts=N/FRAME_RATE/TB,fps=F` (Track 0.15: xfade needs matching timebases, and a bare `fps` dropped a frame at degenerate joins). Assert frame-exact colour runs (`frameRuns`), never sampled colours. (2) **Every source is held past its end:** picture reads start from `[i:V]tpad=clone`, sound reads from `[i:a]apad` — a window reaching past a track's end inside `SOURCE_END_TOLERANCE_SEC` renders a frozen last frame / silence for its full length instead of coming up short (a short segment pulled every later cut 0.4 s ahead of the continuous audio). The delivered length is `edlDurationMs(edl)`. An MPEG-TS/PS source (per-track ends not on the render's clock) is bounded by its container-declared duration + `DECLARED_END_TOLERANCE_SEC` (5 s) — an upper bound, so it never refuses a correct edit — so an overrun can't render minutes of frozen picture; ONLY when a packet scan shows every mapped track's DTS running continuously — never back, never forward past the CLI's 10 s `-dts_delta_threshold`: a joined/reconnected TS declares last-minus-first timestamp, LESS than the render plays through (18 s byte-joined file → 6 s declared), and a restarted clock >60 s below the first timestamp is "unwrapped" by libavformat into a ~26.5 h forward step (16 s file → ~25 h declared) — both skipped. Also: ffprobe `-of csv` answers are read through `csvFields` (`providers/video/ffprobe-csv.ts`) — TS prints each stream twice, and side data (rotated MP4, MPEG-2) appends an empty field; a plain split read 29.97 fps as 30000 → a 60 fps canvas. A failed scan elsewhere stays skipped too (a declaration can UNDER-report there). (3) **No video `filter_complex` wider than `VIDEO_FILTERGRAPH_MAX_SEGMENTS` (30):** on the CI/cloud runner a wider picture graph SILENTLY DROPS FRAMES (45 ok, 60 → −74, 90 → −629 with the pinned 8.1.2; not reproducible in Docker) — pre-existing for every wide video render. Test with **frame counts and cut timing**, never end-drift alone (a 60-graph dropped 74 frames at 0.01 s drift). A crossfade run past 30 is closed at a hard cut made inside one of its segments; only a run whose segments are all too short for their crossfades stays one wider graph. (4) **Every input is seeked** (`-ss` to its earliest read in the slice minus `INPUT_SEEK_MARGIN_SEC`, every trim rebased): `trim`/`atrim` run after the decoder, so an unseeked late chunk decoded its sources from t=0 (an hour-long 1080p edit's last chunk: 1,475 s vs its 1,200 s kill budget). Byte-identical to the unseeked render. A slice whose windows are SPARSE across a long source still decodes the span between them (per-input, not per-segment, seek). (5) **A chunked video render's audio is never chunk-concatenated as AAC** (encoder priming at every seam): chunks carry picture only (`omitAudio`); the audio renders in slices of the AUDIO plan (≤ `AUDIO_FILTERGRAPH_MAX_SEGMENTS` = 30 per slice, a crossfade run closed inside one of its segments, as for the picture) as lossless PCM, joined and AAC-encoded ONCE in the mux. A picture chunk that rounds to NO frame on the grid (slivers under half a frame in all) is not rendered — its sound still plays in that audio pass and the next chunk's grid position already counts it; the "keep one frame" rule (`gridFrames[0] = 1`, `gridHold(max(1, …))`) is only for a render that is ONE chunk, and the single-chunk output decision reads the PLAN (`chunks.length`), never the number of rendered files. A chunked AUDIO render's chunks are the same lossless slices, joined and AAC-encoded once (per-chunk AAC + a stream-copy join ended one AAC frame long). Only video chunks are checkpointed; a retry re-runs the audio, which takes seconds per slice. (6) **Resume keys are `sliceFingerprint`** — a hash of the exact command (graph, encode args, input seeks, source ids+URLs, ffmpeg build); never a hand-built key or scheme version (an unfingerprinted `chunk-<c>` key once spliced an old plan's chunks into a new one: completed, billed, evidence deleted). Checkpoints are deleted once the final output exists or the render is cancelled (`JobCancelledError` only — a retryable failure keeps them for the same-jobId retry); nothing else deletes `apply-edl-cache/` (no R2 lifecycle rule yet). (7) Graphs go to ffmpeg as a FILE (`-/filter_complex`): Linux caps one argv string at 128 KiB (spawn E2BIG), which a dev Mac never shows. |
...
| Combine-videos audio uniformity | `normalizeVideoForCombine` also pins audio to `-ar 44100 -ac 2` (same pin as `runBlockFit` in assemble-narrated-video.ts) — the cut fast path splices normalized clips with `-f concat -c copy`, which stamps the whole output audio track with the FIRST clip's decoder config; a later clip at another sample rate then plays slowed/pitch-shifted and desyncs (job 3dca9c76: 32kHz + 44.1kHz sources → 14s audio on a 10s video). `combineVideos` additionally probes audio presence up front (skipped for `audioMode="remove"`) and injects a silent 44.1kHz stereo AAC track into soundless clips when the set is MIXED — concat `-c copy` otherwise silently ends the audio track at the first soundless segment (rest of video mute), and the crossfade/keep filter graphs would error on the missing `[i:a]` pad. |
| Combine-videos audio crossfade (decoupled) | **Invariant: audio settings NEVER alter the video stream.** `audioCrossfadeDuration` (route/worker/`CombineVideosData`; falls back to `transitionDuration` for pre-split workflows) shapes ONLY the soundtrack. Hard-cut video (transition `cut`, or ANY transition at duration ≤ 0 — `hardCutVideo`) + crossfade renders audio in its own pass and muxes it onto the **stream-copied** concat video — byte-identical video to the plain-cut fast path (verified by video-stream MD5). The audio graph is `buildHardCutCrossfadeAudioFilter`, an anchored **L-cut**: every clip's audio `adelay`s to its exact video cut (no lead, no drift); each NON-last clip is `atempo`-stretched by `dur/(dur+d)` so its tail lingers `d` past its cut, fading out under the incoming fade-in; the LAST clip is untouched and ends exactly with the video. History (2026-07-12, do not regress to either): #3307's boundary fades = audible dropout at every cut; the acrossfade+apad chain that briefly replaced it = audio led video AND the last clip's sound ended `(n-1)·d` early leaving end silence (user bug). xfade transitions + crossfade use `buildAnchoredCrossfadeAudioFilter` (per-clip `afade` in/out at `audioCrossfadeDuration` + `adelay` to the clip's xfade video start + `amix`) — same anchoring invariants; last clip never faded out. **Voice-preserving smart boundaries (2026-07-25, "voice was cut"):** at a MATCHED smart-cut boundary under this L-cut graph, the prev clip's end trim is VIDEO-ONLY (`trimEdgeFrames` `preserveAudioTail` — the dropped frames are duplicated video but their audio is unique speech; measured xcorr |r|≈0.1 vs next's head), the clip's L-cut leg skips the atempo stretch (the REAL tail lingers), and the concat fallback re-bounds those clips' audio to the video length first (concat demuxer would drift on the longer track). Matched boundaries + this graph ONLY — manual/unmatched trims and every other join strategy stay symmetric. |
| Combine-videos smart cut | `smartCutEnabled` + `smartCutFramesPrev/Next` (route/worker/payload-builder/MCP `smart_cut*`): per boundary, `findSmartCutBoundary` (`providers/video/smart-cut.ts`, reuses smart-loop-cut's probe + sharp-PSNR at a 192px compare scale) searches the last N frames of clip k and the first M of clip k+1 on the NORMALIZED files, ends k ON the best-matching frame and starts k+1 right AFTER its match (duplicate plays once → continuous motion; built for i2v continuation clips). Results feed the per-clip `trimEdgeFrames` plan, REPLACING the fixed `trimStart/EndFrames` at boundaries (outer edges always 0); a failed boundary search falls back to the fixed trims for that boundary only. A pair must clear `SMART_CUT_MIN_PSNR_DB` (24dB at the compare scale; continuation twins ≥ ~28, unrelated clips ≤ ~15) to count as `matched` — unmatched boundaries KEEP the fixed trims (which the UI shows as the fallback). Applied per-boundary values are returned in `CombineVideosResult.smartCuts` and surfaced in the job's `output_data.smartCuts` (`matched: false` = fixed trims applied; `psnrDb: null` = search errored). PRECISION INVARIANT: `trimEdgeFrames` and combine's `durations[]` use `getVideoStreamDuration` (VIDEO stream), NOT `getVideoDuration` (container) — AI clips carry an audio track ~1-2 frames longer than the video, and a frame cut computed from the container duration lands past the last video frame and silently under-trims (trim-2 removed ZERO frames on real clips). |
...
| Deploy drain / SIGTERM contract | Container-stop signals reach node ONLY because `start.sh` (PID 1) traps TERM/INT and forwards to every child — NEVER `exec caddy` (pre-2026-07-16 topology: Caddy was PID 1, node siblings got SIGKILL, active BullMQ jobs died lock-held and rows froze at `processing` for lockDuration+stalled-check ≈ 15–20 min; incident 2026-07-15). On SIGTERM `worker.ts` calls `beginWorkerDrain()` (`lib/worker-drain.ts`) before `worker.close()`: the shared poll `sleep` in `providers/kie/client.ts` throws `DrainAbortError` at the next wait point, the video-worker catch RETHROWS it untouched (no mark-failed, no refund, ANY attempt), BullMQ requeues with the lock released, and the replacement process's stall guard inline-reconciles with a FULL poll budget (`ReconcileOpts.pollAttempts`; cron keeps its one-shot probe). Keep `start.sh`, `worker.ts`, and this chain in sync — a regression in any link silently reintroduces the 15-min blackout. **A paid call made INSIDE the worker is the opposite case** (a private plugin's Scene3D planner/critic stage, ≤360 s + persist): its answer exists only in this process and its journal marker makes an interrupted call unresolvable, so it is never aborted — `worker.ts` drains for the platform window (`inFlightDrainDeadlineMs()` reads `RAILWAY_DEPLOYMENT_DRAINING_SECONDS` minus a 5 s margin; 25 s when unset, and the platform's unset window measured ~11 s), the stage finishes and completes, and the NEXT claim made with `{ handOffOnDrain: true }` throws `DrainAbortError` before any journal write (`lib/private-plugins/stage-journal.ts`). The video-worker catch recognises a drain abort anywhere in the `cause` chain (`isDrainAbortError`) and requeues at zero attempt cost. `server.ts`/`orchestrator.ts`/`render-worker.ts` stay on the fast `SHUTDOWN_DRAIN_MS` (the API's in-process orchestrator never drains, so a long deadline would delay execution recovery). Incidents 2026-09-15: Pro jobs 351f0270 / 35bd1f1f refunded after a deploy killed a planner call. |
...
| `pending_review` is in-flight | A job held by a registered job policy (`lib/job-policy.ts`, design note `docs/design/job-policy-seam.md`) sits in `pending_review`: output written to storage, `output_data` NULL, the held payload in `jobs.held_*` columns (outside every public key list and outside migration 347's column grant, so it cannot ride PostgREST or Realtime), credits still `reserved`. It is **exempt from every reconcile, timeout, stall and cancel-in-flight sweep** — the ONE permitted exception is `sweepExpiredHolds`, gated on `JOB_HOLD_TTL_HOURS` (unset = disabled) — and every waiter treats it as non-terminal (`IN_FLIGHT_JOB_STATUSES` + `PARKED_JOB_STATUSES` in `lib/job-status.ts`; the partition test guards the sets). `markJobFailed`'s default `from` excludes it on purpose — only the review reject path passes `from: ["pending_review"]`; approve does its own CAS `pending_review → completed` and replays `runCompletionTail`, and `markJobCompleted`'s CAS is NEVER widened to admit it (a stray worker would complete a held row and re-enter the gate). A new sweep, a new hand-rolled terminal set, or a new `IN_FLIGHT` consumer that forgets it will either kill a job awaiting review or leak its reservation — `lib/__tests__/job-status-literals.test.ts` catches the hand-rolled set. |
| Internal-only models | Synthetic provider/model ids set by the orchestrator (never user-submitted) bypass the user-facing UI half of the Provider Enum Sync (steps 1, 2, 2b, 8, 8b, 11, 12, 12b). They MUST be in `STATIC_CREDIT_COSTS` + `model_pricing` seed + provider `supportedModels`. Workflow-run swap lives in `payload-builder.ts` (case `generate-image`); single-node Run uses a pre-Zod swap in `routes/generate-image.ts` via the `_internalLora` body hint AND a matching short-circuit in the inline `creditGuard` preHandler resolver (otherwise credits reserve as the default provider, 1cr silent under-bill). Reference: `flux-lora-character` — selected when a single trained `@character` mention is detected. Companion training id `character-lora-training` (Cloud only; read the price from `STATIC_CREDIT_COSTS`, not from here — this line said 150cr for a day after the ×10 re-denomination). |
...
Rendered audio/video output is **ffmpeg-version-dependent** (the 5.1→8 upgrade alone: `afir`'s flat ×2 intrinsic gain became an `irnorm` ℓ1-IR-normalization, default ON — which would have silently crushed every reverb wet leg 20–37 dB; found and fixed via the harness before shipping). Three interlocking guards, all load-bearing:

1. **The Dockerfile pins ffmpeg** (`ARG FFMPEG_TARBALL_URL_{AMD64,ARM64}` + `_SHA256_*`, runner stage — an exact BtbN dated-release static build, checksum-verified, immutable). NEVER "fix" a download/checksum failure by unpinning. Bumping the pins = a real ffmpeg upgrade: re-bless the characterization goldens inside the new image, review the `characterize:report` diff, update `DEFAULT_GOLDEN_FILE` (in `__characterization__/golden.ts`), and ship it all in ONE PR. The reverb's afir-gain handling is measured per-IR at runtime (`afirEffectiveGain` in `audio-fx.ts` — wet-leg `volume` compensation, NOT IR pre-scaling, because norm-based gain is scale-invariant); the probe verifies the gain is a FLAT scalar (peak vs energy ratio agreement) and on failure THROWS after one retry — never a guessed fallback gain, never a memoized failure (a wrong guess = silently mis-leveled reverb billed to a customer; a failed job refunds). If a future upgrade moves reverb wet-leg energy anyway, the probe design broke — fix it, don't re-bless.
