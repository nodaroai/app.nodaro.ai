...
| Combine-videos audio crossfade (decoupled) | **Invariant: audio settings NEVER alter the video stream.** `audioCrossfadeDuration` (route/worker/`CombineVideosData`; falls back to `transitionDuration` for pre-split workflows) shapes ONLY the soundtrack. Hard-cut video (transition `cut`, or ANY transition at duration ≤ 0 — `hardCutVideo`) + crossfade renders audio in its own pass (chained overlapping `acrossfade` + `apad` to the video total) and muxes it onto the **stream-copied** concat video — byte-identical video to the plain-cut fast path (verified by video-stream MD5). The overlap means post-boundary audio leads its picture by `d` per boundary — a DELIBERATE 2026-07-12 product decision reversing PR #3307's fade-through-silence (user: blend quality beats theoretical sync on ambient AI-clip tracks; dialogue-critical content should use `keep`). Do NOT "fix" this back to boundary fades. xfade transitions + crossfade use `buildAnchoredCrossfadeAudioFilter` (per-clip `afade` in/out at `audioCrossfadeDuration` + `adelay` to the clip's xfade video start + `amix`) — sync-safe by construction, so the audio fade length is independent of the video fade. |
