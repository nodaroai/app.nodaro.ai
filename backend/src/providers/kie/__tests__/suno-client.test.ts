...
describe("sunoGenerate — create-task body shape", () => {
  it("posts to /api/v1/generate with required fields + default model V6", async () => {
    fetchMock
...
describe("sunoCover", () => {
  ...
  it("default model V6 + customMode false + instrumental false", async () => {
    fetchMock
      .mockResolvedValueOnce(jsonResponse({ code: 0, data: { taskId: "t" } }))
      .mockResolvedValueOnce(recordInfoSuccess())
...
describe("sunoMashup", () => {
  it("posts to /api/v1/generate/mashup with upload_url_list", async () => {
    ...
    expect(body.model).toBe("V6") // default
  })
})
...
describe("sunoAddInstrumental", () => {
  it("posts to /api/v1/generate/add-instrumental with default model V6", async () => {
    fetchMock
      .mockResolvedValueOnce(jsonResponse({ code: 0, data: { taskId: "t" } }))
      .mockResolvedValueOnce(recordInfoSuccess())
