...
describe("handleCinematicAvatar — on the nodaro.ai connection", () => {
  it("relays the job to the connection and never calls HeyGen when the install has no key and is connected", async () => {
    ...
    const job = makeJob()
    const ctx = makeCtx()
    await handleCinematicAvatar(job as never, ctx as never)
    expect(cloudMocks.relayVideoJobToCloud).toHaveBeenCalledWith(job, ctx, "cinematic-avatar")
    expect(mocks.mockGenerateCinematicAvatar).not.toHaveBeenCalled()
