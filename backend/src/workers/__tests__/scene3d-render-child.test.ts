...
const mocks = vi.hoisted(() => ({
  ...
  upload: vi.fn(), watermark: vi.fn(), writePng: vi.fn(), cancellation: vi.fn(),
}))
vi.mock("../shared.js", () => ({ markJobCompletedDetailed: mocks.complete }))
...
vi.mock("../../lib/private-plugins/scene3d-write-json.js", () => ({ writeScene3DPng: mocks.writePng }))
...
describe("scene render worker lifecycle", () => {
  ...
  it("refuses completion after the parent loses its reservation during upload", async () => {
    ...
    await expect(processSceneRenderChild(job, async () => "bundle", {})).rejects.toThrow("reserved")
    expect(mocks.complete).not.toHaveBeenCalled(); expect(mocks.fail).toHaveBeenCalledOnce()
  })
