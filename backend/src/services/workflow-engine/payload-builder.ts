...
export function buildPayload(
  ...
  switch (type) {
    ...
    case "edit-plan": {
      ...
      // own data (video `duration`, upload-audio `metadata.durationSeconds`). A
      // URL/youtube master (`reference-audio`, direct-URL) carries its length in
      // NEITHER — its length isn't known until the audio is fetched — so it would
      // otherwise reserve the 180-minute ceiling (`edit-plan:<mode>:<tier>:180m`),
      // a ~3-6× overbill on a normal episode (incident: a 59.4-min standard
      // tighten reserved the 180m base 720 → 793 charged at a 10% markup, vs the
      // correct 60m base 240 → 264). Fall back to the TRANSCRIPT's own clock: it
      // is a REQUIRED input (thrown for above) and the timing map of this same
      // master, so its last word's endMs buckets a 59.4-min episode to 60m. Only
      // a truly underivable duration (no source length AND an empty transcript)
      // takes the ceiling — the safe over-reserve direction. The cloud plugin
      // re-probes the master and money-GATES (refuses, never overcharges) if the
      // probed length exceeds the reserved bucket, so an under-bucketed reserve
    ...
    case "video-upscale": {
      ...
      const vuModel = vuProvider === "veo-1080p" ? "veo-1080p"
        ...
        : "topaz-video"
      // The VEO upscalers re-render KIE's own VEO task — they cannot take a URL,
      // so an uploaded (or any non-VEO) video satisfies the media table
      // (videoUrl) yet dies at the route's Zod ("VEO upscale requires kieTaskId")
      // after the job row and the reservation exist. Refuse here, at build time,
      // with the fix spelled out (2026-09-09: an uploaded clip + provider VEO 1080p).
      const vuVeo = vuProvider === "veo-1080p" || vuProvider === "veo-4k"
      if (vuVeo && !(resolvedInputs.kieTaskId || data.kieTaskId)) {
        const vuLabel = typeof data.label === "string" && data.label.trim() ? data.label : "Upscale Video"
        throw codedMediaRefusal(
          "video_required",
          `node "${vuLabel}" is set to VEO upscale, which only works on a video generated by VEO earlier in this workflow — connect that VEO node, or switch the provider to Topaz for an uploaded or non-VEO video.`,
        )
      }
      return {
    ...
    // --- Transcription / OCR ---
    case "transcribe": {
      const provider = (data.provider as string) ?? DEFAULT_TRANSCRIBE_NODE_PROVIDER
