...
export function registerRecipeTool(server: McpServer, session: McpSession): void {
  ...
  server.registerTool(
    "get_recipe",
    {
      title: "Get Recipe",
      description:
        "Discover and load multi-step Nodaro content recipes — proven playbooks for known content goals (a narrated explainer, a consistent-character composite, …). Call with NO argument to list available recipes (name, description, trigger phrases) and load the matching one BEFORE building. Call with `recipe` to load that recipe's full instructions; add `file` to read a bundled reference file inside it. The \"video-explainer\" recipe is for explainers told through generated animated footage; for kinetic-typography/motion-graphics explainers use start_video_director instead. When the user hasn't specified a style, the recipe itself asks. On Nodaro Cloud the listing also includes a UGC website video: a creator talking to camera about a site, with its screenshots as cards.",
      inputSchema: {
