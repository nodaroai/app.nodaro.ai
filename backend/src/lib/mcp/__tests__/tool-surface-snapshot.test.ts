...
// reconcile-on-read) and nowhere else; six descriptions said the media "lands
// by itself", which reads as "no further step", and a user polling
// `wait_for_job` to `completed` therefore saw an empty film. The six now name
// the read that lands them and say that `get_job` / `wait_for_job` land
// nothing, and the read says out loud that it is also the write. MEASURED by
// this suite: 347_887 total − 347_305 = 582 B for seven descriptions. No tool
// was added, so the fixture does not move, and none of the seven is near the
// per-tool budget (the largest in the list is `animate_image` at 8_122 B).
