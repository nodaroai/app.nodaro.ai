...
describe("listProviderCredentialStates", () => {
  it("reports set/source per provider without ever exposing a value", async () => {
    ...
    expect(states.find((s) => s.id === "kie")).toEqual({ id: "kie", set: true, source: "env" })
    expect(states.find((s) => s.id === "fal")).toEqual({ id: "fal", set: true, source: "app" })
    expect(states.find((s) => s.id === "gemini")).toEqual({ id: "gemini", set: false, source: null })
