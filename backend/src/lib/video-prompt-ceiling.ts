 ...
 *
 * The direction fold's truncation warning needs that same number: thresholding
 * on the raw cap under-fires on exactly the runs that get truncated (a prompt
 * under the cap but over `cap - suffix` loses its tail silently). This is the
 * one place the reservation is mirrored, so the two video routes cannot drift
 * from each other — and a change to the suffix shape has one call site to fix.
 */
