...
const RAW_NODE_REGISTRY: NodeDescriptor[] = [
  ...
  },
  {
    ...
    inputSchema: {
      fields: [
        ...
        { key: "prompt", type: "text" },
        { key: "resolution", type: "select", options: ["480p", "720p", "1080p"] },
  ...
  { type: "still-to-video", label: "Still to Video", category: "processing", description: "Turn one still image + one audio track into an MP4 with an optional motion effect (zoom / pan / Ken Burns). Local FFmpeg — no provider, no GPU, zero credits. The output duration is the audio's duration (no duration field).", outputType: "video", creditCost: 0, inputSchema: { fields: [
    { key: "imageUrl", type: "image-url", required: true },
    { key: "audioUrl", type: "audio-url", required: true },
    { key: "motion", type: "select", options: ["none", "zoom-in", "zoom-out", "pan-left", "pan-right", "ken-burns"] },
    { key: "intensity", type: "number" },
    { key: "resolution", type: "select", options: ["720p", "1080p", "4K"] },
    { key: "aspectRatio", type: "select", options: ["16:9", "9:16", "1:1", "4:3"] },
    { key: "fps", type: "select", options: ["24", "30"] },
    { key: "fit", type: "select", options: ["cover", "contain"] },
    { key: "padColor", type: "text" },
  ] } },
  { type: "slideshow", label: "Slideshow", category: "processing", description: "Turn 2-100 images + one optional audio track into an MP4 slideshow with per-slide motion (zoom / ken-burns / alternate) and transitions. Local FFmpeg — no provider, no GPU, zero credits. Audio wired: the output duration IS the audio's duration (never cropped); unwired: N x perImageDuration, silent output. Per-image duration overrides supported via imageDurations (null = auto).", outputType: "video", creditCost: 0, inputSchema: { fields: [
    { key: "imageUrls", type: "image-url-array", required: true },
    { key: "audioUrl", type: "audio-url" },
    { key: "imageDurations", type: "number-array" },
    { key: "perImageDuration", type: "number" },
    { key: "transition", type: "text" },
    { key: "transitionDuration", type: "number" },
    { key: "motion", type: "select", options: ["none", "zoom-in", "zoom-out", "ken-burns", "alternate"] },
    { key: "intensity", type: "number" },
    { key: "resolution", type: "select", options: ["720p", "1080p", "4K"] },
    { key: "aspectRatio", type: "select", options: ["16:9", "9:16", "1:1", "4:3"] },
    { key: "fps", type: "select", options: ["24", "30"] },
  ...
  { type: "composition-effects",  label: "Composition Effect",   category: "parameter", description: "Pick a composition effect from 19 entries (none, bursting-through-frame, pixel-disintegration, ...). Emits a composition prompt fragment via the cinematography handle.", outputType: "text" },
