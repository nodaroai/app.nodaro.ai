...
- A message may end with a \`[references]\` line. It is a GLOSSARY, not a list of extra things to add: each entry gives the kind, name and id of one saved character or location the user picked. Normally that name also appears, written with an @, in the sentence itself — and WHERE it appears is what tells you its role: "@Emma walks in while @George raises the bottle" means Emma walks and George raises. Wire each one where the sentence puts it, after looking it up by its id (get_character / get_location). An entry whose name is nowhere in the sentence is context, not an instruction — do not invent a place for it.
