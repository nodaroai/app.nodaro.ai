...
import { objectRestoreRoutes } from "../object-restore.js"
...
const TEST_OBJECT_ID = "00000000-0000-4000-8000-000000000030"
...
beforeEach(async () => {
  ...
  await app.register(async (instance) => {
    await objectRestoreRoutes(instance)
  })
  await app.ready()
})
...
function mockRestoreChains(opts: {
  ...
  const archived = opts.archived ?? {
    data: { id: TEST_OBJECT_ID, name: "Compass", deleted_at: "2026-05-01T00:00:00Z" },
    error: null,
  }
  const collision = opts.collision ?? { data: null, error: null }
  const update = opts.update ?? {
    data: { id: TEST_OBJECT_ID, name: "Compass" },
    error: null,
  }
...
describe("POST /v1/objects/:id/restore", () => {
  it("returns 401 when unauthenticated", async () => {
    const res = await app.inject({
      method: "POST",
      url: `/v1/objects/${TEST_OBJECT_ID}/restore`,
    })
  ...
  it("returns 400 on invalid id param", async () => {
    const res = await app.inject({
      method: "POST",
      url: `/v1/objects/not-a-uuid/restore`,
  ...
  it("happy path: archived row → 200 with restored { id, name } and UPDATE clears deleted_at", async () => {
    ...
    expect(res.json()).toEqual({ id: TEST_OBJECT_ID, name: "Compass" })
    ...
    expect(updatePayload.name).toBe("Compass")
    ...
    expect(updateEq).toHaveBeenCalledWith("id", TEST_OBJECT_ID)
  ...
  it("auto-suffixes '(restored)' when an active row already owns the name", async () => {
    const stubs = mockRestoreChains({
      // Collision: another active row owns "Compass"
      collision: { data: { id: "other-active-id" }, error: null },
      update: { data: { id: TEST_OBJECT_ID, name: "Compass (restored)" }, error: null },
    })
    vi.mocked(supabase.from).mockImplementation(stubs.next)
    ...
    expect(res.statusCode).toBe(200)
    expect(res.json()).toEqual({ id: TEST_OBJECT_ID, name: "Compass (restored)" })

    // The UPDATE patch must use the suffixed name.
    const updatePayload = stubs.updateFn.mock.calls[0]?.[0] as Record<string, unknown>
    expect(updatePayload.name).toBe("Compass (restored)")
    expect(updatePayload.deleted_at).toBe(null)
  })

  it("returns 404 'not_found' when the object does not exist (uniform code per Pass 10 F-90b)", async () => {
    const stubs = mockRestoreChains({
  ...
  it("returns 404 'not_found' when the object is cross-user (uniform code, no leak)", async () => {
  ...
  it("returns 404 'not_found' when the row exists but is already active (uniform code — diverges from location's idempotent-200)", async () => {
    // Per spec Pass 10 F-90b + Pass 3 F-32, object DELIBERATELY diverges
    ...
    const stubs = mockRestoreChains({
      archived: {
        data: { id: TEST_OBJECT_ID, name: "Compass", deleted_at: null },
        error: null,
      },
    })
    vi.mocked(supabase.from).mockImplementation(stubs.next)
