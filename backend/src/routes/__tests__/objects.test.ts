...
describe("POST /v1/objects", () => {
  ...
  it("UPDATE accepts a partial body without name/nodeId (boards-only write)", async () => {
    ...
    // 400ing the studio's partial writes ({id, boards} for Product Boards,
    ...
    const res = await app.inject({
      ...
      payload: {
        id: TEST_OBJECT_ID,
        userId: TEST_USER_ID,
        boards: [{ name: "Brass", url: "https://cdn.example/brass.png" }],
      },
    })

    expect(res.statusCode).toBe(200)
    const patch = mockUpdate.mock.calls[0][0] as Record<string, unknown>
    expect(patch.boards).toEqual([{ name: "Brass", url: "https://cdn.example/brass.png" }])
  ...
  it("persists board type + sourceImages (identity sheet) without stripping", async () => {
    ...
    const res = await app.inject({
      method: "POST",
      url: "/v1/objects",
      payload: { id: TEST_OBJECT_ID, userId: TEST_USER_ID, boards },
    })
  ...
  it("INSERT still requires name + nodeId (enforced in the handler)", async () => {
    const res = await app.inject({
      method: "POST",
      url: "/v1/objects",
      payload: { userId: TEST_USER_ID, name: "Sword" },
    })
    expect(res.statusCode).toBe(400)
    expect(res.json().error.code).toBe("validation_error")
  })
 ...
 */
describe("GET /v1/objects — pagination", () => {
  ...
  it("?limit=N over-fetches by one and slices the probe row back off", async () => {
    const { mockSelect, chainable } = mockListChain({ data: rowsOf(3), error: null })
    vi.mocked(supabase.from).mockReturnValue({ select: mockSelect } as never)
    const res = await app.inject({ method: "GET", url: "/v1/objects?limit=2" })
    expect(res.statusCode).toBe(200)
    expect(chainable.limit).toHaveBeenCalledWith(3)
    expect(res.json().objects).toHaveLength(2)
  ...
  it("a returned cursor round-trips into the keyset predicate", async () => {
    ...
    const page1 = await app.inject({ method: "GET", url: "/v1/objects?limit=2" })
    ...
    const page2 = await app.inject({ method: "GET", url: "/v1/objects?limit=2&cursor=" + encodeURIComponent(cursor) })
  ...
  it("an undecodable cursor is a 400, not a silent page 1", async () => {
    const { mockSelect } = mockListChain({ data: [], error: null })
    vi.mocked(supabase.from).mockReturnValue({ select: mockSelect } as never)
    const res = await app.inject({ method: "GET", url: "/v1/objects?limit=2&cursor=%%%garbage" })
    expect(res.statusCode).toBe(400)
  })
  it("cursor without limit is a 400 — not a filtered unbounded legacy fetch", async () => {
    ...
    const { mockSelect } = mockListChain({ data: [], error: null })
    vi.mocked(supabase.from).mockReturnValue({ select: mockSelect } as never)
    const res = await app.inject({ method: "GET", url: "/v1/objects?cursor=" + encodeURIComponent(valid) })
    expect(res.statusCode).toBe(400)
  })
  ...
  it("orders by created_at THEN id — the tie-break that makes the order total", async () => {
    ...
    vi.mocked(supabase.from).mockReturnValue({ select: mockSelect } as never)
    await app.inject({ method: "GET", url: "/v1/objects?limit=2" })
