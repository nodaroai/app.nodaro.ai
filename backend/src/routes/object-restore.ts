 ...
 *
 * Un-archive a soft-deleted object. Mirrors the location-restore pattern
 * (`location-restore.ts`) with two object-specific deltas:
 *
 *   1. **Uniform `"not_found"` 404 code** (spec Pass 10 F-90b + Pass 3
 *      F-32): all failure paths — doesn't-exist / cross-user / already-
 *      active — return the SAME 404 code. Object DELIBERATELY diverges
 *      from location's per-path codes + location's idempotent-200 on
 *      already-active to prevent callees from enumerating object IDs by
 *      observing which paths 404 vs which return 200.
 *
 *   2. **No idempotent-200 on already-active**: location returns 200 if
 *      the row is already active (deleted_at IS NULL). Object returns
 *      404 "not_found" instead — the failure surface is uniform.
 *
 * Name-collision handling (preserved from location): if another active row
 ...
 * Objects have no `LOWER(name)` unique index (unlike characters), so the
 ...
 * existing DELETE route in objects.ts (Phase C2 — matches location
 * convention).
 */
  ...
  app.post("/v1/objects/:id/restore", async (req, reply) => {
    ...
    if (!parsed.success) {
      return reply.status(400).send({
        error: { code: "validation_error", message: "Invalid object ID" },
      })
    }
    ...
    // uniform "not_found" 404 (object diverges from location's
    ...
    // name. Case-insensitive (ilike) so "compass" and "Compass" are
    // treated equal.
    ...
    if (updateErr || !updated) {
      return reply.status(500).send({
        error: { code: "restore_failed", message: updateErr?.message ?? "Failed to restore object" },
      })
    }
