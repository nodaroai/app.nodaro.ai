...
export async function extractAudioRoutes(app: FastifyInstance) {
  app.post("/v1/extract-audio", { preHandler: creditGuard(() => "extract-audio") }, async (req, reply) => {
    ...
    const modelIdentifier = "extract-audio"
    ...
    const { data: job, error } = await supabase
      ...
      .insert({
        ...
        user_id: userId,
        status: "pending",
        input_data: buildJobInputData(parsed.data, "extract-audio"),
      })
      .select("id")
      .single()
