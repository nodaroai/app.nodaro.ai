...
/**
 * Creature main-image approval + canonical-description LLM caption.
 *
 * POST /v1/creatures/:id/approve-main-image — body { candidateJobId }
 *   Promotes a candidate-generation job's output image to the creature's
 *   permanent `source_image_url` AND fires the Claude Sonnet vision caption
 *   inline to populate `canonical_description`.
 *
 * Mirrors `object-main-image-approval.ts` with key security-hardening
 * deltas per the Creature Studio spec:
 *   - Both candidate + creature are fetched in parallel via Promise.all
 *     (spec Pass 3 F-25) — both must succeed BEFORE the ~5-30s paid LLM
 *     call. Without the creature pre-fetch, PostgREST `.update().eq().eq()`
 *     silently succeeds on zero rows and we'd leak LLM output for a
 *     creature the caller doesn't own (soft IDOR).
 *   - Both queries are scoped by `req.userId`, so cross-user candidate /
 *     creature returns null → 404 (indistinguishable from "doesn't exist").
 *   - Creature pre-fetch ALSO filters `deleted_at IS NULL` — archived
 *     creatures are 404 (cannot resurrect-by-approve).
 *   - Cross-link IDOR gate (spec Pass 3 F-26): the candidate's
 *     `input_data.attachToCreatureId` MUST match the URL `:id`. Mismatch
 *     returns 400 `candidate_creature_mismatch` (NOT 404 — the caller is
 ...
 * "not yours / archived / malformed-row" failure paths — creature is
 ...
 *   4. candidate.input_data.attachToCreatureId !== :id → 400
 *      "candidate_creature_mismatch" (Pass 3 F-26 IDOR gate)
 *   5. creature not found / cross-user / archived → 404 "not_found"
 *
 * Creature 404 evaluates AFTER the candidate gates so a known-bad candidate
 * id doesn't leak whether the creature exists.
 *
 * Caption-failure semantics (matches object):
 ...
 *
 * TODO Phase 2: deduct 1 CR per LLM call (matches the object precedent's
...
const bodySchema = z.object({
   ...
   * Mirrors the pattern in POST /v1/creatures (Phase B).
   */
  expectedUpdatedAt: z.string().datetime().optional(),
})

export async function creatureMainImageApprovalRoutes(app: FastifyInstance) {
  app.post(
    "/v1/creatures/:id/approve-main-image",
    {
      config: {
    ...
    async (req, reply) => {
      ...
      // it here with a clean 503 instead. Mirrors the object precedent.
      ...
      // ----- Parallel fetch: candidate job + creature pre-fetch (spec Pass 3 F-25) -----
      // Both must succeed before the paid LLM call. Both are scoped by
      // user_id so a cross-user candidate / creature returns null (mapped
      // to 404). Creature pre-fetch also rejects archived rows.
      //
      // We select `input_data` on the candidate so we can run the
      // cross-link IDOR check before paying for the LLM call.
      const [candidateResult, creatureResult] = await Promise.all([
        supabase
          .from("jobs")
      ...
      //   4. candidate.input_data.attachToCreatureId !== :id (400, Pass 3 F-26)
      //   5. creature 404 (archived or cross-user — uniform "not_found")
      // Creature 404 evaluates AFTER candidate gates so a known-bad
      // candidate id doesn't leak whether the creature exists.
      ...
      // `attachToCreatureId` MUST match the URL `:id`. Without this gate, a
      // user could "approve" candidate-A into creature-B even though A was
      // generated against creature-A. Both rows are owned by the caller so
      ...
      const candidateAttachId =
        typeof candidateInput.attachToCreatureId === "string" ? candidateInput.attachToCreatureId : null
      if (candidateAttachId !== null && candidateAttachId !== creatureId) {
        return reply.status(400).send({
          error: {
            code: "candidate_creature_mismatch",
            message: "Candidate was generated for a different creature",
          },
        })
      }

      const { data: creatureRow, error: creatureErr } = creatureResult
      ...
      // captionCreature() swallows internally and returns null on any
      ...
      const caption = await captionCreature(imageUrl)
      ...
      if (updateErr || !updated) {
        ...
        // path. Mirrors the pattern in `routes/creatures.ts` (Phase B).
        if (expectedUpdatedAt) {
      ...
      // CreatureNodeData.canonicalDescription type stays consistent.
      return reply.send({
