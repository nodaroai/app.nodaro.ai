...
const upsertLocationBody = z.object({
  ...
  // Named Location Boards (dense reference sheets generated by Studio via the
  // location-board factory preset — one per variant/mood). USER-owned: a
...
const SELECT_COLUMNS =
  "id, user_id, node_id, project_id, name, description, category, style, source_image_url, image_provider, time_of_day, weather, angles, lighting, seasons, atmosphere_motions, boards, reference_photos, canonical_description, style_lock, pii_consent_at, selected_asset_by_variant, sheets, detail_closeups, deleted_at, created_at, updated_at"
...
export async function locationRoutes(app: FastifyInstance) {
  ...
  app.get("/v1/locations", async (req, reply) => {
    ...
    const { page, nextCursor } = sliceKeysetPage((data ?? []) as LocationRow[], limit)
    return { locations: page.map((loc) => toCamel(loc)), nextCursor }
  })
