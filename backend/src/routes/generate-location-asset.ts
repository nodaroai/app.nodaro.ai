...
export async function generateLocationAssetRoutes(app: FastifyInstance) {
  app.post("/v1/generate-location-asset", async (req, reply) => {
    ...
    if (config.EDITION !== "self-hosted" && userId) {
      try {
        ...
        console.error("[generate-location-asset] Credit check failed:", err)
    ...
    if (config.EDITION !== "self-hosted" && userId) {
      try {
        ...
        console.error("[generate-location-asset] Credit reservation failed:", err)
