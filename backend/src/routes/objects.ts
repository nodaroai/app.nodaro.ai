...
const upsertObjectBody = z.object({
  ...
  // Named Product Boards (dense reference sheets generated by Studio via the
  // product-board factory preset — one per variant/colorway). USER-owned: a
...
const SELECT_COLUMNS =
  "id, user_id, node_id, project_id, name, description, category, style, source_image_url, image_provider, angles, materials, variations, motion_clips, boards, reference_photos, canonical_description, style_lock, selected_asset_by_variant, sheets, detail_closeups, deleted_at, created_at, updated_at"
...
export async function objectRoutes(app: FastifyInstance) {
  ...
  app.post("/v1/objects", { preHandler: requireAppScope("assets:write") }, async (req, reply) => {
    ...
    if (name === undefined || !nodeId) {
      return reply.status(400).send({
        error: {
          code: "validation_error",
          message: "name and nodeId are required when creating a new object (id omitted).",
        },
      })
    }
    const insertRow = {
  ...
  // deleting one take — e.g. a material variant the user wants to regenerate
  // — goes through the atomic `remove_object_asset` RPC (migration 242): a
  // single SQL statement filters the entry out, immune to the read-modify-write
  // race the upsert guard exists to prevent. Mirrors the locations.ts handler.
  // ---------------------------------------------------------------------------
  app.post("/v1/objects/:id/remove-asset", { preHandler: requireAppScope("assets:write") }, async (req, reply) => {
    const userId = req.userId
    if (!userId) {
      return reply.status(401).send({
        error: { code: "unauthorized", message: "Authentication required" },
    ...
    const parsedBody = z
      .object({ column: z.enum(OBJECT_ATTACH_COLUMNS), url: safeUrlSchema })
      .safeParse(req.body)
    ...
    const { data: removed, error } = await supabase.rpc("remove_object_asset", {
      p_object_id: parsedParams.data.id,
      p_user_id: userId,
    ...
    if (!removed) {
      ...
      return reply.status(404).send({
        error: { code: "not_found", message: "Asset not found on this object" },
      })
    }
    return { removed: true }
  })
  ...
  app.delete("/v1/objects/:id", { preHandler: requireAppScope("assets:write") }, async (req, reply) => {
    ...
    if (isPermanent) {
      ...
      const keys = await deletableKeys(collectObjectR2Keys(row as Record<string, unknown>))
