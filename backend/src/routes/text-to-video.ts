...
export async function textToVideoRoutes(app: FastifyInstance) {
  app.post("/v1/text-to-video", { preHandler: creditGuard((req) => { const body = req.body as Record<string, unknown>; return buildVideoCreditModelIdentifier((body?.provider as string) ?? "minimax", body?.duration as number | string | undefined, body?.sound as boolean | undefined, "text-to-video", body?.mode as string | undefined) }) }, async (req, reply) => {
    ...
    const { prompt, provider, duration, mode, sound, negativePrompt, cfgScale, aspectRatio, multiShot, shots, elements, removeWatermark, seed, characterIdList } = parsed.data
    ...
    const { data: job, error } = await supabase
      ...
      .insert({
        ...
        user_id: userId,
        status: "pending",
        input_data: { prompt, provider, duration, mode, sound, negativePrompt, cfgScale, aspectRatio, multiShot, shots, elements, removeWatermark, seed, characterIdList, type: "text-to-video" },
      })
      .select("id")
      .single()
