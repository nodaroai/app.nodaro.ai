...
try {
  ...
  for (const [suffix, sameHash] of [[990, false], [991, true]]) {
    ...
    await admin.query("INSERT INTO auth.users(id,email,raw_user_meta_data,aud,role) VALUES($1,$2,'{}','authenticated','authenticated')",
      [id, `retained-concurrency-${suffix}@test.invalid`])
    created.push(id)
    ...
    const results = await Promise.allSettled(clients.map((client, index) => client.query(
      "SELECT public.reserve_retained_image($1,$2,$3,800,10,10,'image/png','enforce') AS snapshot",
      [id, workflowId, (sameHash || index === 0 ? "a" : "b").repeat(64)],
    )))
    const fulfilled = results.filter((result) => result.status === "fulfilled")
    assert.equal(fulfilled.length, sameHash ? 2 : 1)
    if (sameHash) assert.equal(fulfilled[0].value.rows[0].snapshot.id, fulfilled[1].value.rows[0].snapshot.id)
    else assert.match(results.find((result) => result.status === "rejected").reason.message, /Storage limit exceeded/)
    const { rows } = await admin.query("SELECT storage_used_bytes,(SELECT count(*) FROM public.retained_images WHERE user_id=$1) AS snapshots FROM public.profiles WHERE id=$1", [id])
    assert.equal(Number(rows[0].storage_used_bytes), 800)
    assert.equal(Number(rows[0].snapshots), 1)
  }
  process.stdout.write("Retained image concurrency proofs passed: quota isolation and identical-byte deduplication\n")
} finally {
