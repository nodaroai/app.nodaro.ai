...
export const MOTION_GRAPHICS_PRESETS: readonly FactoryPreset[] = [
  ...
  // ── FX Overlays ───────────────────────────────────────────────────────────
  {
    ...
    data: {
      ...
      backgroundColor: "#00000000",
      motionPrompt:
        "A celebratory confetti burst that overlays footage. Small vector confetti pieces erupt from a point near the lower-center, fountaining up and outward then drifting down under a gentle gravity-like ease, tumbling slowly and fading out as they fall so the frame clears by the end. Build it compactly: author ONE small group-wrapped rectangle or ellipse per color — colorA, colorB, colorC, colorD — and multiply each with a repeater (8–14 copies, with per-copy position/rotation offsets in the repeater's own transform so the copies fan out instead of stacking). Stagger the four groups' launch a few frames apart for an organic burst — never author individual confetti pieces. KEEP THE JSON COMPACT: at most 4 shape layers and 3–4 keyframes per animated property. Linear-ish motion on launch easing into a soft fall. Transparent background — overlays footage, so add no backdrop. One-shot, not looped: the burst plays once and fully clears before the out point. Expose slots: colorA, colorB, colorC, colorD.",
    },
  },
  {
