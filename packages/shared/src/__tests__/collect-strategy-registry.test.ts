...
import { COLLECT_STRATEGIES, COLLECT_STRATEGY_IDS, getStrategy } from "../collect-strategy-registry"

describe("collect-strategy-registry", () => {
  it("registers exactly the 6 v1 strategies (snapshot)", () => {
    expect(COLLECT_STRATEGY_IDS).toEqual([
      "pick-best-llm",
      "concat",
      "first-non-empty",
      "count",
  ...
  it("every strategy has id, label, description, configSchema, defaultConfig, outputType, creditCostKey", () => {
    for (const s of COLLECT_STRATEGIES) {
      ...
      expect(s.creditCostKey).toMatch(/^collect:/)
    }
  })
  ...
  it("getStrategy throws on unknown id", () => {
    expect(() => getStrategy("nope" as never)).toThrow(/unknown collect strategy/i)
  })
})
