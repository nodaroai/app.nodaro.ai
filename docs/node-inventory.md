...
**Scope:** the 185 entries of `NODE_DEFINITIONS` (`frontend/src/types/nodes.ts`) plus the 2 deprecated type strings the loader still migrates (`loop` → `list`, `ai-writer` → `llm-chat`) = **187 rows**. One entry, `preview`, is retired (soft-delete, 2026-08): it stays in `NODE_DEFINITIONS` so saved workflows keep loading, but is no longer creatable — see its row.
...
| what it does | `NODE_REGISTRY.description` in `backend/src/lib/node-registry.ts` (173 types); `docs/nodes/README.md` for the rest; `(inferred)` marks sentences written from the implementation because no descriptor exists |
...
| `dubbing` | Dubbing | ai | Translate spoken audio into another language while preserving the original speaker's voice and identity | `audio`:audio | `audio`:audio | NO |
...
| `generate-music` | Generate Music | ai | Generate music with Suno | `prompt`:text, `ref-audio`:audio, `audio-style`:audio | `audio`:audio | NO |
...
| 32 video-result nodes: `add-captions`, `ai-avatar`, `assemble-narrated-video`, `cinematic-avatar`, `combine-videos`, `component`, `edit-video-pro`, `extend-video`, `face-swap`, `fade-video`, `generate-video`, `generate-video-pro`, `lip-sync`, `loop-video`, `merge-video-audio`, `motion-transfer`, `remove-audio`, `render-video`, `resize-video`, `social-media-format`, `speech-to-video`, `speed-ramp`, `suno-music-video`, `switchx`, `transcode-video`, `trim-video`, `upload-video`, `video-sfx`, `video-to-video`, `video-upscale`, `voice-changer`, `voice-changer-pro` | `openFreeCut(...)` from `use-workflow-store` → same FreeCut iframe editor | post-hoc timeline edit of an already-produced result (optional; re-uploads as a new result) |
