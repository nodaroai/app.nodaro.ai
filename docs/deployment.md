...
| `STUDIO_PRODUCTIONS_API` | off | The studio production API (`/v1/studio/productions/*`) and its MCP tool family — read, validate, create and extend a Nodaro Studio production from an agent. Off = 404 on every route and the tools are absent from `tools/list`, which is the feature-detect a client uses |
