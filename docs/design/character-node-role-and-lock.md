...
- **Identity-lock control** on its **own compact sub-row** (mirroring the `${style}·${gender}` sub-row at `:370`), not crammed next to the truncating name. Writes `identityLock` (`off/soft/strict`), default-display `DEFAULT_IDENTITY_LOCK` (now `"off"`). Same field the config panel (`config-panels/entity-configs.tsx`), Character Studio (`character-studio/pages/profile-page.tsx`), and `lib/character-node-data.ts` already read/write, so all stay in sync. All three unset-display fallbacks now read the shared `DEFAULT_IDENTITY_LOCK` constant (the earlier node-`"soft"` / Studio-`"strict"` divergence is resolved).
