...
**Inside a shot's time window.** When the transition is written into one shot of a multi-shot prompt (`0-2s — …`, `2-4s — …`), the Position wording refers to that shot, not the whole clip: `the transition occurs in the middle of this shot` instead of `… of the clip`. `@nodaro/prompts` callers opt in with `composeTransitionHintFromConnections(id, startHints, endHints, timing, mode, { scope: "shot" })`; without it the wording is the clip's.
