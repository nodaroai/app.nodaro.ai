...
**1. Never attach the clay render without a scoping line.** One sentence per reference, naming what it is *for* and what to *ignore*. Wire the Render Video output into a video node's **Video references** input and a **workflow run** adds the line for that reference itself (the video node's own Run button does not add it yet — type it into the prompt there); through the API, pass it as the reference's caption — `referenceVideoCaptions[N]` for the clip on `referenceVideoUrls[N]`. The line the platform sends for a clip is:
