...
| Number images | Switch | Off | Stamp a 1-based sequence number at each image's **top-right** — storyboard mode. Numbers count `1, 2, 3…` in the collage's final reading order (drag rows to renumber). On the API this is `numbered` (boolean; absent/`false` = off). |
