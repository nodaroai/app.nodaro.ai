...
- **Avatar picker** — searchable tile grid fed live from the HeyGen API. Filters: gender, **Stock / Custom** (shown only when the account has looks of its own — *Custom* means the account's own photo-avatar looks, *Stock* the HeyGen presets), and Avatar V support. Preview thumbnails show how the avatar looks. Your own looks are always listed first.
