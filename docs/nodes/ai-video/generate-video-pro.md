...
| Shot timestamps | Checkbox | Off | A/B lever — injects each beat's time range (rebased to the segment's own clock) into **Hybrid / Hybrid Plus / Condensed** segment prompts, which are timestamp-free by default. No effect on Faithful split / Slot-anchored (they keep your script's own timestamps either way) |
