...
> **Pre & post text:** every AI prompt node can carry [`promptPrefix` / `promptSuffix`](../prompt-pre-post-text.md) — settings-only text wrapped around the prompt at run time (hidden from app users, captured by presets).
...
| [Upscale Image](./ai-image/upscale-image.md) | Increase image resolution (Recraft, Topaz) | Enhance generated or uploaded images at 2x or 4x |
...
| [Paint Mask](./ai-image/paint-mask.md) | Hand-painted mask as a reusable canvas value (free, no AI) | Brush a mask over a connected image, or hand-refine a Generate Mask output, and feed it to any mask input |
...
| [Describe to Picker](./ai-image/describe-to-picker.md) | Analyze an image with a vision model → catalog-valid picker JSON (Person) to auto-fill a parameter picker | Turn a reference photo into a Person casting brief; recurring-character setup |
...
| Node | Description | When to Use |
|------|-------------|-------------|
| [Generate Video](./ai-video/generate-video.md) | Unified video producer — text-only, image-to-video, first+last frame, reference mode, or video-edit (V2V) driven by which inputs are wired (`VIDEO_GEN_PROVIDERS` catalog); providers include VEO 3.x, Gemini Omni (Pro & Flash), Kling, Seedance 2, Wan 3.0, LTX 2.3, and more | Animate images, generate from text, run reference-driven multimodal mode (Seedance 2, VEO 3.x), or edit existing video (Gemini Omni) — one node, all modes |
...
| [Render Video](./composition/render-video.md) | Finalize composition to video file | Render any composition plan to final video output; a 3D scene is priced by frame size |
