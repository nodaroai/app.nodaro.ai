...
- A reference can carry a default: `{Label || fallback}` uses the trimmed `fallback` when nothing provides `Label`. `{Label || }` (empty after `||`) resolves to nothing
- On node types that compose their prompt through the shared prompt composer, a plain `{Label}` (no `||`) naming a node that exists but produced nothing also resolves to empty text; a `{Label}` naming no node anywhere in the workflow refuses the run with an `unresolved_reference` error before anything is dispatched or charged. Text that arrives through a wired input (rather than a `{Label}` reference) is data, not a reference, and is never rewritten
