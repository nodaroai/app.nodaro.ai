...
| `song-from-reference` | A song similar to an existing one — a YouTube link or uploaded track carried by a `reference-audio` node into a Suno cover, with an honest fork between covering the song and chasing its vibe | "similar song", "cover this song", "make a song from this link", and similar |
