...
| `song-from-reference` | A song similar to an existing one — a YouTube link or uploaded track carried by a `reference-audio` node into a Suno cover, with a style-analysis fallback (`video-analysis` hears the source and writes a style brief) that composes an original song when the cover is refused on rights grounds | "similar song", "cover this song", "make a song from this link", and similar |
