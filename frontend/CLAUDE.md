...
- **`common.*`** holds ~90 generic buttons and states (`common.loading`, `common.retry`, `common.remove`, …) — reuse before adding a per-feature duplicate. The `he.ts` AND `ja.ts` value for every `en.ts` key is mandatory (`i18n.test.ts`, `ja-dictionary.test.ts`), and config-panel namespaces must contain Hebrew letters.
