...
import { isValidUpscaleImageConnection } from "@/lib/image-producer-handles"
...
import type { UpscaleImageData } from "@/types/nodes"
...
function UpscaleImageNodeComponent({ id, data, selected }: NodeProps) {
  ...
  const creditModelId = buildCreditModelIdentifier(
    nodeData.provider ?? "recraft-upscale",
    nodeData as unknown as Record<string, unknown>,
  )
  const credits = useModelCredits(creditModelId, 2)
  ...
  return (
    ...
    <HandleWithPopover nodeId={id} nodeType="upscale-image" handleId="image" type="target" position={Position.Left}  label="Image" color={HANDLE_COLORS.image} icon={<ImageIcon />} side="left"  top="calc(100% - 24px)" accepts={ACCEPTS_IMAGE} />
    <HandleWithPopover nodeId={id} nodeType="upscale-image" handleId="image" type="source" position={Position.Right} label="Image" color={HANDLE_COLORS.image} icon={<ImageIcon />} side="right" top="24px" />
    {activeUrl && (
      <MediaPreviewModal
        isOpen={previewOpen}
...
export const UpscaleImageNode = memo(UpscaleImageNodeComponent)
