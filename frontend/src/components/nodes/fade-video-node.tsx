...
import { SunDim, Loader2, AlertCircle, X, Clapperboard, Film, Maximize } from "lucide-react"
...
function FadeVideoNodeComponent({ id, data, selected }: NodeProps) {
  ...
  return (
    <div className="relative" style={{ maxWidth: '220px' }}>
      <EditableNodeLabel label={nodeData.label} icon={<SunDim className="w-3.5 h-3.5" />} onSave={(newLabel) => updateNodeData(id, { label: newLabel })} />
    <BaseNode id={id} label={nodeData.label} icon={<SunDim className="h-4 w-4" />} category="processing" credits={credits} selected={selected} isRunning={status === "running"}
      hideHeader
      minWidth={220}
    ...
    >
      <div className="flex flex-col gap-1">
        ...
        {status !== "running" && !activeUrl && status !== "failed" && (
          <div className="flex items-center justify-center h-16 rounded-md border-2 border-dashed border-muted-foreground/20 text-muted-foreground/40"><SunDim className="w-5 h-5" /></div>
        )}
        {results.length > 1 && (
...
export const FadeVideoNode = memo(FadeVideoNodeComponent)
