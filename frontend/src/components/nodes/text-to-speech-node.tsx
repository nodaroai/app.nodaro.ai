...
function TextToSpeechNodeComponent({ id, data, selected }: NodeProps) {
  ...
  return (
    ...
    <HandleWithPopover nodeId={id} nodeType="text-to-speech" handleId="prompt" type="target" position={Position.Left}  label="Prompt" color={TEXT_HANDLE_COLOR} icon={<Type />}    side="left"  top="calc(100% - 24px)" accepts={ACCEPTS_PROMPT} />
    <HandleWithPopover nodeId={id} nodeType="text-to-speech" handleId="audio"  type="source" position={Position.Right} label="Audio"  color={HANDLE_COLORS.audio} icon={<Volume2 />} side="right" top="24px" />
