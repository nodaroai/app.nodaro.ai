    ...
    >
      <div className="flex flex-col gap-2 p-3" style={{ minHeight: 180 }}>
        ...
        <div className="flex justify-between text-muted-foreground">
          <span className="text-xs">{t("node.sunoCoverModel", { model: nodeData.model ?? DEFAULT_SUNO_MODEL })}</span>
          {nodeData.title && <span className="text-xs truncate max-w-[120px]">{nodeData.title}</span>}
        </div>
      </div>
    </BaseNode>
    <HandleWithPopover nodeId={id} nodeType="suno-cover" handleId="audio"  type="target" position={Position.Left}  label="Audio"  color={HANDLE_COLORS.audio} icon={<Disc3 />} side="left"  top="calc(100% - 24px)" accepts={ACCEPTS_AUDIO} />
    <HandleWithPopover nodeId={id} nodeType="suno-cover" handleId="prompt" type="target" position={Position.Left}  label="Prompt" color={TEXT_HANDLE_COLOR} icon={<Type />}  side="left"  top="calc(100% - 56px)" accepts={ACCEPTS_PROMPT} />
    <HandleWithPopover nodeId={id} nodeType="suno-cover" handleId="voice"  type="target" position={Position.Left}  label="Voice"  color={HANDLE_COLORS.identity} icon={<Mic />}   side="left"  top="calc(100% - 88px)" accepts={ACCEPTS_VOICE} />
    <HandleWithPopover nodeId={id} nodeType="suno-cover" handleId="audio"  type="source" position={Position.Right} label="Audio"  color={HANDLE_COLORS.audio} icon={<Disc3 />} side="right" top="24px" />
...
export const SunoCoverNode = memo(SunoCoverNodeComponent)
