...
import { UserRound, Loader2, AlertCircle, Film, Volume2, Type, Expand, Download, Link, X, LayoutGrid, Scissors, Settings } from "lucide-react"
...
function AiAvatarNodeComponent({ id, data, selected }: NodeProps) {
  ...
  return (
    <div className="relative" style={{ width: "100%", height: "100%" }}>
      ...
      {/*
        Both input handles are ALWAYS mounted (voice-changer pattern — never unmount a
        handle, as there is no edge-pruner and unmounting would orphan existing edges).
        The handle NOT matching speechMode is `disabled` to block new connections.
      */}
      <HandleWithPopover
        nodeId={id}
        nodeType="ai-avatar"
