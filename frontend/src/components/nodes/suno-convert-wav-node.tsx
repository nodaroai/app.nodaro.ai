...
import { FileAudio, Loader2, AlertCircle, Volume2 } from "lucide-react"
...
function SunoConvertWavNodeComponent({ id, data, selected }: NodeProps) {
  ...
  const credits = useModelCredits("suno-convert-wav", 1)
  ...
  return (
    ...
    <EditableNodeLabel label={nodeData.label} icon={<FileAudio className="w-3.5 h-3.5" />} onSave={(newLabel) => updateNodeData(id, { label: newLabel })} />
    <BaseNode id={id} label={nodeData.label} icon={<FileAudio className="h-4 w-4" />} category="ai" credits={credits} selected={selected} isRunning={status === "running"} hideHeader topToolbarContent={status !== "running" ? (<RunNodeButton nodeId={id} credits={credits} isRunning={false} onRun={(nid) => runSingleNode?.(nid)} />) : undefined} handles={[]}>
      <div className="flex flex-col gap-2 p-3" style={{ minHeight: 180 }}>
        ...
        {status !== "running" && !audioUrl && status !== "failed" && (<div className="flex items-center justify-center rounded-md border-2 border-dashed border-muted-foreground/20 text-muted-foreground/40" style={{ minHeight: 120, flex: 1 }}><FileAudio className="w-5 h-5" /></div>)}
    ...
    <div className="absolute pointer-events-none z-20 flex items-center justify-center w-7 h-7 rounded-full bg-[#ff0073] shadow-lg shadow-pink-500/30" style={{ top: '50px', right: '-29px' }}><FileAudio className="w-3.5 h-3.5 text-white" /></div>
    </div>
  )
}

export const SunoConvertWavNode = memo(SunoConvertWavNodeComponent)
