...
function TranscodeVideoNodeComponent({ id, data, selected }: NodeProps) {
  ...
  return (
    <div className="relative group/node" style={{ width: hasResult ? (videoDimensions?.width ?? 220) : 220, height: hasResult ? (videoDimensions?.height ?? 160) : undefined, overflow: 'visible' }}>
        ...
        {hasResult ? null : (
          <div className="flex flex-col gap-1">
            ...
            {status !== "running" && !activeUrl && status !== "failed" && (
              <div className="flex items-center justify-center h-16 rounded-md border-2 border-dashed border-muted-foreground/20 text-muted-foreground/40"><RefreshCw className="w-5 h-5" /></div>
            )}
            {results.length > 1 && (
            ...
            <p className="text-muted-foreground text-xs">{nodeData.codec === "h265" ? "H.265" : "H.264"} / CRF {nodeData.crf ?? 23}</p>
          </div>
        )}
      </BaseNode>
      {hasResult && (
      ...
      <HandleWithPopover nodeId={id} nodeType="transcode-video" handleId="video" type="target" position={Position.Left}  label="Video" color={HANDLE_COLORS.video} icon={<Film />} side="left"  top="calc(100% - 24px)" />
      <HandleWithPopover nodeId={id} nodeType="transcode-video" handleId="video" type="source" position={Position.Right} label="Video" color={HANDLE_COLORS.video} icon={<Film />} side="right" top="24px" />
...
export const TranscodeVideoNode = memo(TranscodeVideoNodeComponent)
