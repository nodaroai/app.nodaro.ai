...
function VoiceChangerNodeComponent({ id, data, selected }: NodeProps) {
  ...
  // compact waveform layout (no aspect, default sizing) untouched.
  const { aspectRatio: mediaAspectRatio, onLoadDimensions: handleLoadDimensions } =
    useResultAspectRatio(id, results, activeIndex)

  // The video overlay (absolutely positioned, fills the node) only shows once a
  // video result is loaded without error — same gate as loop-video/remove-audio.
  ...
  return (
    ...
    >
      {hasVideoResult ? null : (
        <div className={`flex flex-col gap-2 ${isVideoResult ? "" : "p-3"}`} style={{ minHeight: isVideoResult ? undefined : 180 }}>
          ...
          {/* Audio mode only — the rich audio player. Video results render via the
              VideoResultOverlay sibling below (it fills the transparent node). */}
    ...
    {/* Video result: rich player overlay filling the transparent node — same
        controls (expand / download / copy / save / settings) the standard
        video nodes have. */}
    {hasVideoResult && (
      <VideoResultOverlay
        url={activeUrl!}
