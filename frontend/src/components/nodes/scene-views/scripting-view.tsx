  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-baseline justify-between">
        ...
        <div className="text-[10px] text-zinc-500">{data.emotional_beat}</div>
      </div>
      <div className="font-medium text-sm truncate">
        {data.label ?? data.description ?? "Untitled scene"}
      </div>
      {data.description && (
        <p className="text-[11px] text-zinc-600 line-clamp-2">{data.description}</p>
      )}
      {shots.length === 0 ? (
        ...
        <div className="flex flex-col gap-1.5">
          {shots.map((shot) => (
            <div
              key={shot.shot_id}
              className="flex flex-col gap-0.5 rounded border border-zinc-200 bg-zinc-50/60 p-1.5"
            >
              <div className="flex items-center justify-between text-[9px] text-zinc-500">
                <span className="font-mono">{shot.shot_id}</span>
                <span>{shot.duration_seconds}s · {shot.camera.shot_type}</span>
              </div>
              <p className="text-[11px] text-zinc-700 line-clamp-2">{shot.action}</p>
              {shot.dialogue_line && (
                <p className="text-[11px] italic text-zinc-600 line-clamp-2">
