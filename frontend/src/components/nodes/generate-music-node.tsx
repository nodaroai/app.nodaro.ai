...
function GenerateMusicNodeComponent({ id, data, selected }: NodeProps) {
  ...
  return (
    ...
    {/* Invisible input handle - prompt */}
    <Handle
      id="in"
      type="target"
      position={Position.Left}
      className="!w-7 !h-7 !bg-transparent !border-0 !opacity-0 touch-manipulation"
      style={{ top: '50px', left: '-29px', transform: 'none' }}
    />
    {/* Invisible input handle - ref audio */}
    <Handle
      id="ref-audio"
      type="target"
      position={Position.Left}
    ...
    <div
      ...
      style={{ top: '50px', left: '-29px' }}
    >
      <Type className="w-3.5 h-3.5 text-white" />
    </div>
    {/* Input handle icon - ref audio */}
    <div
      className="absolute pointer-events-none z-20 flex items-center justify-center w-7 h-7 rounded-full bg-[#ff0073] shadow-lg shadow-pink-500/30"
