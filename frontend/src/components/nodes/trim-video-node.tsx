...
import { Scissors, Loader2, AlertCircle, X, Film, LayoutGrid } from "lucide-react"
...
function TrimVideoNodeComponent({ id, data, selected }: NodeProps) {
  ...
  return (
    <div className="relative group/node" style={{ width: '100%', height: '100%', overflow: 'visible' }}>
          ...
          <div className="flex flex-col gap-1">
            ...
            {status !== "running" && activeUrl && videoError && (
              <div className="relative group">
                ...
                <div className="absolute bottom-1 right-1 bg-black/70 text-white text-[10px] px-1 rounded">{t("node.trimmed")}</div>
                {results.length > 0 && (
      ...
      <HandleWithPopover nodeId={id} nodeType="trim-video" handleId="in"        type="target" position={Position.Left}  label="Video" color={FFMPEG_COLORS.video} icon={<Film />}     side="left"  top="calc(100% - 24px)" accepts={ACCEPTS_VIDEO} />
      <HandleWithPopover nodeId={id} nodeType="trim-video" handleId="video-out" type="source" position={Position.Right} label="Video" color={FFMPEG_COLORS.video} icon={<Film />}     side="right" top="24px" />
