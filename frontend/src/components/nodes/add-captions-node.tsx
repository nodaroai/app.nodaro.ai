...
import { Captions, Loader2, AlertCircle, X, Clapperboard, Film, Maximize } from "lucide-react"
...
function AddCaptionsNodeComponent({ id, data, selected }: NodeProps) {
  ...
  return (
    ...
    <BaseNode id={id} label={nodeData.label} icon={<Captions className="h-4 w-4" />} category="processing" credits={credits} selected={selected} isRunning={status === "running"}
      hideHeader
      minWidth={220}
    ...
    >
      <div className="flex flex-col gap-1">
        ...
        {status !== "running" && activeUrl && (
          <div className="relative group">
            {activeThumbnail ? (
              ...
              <video src={activeUrl} className="w-full h-28 object-cover rounded-md" controls playsInline />
            )}
            <button
              type="button"
              aria-label="Expand"
        ...
        {status !== "running" && !activeUrl && status !== "failed" && (
          <div className="flex items-center justify-center h-16 rounded-md border-2 border-dashed border-muted-foreground/20 text-muted-foreground/40"><Captions className="w-5 h-5" /></div>
        )}
        {results.length > 1 && (
