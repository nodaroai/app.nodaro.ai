...
import { VolumeX, Loader2, AlertCircle, X, Film, LayoutGrid } from "lucide-react"
...
import type { RemoveAudioData } from "@/types/nodes"

// Remove Audio: video in → silent video out. Modeled on resize-video-node
// (the canonical single-in/single-out video-output FFmpeg node).
function RemoveAudioNodeComponent({ id, data, selected }: NodeProps) {
  ...
  return (
    <div className="relative group/node" style={{ width: hasResult ? (videoDimensions?.width ?? 220) : 220, height: hasResult ? (videoDimensions?.height ?? 160) : undefined, overflow: 'visible' }}>
      <EditableNodeLabel label={nodeData.label} icon={<VolumeX className="w-3.5 h-3.5" />} onSave={(newLabel) => updateNodeData(id, { label: newLabel })} />
      <BaseNode id={id} label={nodeData.label} icon={<VolumeX className="h-4 w-4" />} category="processing" credits={credits} selected={selected} isRunning={status === "running"}
          ...
          <div className="flex flex-col gap-1">
            ...
            {status !== "running" && activeUrl && videoError && (
              <div className="relative group">
                ...
                <div className="absolute bottom-1 right-1 bg-black/70 text-white text-[10px] px-1 rounded">Silent</div>
                {results.length > 0 && (
            ...
            {status !== "running" && !activeUrl && status !== "failed" && (
              <div className="flex items-center justify-center h-16 rounded-md border-2 border-dashed border-muted-foreground/20 text-muted-foreground/40"><VolumeX className="w-5 h-5" /></div>
            )}
            <p className="text-muted-foreground">Remove Audio</p>
          </div>
        )}
      </BaseNode>
      ...
      <HandleWithPopover nodeId={id} nodeType="remove-audio" handleId="in"        type="target" position={Position.Left}  label="Video" color={FFMPEG_COLORS.video} icon={<Film />} side="left"  top="calc(100% - 24px)" accepts={ACCEPTS_VIDEO} />
      <HandleWithPopover nodeId={id} nodeType="remove-audio" handleId="video-out" type="source" position={Position.Right} label="Video" color={FFMPEG_COLORS.video} icon={<Film />} side="right" top="24px" />
...
export const RemoveAudioNode = memo(RemoveAudioNodeComponent)
