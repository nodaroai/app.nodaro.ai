...
import { Maximize2, Loader2, AlertCircle, X, Play } from "lucide-react"
...
function ResizeVideoNodeComponent({ id, data, selected }: NodeProps) {
  ...
  return (
    ...
    <BaseNode id={id} label={nodeData.label} icon={<Maximize2 className="h-4 w-4" />} category="processing" credits={1} selected={selected} isRunning={status === "running"}
      handles={[
