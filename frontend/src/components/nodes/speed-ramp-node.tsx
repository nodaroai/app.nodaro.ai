...
import { Gauge, Loader2, AlertCircle, X, Clapperboard, Film, Maximize } from "lucide-react"
...
function SpeedRampNodeComponent({ id, data, selected }: NodeProps) {
  ...
  return (
    <div className="relative" style={{ maxWidth: '220px' }}>
      <EditableNodeLabel label={nodeData.label} icon={<Gauge className="w-3.5 h-3.5" />} onSave={(newLabel) => updateNodeData(id, { label: newLabel })} />
      ...
      <div className="flex flex-col gap-1">
        ...
        {status !== "running" && activeUrl && !videoError && (
          <div className="relative group">
            ...
            <div className="absolute bottom-1 right-1 bg-black/70 text-white text-[10px] px-1 rounded">{speedLabel}</div>
            {results.length > 0 && (
        ...
        {status !== "running" && !activeUrl && status !== "failed" && (
          <div className="flex items-center justify-center h-16 rounded-md border-2 border-dashed border-muted-foreground/20 text-muted-foreground/40"><Gauge className="w-5 h-5" /></div>
        )}
        {results.length > 1 && (
        ...
        <p className="text-muted-foreground">{speedLabel}</p>
      </div>
    </BaseNode>
...
export const SpeedRampNode = memo(SpeedRampNodeComponent)
