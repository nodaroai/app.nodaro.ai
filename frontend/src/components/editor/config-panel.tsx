...
function NodeTypeConfig({ nodeType, nodeData, configProps, updateNodeData, onExpandDirector, update, selectedNodeId }: {
  ...
  switch (nodeType) {
    ...
    case "pro-3d-render": return <Pro3DRenderConfig {...configProps} nodeId={selectedNodeId} />
    ...
    case "reference-board": return <ReferenceBoardConfig {...configProps} nodeId={selectedNodeId} />
    ...
    <div className="flex flex-col border-b border-gray-200 dark:border-[#2D2D2D] bg-white dark:bg-[#1E1E1E] shrink-0">
      <div className="flex items-center justify-between px-4 py-3">
        <h3 className="text-xs font-semibold uppercase tracking-widest text-gray-700 dark:text-[#ff0073]">
          {t("configPanel.nodeSettingsTitle", { name: localizeNodeLabel(nodeTypeDefaultLabel(nodeType)) })}
        </h3>
            ...
            <div className="flex items-center justify-between px-4 pb-2">
              <div className="flex items-center gap-2 min-w-0">
                <span className="text-[10px] font-semibold uppercase tracking-widest text-gray-400 dark:text-[#64748B]">
                  {localizeNodeLabel(nodeTypeDefaultLabel(nodeType))}
                </span>
                <span className="text-sm font-medium text-gray-900 dark:text-white truncate">
                  {localizeNodeLabel((selectedNode.data as { label: string }).label)}
                </span>
              </div>
      ...
      <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto overscroll-contain bg-[#F8FAFC] dark:bg-[#121212]">
        <div className="flex flex-col gap-5 p-4">
          <div className="rounded-xl border border-gray-200 dark:border-[#2D2D2D] bg-white dark:bg-[#1E1E1E] p-3 shadow-sm">
            ...
            {/* Shows the localized default (like the canvas header); a typed
                name persists verbatim, and typing the localized default back
                restores the English default — see config-panel-label.ts. */}
            <Input
              id="node-label"
              value={nodeLabelFieldValue((selectedNode.data as { label: string }).label, locale)}
              onChange={(e) => update({ label: nodeLabelFieldCommit(e.target.value, nodeTypeDefaultLabel(nodeType), locale) })}
          ...
          {sources.length > 0 && (
            <div className="text-xs text-gray-500 dark:text-[#94A3B8] bg-gray-100 dark:bg-[#2D2D2D] rounded-lg px-3 py-2 border border-gray-200 dark:border-[#2D2D2D]">
              ...
              {": "}
              {sources.map((s) => localizeNodeLabel(s.label)).join(", ")}
            </div>
          )}
