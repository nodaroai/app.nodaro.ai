...
function executeNodeCore(
  ...
  if (node.type === "generate-video") {
    ...
    // A connected source video routes to image-to-video ONLY for the Gemini Omni
    // family (gemini-omni-video + gemini-omni-flash — its V2V mode), matching the
    // backend payload-builder's gemini-scoped override. Other providers keep the
    // start-frame-only i2v/t2v split — including Wan 3, whose reference videos are
    // a t2v-compatible reference array, not a V2V source.
    const rawProvider = (node.data as { provider?: string } | undefined)?.provider;
    const hasGeminiVideoRef =
      isGeminiOmniProvider(rawProvider) &&
    ...
    const gvpRequest = {
      ...
      segmentDurations:
        Array.isArray(gvpData.segmentDurations) && gvpData.segmentDurations.length > 0
          ...
          : undefined,
      // RENDER METHOD — mirrors payload-builder's coercion so the Run button
      // and a workflow run dispatch identically. `extend` sends the previous
      // segment's tail as a reference VIDEO, which only the r2v family accepts;
      // a provider that cannot extend has exactly one method available, so an
      // absent choice resolves to it rather than 400ing at reserve. (This path
      // never sent the field at all before 2026-08-05, which is why keyframes
      // was only reachable through the orchestrator.)
      renderMethod:
        gvpData.renderMethod === "keyframes" || !supportsExtendRender(gvpData.provider)
          ? ("keyframes" as const)
          : undefined,
      // ANCHOR MODE — how much a keyframes run pins its shots to pre-generated
      // stills. Translated through the SAME shared resolver payload-builder
      // uses, so the Run button and a workflow run request the identical mode.
      // "auto"/absent resolves to undefined, keeping older payloads unchanged.
      anchorMode: resolveGvpAnchorWire(gvpData.anchorMode),
      ...
      // Best-pair search windows — sent for EVERY mode (the engine ignores
      // them under a pre-roll mode), narrowed to the engine's accepted range
      // so a stale/garbage node value can't 400 the whole run.
      smartCutFramesPrev: clampSmartCutWindow(gvpData.smartCutFramesPrev),
      smartCutFramesNext: clampSmartCutWindow(gvpData.smartCutFramesNext),
      idempotencyKey,
    };
  ...
  if (node.type === "video-analysis") {
    ...
    return new Promise<string>((resolve, reject) => {
      startVideoAnalysis({
        ...
        // Cast-variations opt-in — same literal-true narrowing as the levers below.
        variations: d.variations === true ? true : undefined,
