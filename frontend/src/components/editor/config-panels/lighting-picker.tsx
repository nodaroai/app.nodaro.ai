...
import { LIGHTINGS, LIGHTING_CATEGORY_ORDER, LIGHTING_CATEGORY_LABELS, LIGHTING_FIELD_BY_CATEGORY, type Lighting, type LightingCategory, type LightingValue, pickIds, togglePick } from "@nodaro/shared"
    ...
    <div className="flex flex-col gap-1.5 border-t-[3px] border-border/40">
      ...
      <div role={multi ? "group" : "radiogroup"} aria-label={label} className={cn("grid grid-cols-3 gap-1.5 transition-opacity", !checked && "opacity-40")}>
        {lightings.map((lighting) => {
          ...
          return (
            <div key={lighting.id} className="relative">
              <button
                type="button"
              ...
              {multi && selected && (
                <MultiPickBadge
                  ...
                  index={selectedIdx}
                  maxSelected={maxSelected}
                  onActivate={() => onActivateMulti(lighting.id)}
                  onDemote={() => onDemoteToSingle(lighting.id)}
                />
              )}
            </div>
          )
