...
export function SunoGenerateConfig({ data, onUpdate, sources, fieldMappings, onMapField, nodeRefs, refMap, variableDisplayMode }: ConfigProps<SunoGenerateData>) {
  return (
    <div className="flex flex-col gap-3">
      ...
      {(data.sunoTaskId || data.sunoTrackId) && (
        <div className="flex flex-col gap-2 pt-2 border-t border-border">
          ...
          {data.sunoTaskId && (
            <div className="flex flex-col gap-0.5">
              ...
              <div className="text-[11px] font-mono bg-muted/40 px-2 py-1 rounded break-all select-all">{data.sunoTaskId}</div>
            </div>
          )}
          {data.sunoTrackId && (
            <div className="flex flex-col gap-0.5">
              <label className="text-[10px] text-muted-foreground">Track ID</label>
              <div className="text-[11px] font-mono bg-muted/40 px-2 py-1 rounded break-all select-all">{data.sunoTrackId}</div>
            </div>
          )}
        </div>
      )}
