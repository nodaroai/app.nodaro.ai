 ...
 *  `VoicePage` unmounts on every studio navigation (StudioShell renders only the
...
export function VoicePage({ state }: StudioPageProps<CharacterStudioState, CharacterStudioJobs>) {
  ...
  async function speakAndLipSync() {
    if (!v?.voiceId || !state.staged.sourceImageUrl) return
    ...
    try {
      ...
      const ls = await lipSyncApi(state.staged.sourceImageUrl, audio)
  ...
  const hasPortrait = !!state.staged.sourceImageUrl
