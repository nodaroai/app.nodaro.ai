import type { StudioPageProps } from "../../studio-shell/types"
import type { LocationStudioState } from "../use-location-studio"
import type { LocationStudioJobs } from "../use-location-studio-jobs"
import { MotionTab } from "../motion-tab"

/**
 * Motion page — thin wrapper binding the existing `MotionTab` (atmosphere video
 * clips) to the shell's page props. The tab owns its own jobs hook internally.
 */
export function MotionPage({ state }: StudioPageProps<LocationStudioState, LocationStudioJobs>) {
  return (
    <div className="flex-1 overflow-y-auto p-4">
      <MotionTab studio={state} />
    </div>
  )
}
