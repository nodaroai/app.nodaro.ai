...
export function MotionTab({ studio }: MotionTabProps) {
  ...
  useEffect(() => {
    jobs.onFailed((jobId) => {
      toast.error(tx("creature.motionGenerationFailed", { id: jobId.slice(0, 8) }))
    })
  }, [jobs.onFailed])
  ...
  async function handleGenerateAll(): Promise<void> {
    ...
    if (missing.length >= 4) {
      if (!window.confirm(tx("creature.confirmQueueMotionJobs", { n: missing.length }))) return
    }
  ...
  async function handleCustom(): Promise<void> {
    ...
    if (trimmed.length > 2000) {
      toast.error(tx("creature.customMotionPromptTooLong"))
      return
    }
  ...
  const presetTooltip = noSourceImage ? t("entity.approveMainImageFirst") : undefined

  return (
    <div className="space-y-4 max-w-4xl">
      <div className="flex items-center justify-between">
        <h2 className="text-[12px] font-medium text-slate-300">{t("creature.motionClips")}</h2>
        <button
          type="button"
      ...
      {noSourceImage && (
        ...
        >
          {t("creature.motionNeedsSourceFrame")}
        </div>
      )}
          ...
          >
            ...
            <div className="absolute top-1 end-1 flex gap-1 opacity-0 group-hover:opacity-100 z-10">
              <button
                ...
                aria-label={t("creature.enlargeNameAria", { name: item.name })}
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder={t("creature.customMotionPromptPlaceholder")}
          disabled={disabled}
          title={presetTooltip}
