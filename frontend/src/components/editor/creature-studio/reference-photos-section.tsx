...
/**
 * Reference Photos grid for Creature Studio — mood-board style.
 *
 * Mirrors the object-studio precedent (reference-photos-section.tsx) verbatim
 * — creatures reuse the object reference-photo shape (ObjectReferencePhoto)
 * and the same 6 kinds (front/side/detail/context/moodBoard/other). Like
 * objects there is NO piiConsentAt UI — creatures never reference real-world
 * people so the location consent flow doesn't apply.
 *
...
export function ReferencePhotosSection({ photos, onChange }: ReferencePhotosSectionProps) {
  ...
  // human label (e.g. "mood board"), and the trailing filename in the URL.
  // Show input only when the grid is large enough to need it.
