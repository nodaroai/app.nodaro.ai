...
/**
 * Creature Studio job-status hook.
 *
 * Studio-side analog of useObjectStudioJobs (which itself mirrors the
 * location precedent). Supabase Realtime is the PRIMARY signal —
 * `useJobsRealtimeSync` subscribes to UPDATE events on the `jobs` table
 * filtered by user_id and fires the same `onResolved` / `onFailed`
 * callbacks the polling tick fires. The existing batch-polling stays as a
 * FALLBACK (for clients where realtime drops, RLS edge cases occur, or the
 * websocket is offline) throttled to 10s now that realtime carries the
 * latency budget. Uses the batch `getJobStatusBatch` (GET /v1/jobs/status)
 * endpoint instead of N parallel `getJobStatus` calls — the studio commonly
 * tracks 1/2/4 candidates at once so the batch shape pays for itself even at
 * low N. Adds a small ±200ms jitter to spread load when multiple studios are
 ...
 * is re-imported here verbatim — there's no creature-specific deviation in
...
export interface CreatureStudioJobs {
...
export function useCreatureStudioJobs(initial: ReadonlyArray<TrackedJob> = []): CreatureStudioJobs {
