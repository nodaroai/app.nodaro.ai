...
export function MediaEditorModal({ editor }: MediaEditorModalProps) {
  ...
  // Build header info chips
  const headerChips: string[] = []
  if (mediaType === "image" || mediaType === "video") {
    headerChips.push(outputInfo.isCropped ? `${outputInfo.outW}x${outputInfo.outH}` : `${outputInfo.nw}x${outputInfo.nh}`)
  }
  if ((mediaType === "video" || mediaType === "audio") && outputInfo.origDuration > 0) {
    headerChips.push(formatDurationCompact(outputInfo.isTrimmed ? outputInfo.outDuration : outputInfo.origDuration))
  }
  headerChips.push(outputInfo.outFormat)
  if (outputInfo.isCropped || outputInfo.isTrimmed) {
    headerChips.push(`~${formatBytes(outputInfo.estimatedSize)}`)
  } else {
    headerChips.push(formatBytes(outputInfo.origSize))
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              {headerChips.map((chip, i) => (
                <span key={i} className="text-[11px] text-white/35 bg-white/5 px-2 py-0.5 rounded">
                  {chip}
                </span>
              ))}
            </div>
...
function formatDurationCompact(seconds: number): string {
  ...
  return ms > 0 ? `${s}.${String(ms).padStart(2, "0")}s` : `${s}s`
}
