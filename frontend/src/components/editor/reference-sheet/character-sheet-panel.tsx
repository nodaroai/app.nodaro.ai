...
export function CharacterSheetPanel({ adapter, studio, jobs, accent }: Props) {
  ...
  async function onPrepare(): Promise<void> {
    if (!sourceImageUrl) { toast.error(tx("entity.approveMainImageFirst")); return }
    setStatus("preparing"); setProgressLine(tx("entity.sheetPreparing"))
    try {
      const dbId = await adapter.ensureSaved(studio)
      const missing = cost.missing
      for (let i = 0; i < missing.length; i++) {
        const req = missing[i]
        setProgressLine(tx("entity.sheetPreparingProgress", { i: i + 1, total: missing.length, variant: req.variant }))
      ...
      setStatus("idle"); setProgressLine(""); toast.success(tx("entity.anglesPrepared"))
    } catch (e) {
      setStatus("error"); setProgressLine(""); toast.error(e instanceof Error ? e.message : tx("entity.prepareFailed"))
    }
  }

  async function onCompose(): Promise<void> {
    if (!sourceImageUrl) { toast.error(tx("entity.approveMainImageFirst")); return }
    setStatus("composing"); setResultUrl(null); setProgressLine(tx("entity.sheetComposing"))
    try {
      const dbId = await adapter.ensureSaved(studio)
      ...
      setPrepared({}); setStatus("done"); setProgressLine(""); toast.success(tx("entity.sheetGenerated"))
    } catch (e) {
      setStatus("error"); setProgressLine(""); toast.error(e instanceof Error ? e.message : tx("entity.composeFailed"))
    }
  }
  ...
  return (
    <div className="space-y-5 max-w-4xl">
      ...
      <div className="space-y-1.5">
        <div className="text-[9px] uppercase tracking-widest text-slate-600">{t("txtcfg.preset")}</div>
      ...
      {/* À-la-carte boards */}
      <div className="space-y-1.5">
        <div className="text-[9px] uppercase tracking-widest text-slate-600">{t("entity.addBoards")}</div>
          ...
          {ALA_CARTE_BOARDS.map((b) => {
            ...
            return (
              <button
                ...
                disabled={disabled}
                title={overflow ? t("entity.wouldExceedPanels", { max: MAX_PANELS_PER_SHEET }) : undefined}
      ...
      <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
        <div className="space-y-1.5">
          <div className="text-[9px] uppercase tracking-widest text-slate-600">{t("cfgext.refSheetSkinLabel")}</div>
          <div className="flex flex-wrap gap-2">
            {SHEET_SKINS.map((s) => (
        ...
        <div className="space-y-1.5">
          <div className="text-[9px] uppercase tracking-widest text-slate-600">{t("cfgext.refSheetAspectLabel")}</div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          {status === "preparing" ? t("entity.sheetPreparing") : cost.missing.length === 1 ? t("entity.prepareAnglesOne") : t("entity.prepareAnglesMany", { n: cost.missing.length })}
        </button>
        <button
          type="button"
        ...
        >
          {status === "composing" ? t("entity.sheetComposingShort") : t("entity.composeSheet")}
        </button>
        {!busy && cost.missing.length > 0 && !cost.overflow && (
          <span className="text-[11px] text-slate-500">{cost.missing.length === 1 ? t("entity.prepareFirstOne") : t("entity.prepareFirstMany", { n: cost.missing.length })}</span>
        )}
