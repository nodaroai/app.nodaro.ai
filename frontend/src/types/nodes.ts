...
/**
 * Reference Sheet node — polymorphic compose-only node. Takes ONE entity-ref
 * input (character | object | location) and composes a reference sheet from
 * the panels the entity ALREADY has. Two outputs: `sheet` (the composited
 * image) + `panels` (the clean panel set, consumed downstream as a multi-image
 * consistency reference via `referenceImageUrls`).
 */
export type ReferenceSheetData = {
  ...
  /** Resolved at run time from the connected entity (informational — shown on the
   *  node card. The config panel does NOT filter the sheet-type menu by entity:
   *  all four sheet types are valid for every entity kind). */
  connectedEntityKind?: EntityKind
  generatedResults?: GeneratedResult[]
  activeResultIndex?: number
  /** Clean panel URLs emitted on the `panels` output handle. */
  panelUrls?: string[]
  /** Mirror of the URL on the active result, set by the executor (parallels
   *  every other media node's generated* field so the shared poll helper can
   *  drive it via outputKey). */
...
export type CharacterNodeData = {
  ...
  // Reference sheets (composited turnaround / variation / detail / full boards)
  // attached to this entity. Populated from the `sheets` JSONB column via the
  // GET routes; consumed by the Sheet tab.
  sheets?: ReferenceSheet[]
  // Reference-sheet panel buckets (migration 200): macro close-up panels and
  // wardrobe panels generated by the Sheet tab and auto-attached by the worker.
  // Hydrated from the GET route so the Sheet planner can reuse them instead of
...
export type ObjectNodeData = {
  ...
  // Reference sheets (composited turnaround / variation / detail boards)
  // attached to this entity. Populated from the `sheets` JSONB column via the
  // GET routes; consumed by the Sheet tab.
  sheets?: ReferenceSheet[]
  // Reference-sheet macro close-up panel bucket (migration 200): generated by the
  // Sheet tab and auto-attached by the worker. Hydrated from the GET route so the
  ...
  // falling back to the active result then `sourceImageUrl`. NOT a property of
  // the `objects` DB row; two object nodes referencing the same DB object can
  // each carry their own. Never sent to `saveObject`.
  readonly defaultAssetUrl?: string
  readonly defaultAssetName?: string
...
export type LocationNodeData = {
  ...
  // sheet or a Studio-selected variant — it drives the canvas thumbnail,
  // falling back to the active result then `sourceImageUrl`. NOT a property of
  // the `locations` DB row; two location nodes referencing the same DB location
  // can each carry their own. Never sent to `saveLocation`.
  readonly defaultAssetUrl?: string
  readonly defaultAssetName?: string
...
export const NODE_DEFINITIONS: ReadonlyArray<NodeTypeDefinition> = [
  ...
  // Speech-to-Video (Wan 2.2 S2V)
  {
    ...
    category: "ai",
    creditCost: 4,
    inputs: ["image", "audio", "prompt", "cinematography"],
    outputs: ["video"],
    defaultData: {
      label: "Speech to Video",
  ...
  },
  {
    ...
    category: "ai",
    creditCost: 40,
    inputs: ["video", "cinematography", "prompt"],
    outputs: ["video"],
    defaultData: {
      label: "Extend Video",
      provider: "veo-extend",
