...
import { KIND_UI, MentionThumb, PreviewableThumb, SECTION_LABEL_KEY, SECTION_TABS, VariantThumb, safeThumbUrl, scrolledNearBottom, sectionOf } from "./copilot-mention-picker"
...
export function CopilotMentionModal({ mentions, initialTab, search, onSearchChange, fileTotal = null, hasMoreFiles = false, onLoadMoreFiles, onPick, onClose }: CopilotMentionModalProps) {
  ...
  const counts = useMemo(() => {
    const map = new Map<string, number>(SECTION_TABS.map((section) => [section, 0]))
  ...
  return createPortal(
    <div className="fixed inset-0 z-[80] flex items-center justify-center" role="dialog" aria-modal aria-label={t(K.pickerModalTitle)}>
      ...
      <div className="relative w-[680px] max-w-[94vw] max-h-[78vh] flex flex-col bg-[var(--copilot-card)] border border-[var(--copilot-strong)] rounded-xl shadow-[0_16px_40px_rgba(0,0,0,0.5)]">
        <div className="px-4 pt-3.5 pb-3 border-b border-border flex flex-col gap-2.5">
          <div className="flex items-center gap-2">
            <span className="text-[13.5px] font-semibold text-foreground">{t(K.pickerModalTitle)}</span>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="text-[11.5px] text-[var(--copilot-muted)]">{t(K.pickerModalBlurb)}</div>
          <div className="flex items-center gap-2 px-2.5 py-2 bg-[var(--copilot-surface)] border border-border rounded-lg focus-within:border-[var(--copilot-strong)]">
            ...
            <input
              ...
              placeholder={t(K.pickerModalSearch)}
              aria-label={t(K.pickerModalSearch)}
              className="flex-1 bg-transparent border-none outline-none text-[12.5px] text-foreground placeholder:text-[var(--copilot-dim)]"
            />
          </div>
          {!drill && (
            <div className="flex flex-wrap gap-1" role="tablist" aria-label={t(K.pickerModalTitle)}>
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[var(--copilot-dim)]">
                      {section === "Files" && fileTotal !== null ? fileTotal : counts.get(section) ?? 0}
                    </span>
                  </button>
                )
              })}
          ...
          {drill && (
            ...
            >
              <ChevronLeft className={cn("w-3.5 h-3.5", isRtl && "rotate-180")} strokeWidth={2.2} aria-hidden />
              <span className="font-medium text-foreground">{drill.name}</span>
              <span className="text-[var(--copilot-dim)]">· {t(K.pickerBack)}</span>
            </button>
          )}
        </div>
          ...
          {drill ? (
            <div className="grid grid-cols-3 gap-2">
              <ModalTile
                ...
                label={t(K.pickerVariantDefault)}
                sub={t(KIND_UI[drill.kind].chipKey)}
            ...
            <div className="px-2 py-6 text-center text-[12px] text-[var(--copilot-muted)]">{t(K.pickerNoMatch, { query: search })}</div>
          ) : (
            <div className="grid grid-cols-3 gap-2">
              {items.map((item) => (
                <ModalTile
                  ...
                  onClick={() => onPick(item)}
                  label={item.name}
                  sub={t(KIND_UI[item.kind].chipKey)}
                  cornerAction={
                    item.variants?.length
                      ? { label: t(K.pickerVariantsOf, { name: item.name }), onClick: () => setDrill(item) }
                      : undefined
                  }
...
function ModalTile({
  ...
  return (
    <button
      ...
      className="group relative flex items-center gap-2.5 px-2.5 py-2.5 rounded-[10px] border border-border bg-[var(--copilot-surface)]/40 hover:bg-[var(--copilot-surface)] text-start transition-colors"
    >
      ...
      {cornerAction && (
        <span
          ...
          className="absolute top-1.5 end-1.5 w-[20px] h-[20px] rounded-md border border-border bg-[var(--copilot-card)] text-[var(--copilot-dim)] hover:text-foreground flex items-center justify-center"
        >
