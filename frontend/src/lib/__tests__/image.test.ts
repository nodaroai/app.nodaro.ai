...
describe("optimizedImageUrl", () => {
  it("transforms a cdn.scenenode.ai URL with defaults", () => {
    const url = "https://cdn.scenenode.ai/uploads/abc.png"
    const result = optimizedImageUrl(url)
    expect(result).toBe(
      "https://cdn.scenenode.ai/cdn-cgi/image/width=480,format=auto,quality=80/uploads/abc.png"
    )
  })
  ...
  it("does not double-wrap already transformed URLs", () => {
    const url = "https://cdn.scenenode.ai/cdn-cgi/image/width=200,format=auto,quality=80/uploads/abc.png"
    expect(optimizedImageUrl(url)).toBe(url)
  })

  it("preserves the full pathname", () => {
    const url = "https://cdn.scenenode.ai/users/123/images/photo.jpg"
    const result = optimizedImageUrl(url)
    expect(result).toContain("/users/123/images/photo.jpg")
  })

  it("uses default width=480 when not specified", () => {
    const url = "https://cdn.scenenode.ai/img.png"
