...
*Last updated: 2026-05-18 (Flux 2 Pro + Flux 2 Max (Replicate, Safety Tolerance) + multi-upload fix. **Flux 2 Pro** (`black-forest-labs/flux-2-pro`) and **Flux 2 Max** (`black-forest-labs/flux-2-max`) are BFL's flagship Flux 2 models exposed via Replicate with `safety_tolerance` pinned to 5 (the family max — KIE's safety filter never sees the request). Pro accepts up to 4 `image_prompt_1..4` refs; Max scales to 8 (`image_prompt_1..8`). Both available in Generate Image + Image-to-Image + Modify Image (auto-inherited via `MODIFY_IMAGE_PROVIDERS = [...IMAGE_I2I_PROVIDERS, "nano-banana-edit"]`). **Flux 2 Pro pricing**: flat 4 credits. **Flux 2 Max variable pricing**: $0.04 base + $0.03 per ref image — encoded as composite identifiers (`flux-2-max` / `flux-2-max:1ref` / … / `flux-2-max:8ref`, 3-18 user-facing credits). `buildCreditModelIdentifier` extended with an optional `referenceImageCount` parameter; both `/v1/generate-image` and `/v1/image-to-image` routes pass the ref count (i2i counts the primary `imageUrl` as one of the 8). Frontend helper mirrors the extension so the panel's "(X CR)" preview matches what the route charges. **Multi-upload bug fix (bundled in same PR):** node-input-resolver's `upload-image` + `generate-image` source branches were missing `modify-image` / `image-to-image` from the target list that piles into `referenceImageUrls` — so multiple Upload Image edges into a `kontext-multi` / `flux-2-pro` / `flux-2-max` node overwrote `inputs.imageUrl` (last-write-wins) and the worker only ever received one URL. Fix aligns both branches with the other source-type branches so `execute-node`'s `orderedImageUrls[0]` promotes index 0 to imageUrl and the rest stay as refs. Migrations 128 + 129 seed all 10 model_pricing rows (renumbered from 124/125 → 126/127 → 128/129 due to concurrent merges with Seedance 2 1080p, pipeline_entity_variants, and Character LoRA). Files: `backend/src/providers/replicate/image.ts` + tests, `backend/src/routes/{generate-image,image-to-image}.ts`, `backend/src/ee/billing/credits.ts`, `packages/shared/src/{credit-identifiers,model-constants,model-catalog,prompt-wizard-categories}.ts` + credit-identifiers test, `frontend/src/components/editor/config-panels/{model-options,helpers}.ts`, `frontend/src/components/editor/workflow-editor/node-input-resolver.ts` (multi-upload fix), `frontend/src/types/nodes.ts`, `frontend/src/ee/app/(admin)/admin/pricing/pricing-data.ts`, `supabase/migrations/{128_add_flux_2_pro,129_add_flux_2_max}.sql`, `docs/nodes/ai-image/{generate-image,image-to-image}.md`. model-sync test whitelist allows flux-2-pro and flux-2-max overlap between generate/i2i arrays.)*
*Version: 2.20.0*

---

*Previous: 2026-05-18 (Character LoRA training (Phase 1, Cloud only). Migration 126 adds 7 `lora_*` columns to characters + partial index on in-flight statuses. Migration 127 seeds `model_pricing` for `flux-lora-character` (3cr/image inference) + `character-lora-training` (150cr/training). New helpers in `backend/src/lib/character-lora.ts` — image aggregator (7 buckets), buffer-zip-to-R2 via `uploadBufferToR2`, `TOK_<slug>_<6hex>` trigger word, refund helper. LoRA routing decision lives in `packages/shared/src/lora-routing.ts` (single source of truth between orchestrator + single-node Run). New Replicate training wrapper at `backend/src/providers/replicate/training.ts` — `createCharacterTraining` (webhook_events_filter=["completed"]), `cancelCharacterTraining`, `deleteCharacterLora` (raw REST DELETE — SDK has no models.delete; uses `Authorization: Bearer`). New synthetic `flux-lora-character` model in `backend/src/providers/replicate/image.ts` with dynamic version resolution from `extraParams.lora_version`. New routes: `POST /v1/characters/:id/train` (atomic CAS via Supabase JS `.or()`, pre-flight on req.userId+PUBLIC_URL, try/catch envelope with rollback+R2 cleanup, rate-limit 3/min), `GET /v1/characters/:id/training`, `DELETE /v1/characters/:id/lora`. New webhook at `POST /v1/webhooks/replicate-training` with addContentTypeParser for rawBody + `validateWebhook` SDK helper in try/catch + monotonic state guards. Worker (`handlers/image-ai.ts handleGenerateImage`) destructures `model` + `extraParams` from the BullMQ payload — without this, both LoRA paths drop the lora_version and break at runtime. Orchestrator: `expandWiredCharacterRefs` propagates LoRA fields to `ConnectedReference`; `selectLoraRoutingForMentions` returns non-null only when EXACTLY ONE trained character → swap provider→replicate, model→flux-lora-character, refs→[], `skipCharacterMentions=true`, modelIdentifier→flux-lora-character. Single-node Run gets `_internalLora` body hint via extended `generate-image.ts` Zod schema + inline `creditGuard` preHandler short-circuit. Soft-delete cancels+refunds+deletes model. Frontend: `CharacterNodeData` gains 3 LoRA fields (added to `EXECUTION_DATA_KEYS` so undo/isDirty don't fire on poll-driven updates) with mount-time DB backfill (handles pre-existing workflows); modal training section with 8s polling + change-detection guard, mutations write back to canvas; character node corner badge via shared `<TrainedPill>`; `expandCharacterNodeIntoRefs` mirror; `runImageGeneration` passes `internalLora` through. `zipImagesToR2Buffer` fetches images concurrently via Promise.all (was sequential, ~10s wasted). Hard-fail-coverage test gains new `directly-emitted credit identifiers` describe block. New `REPLICATE_WEBHOOK_SECRET` env var. Public docs at `docs/features/character-training.md` + API/SDK additions.)*
*Previous version: 2.19.0*

---

*Previous: 2026-05-18 (Pinterest integration for upload-image — `normalizePinterestUrl(url)` helper in `@nodaro/shared` auto-rewrites `i.pinimg.com/{236x|474x|564x|736x|1200x}/...` paths to `/originals/` so downstream `describe-image` / `image-to-image` / face-swap get source-res input. Wired into `upload-image-node.tsx` at two sites (paste + legacy migration). New "or browse on Pinterest" discovery button in `UploadPicker`. 12 unit tests.)*

---

*Previous: 2026-05-18 (Multi-result refactor pass — pure cleanup of the multi-result PR (#2464), no behavior change. New `variantJobId(base, i)` helper; new `buildImageOutputData` + `finalizeSunoJob` worker helpers; new `buildVariantResults` shared by single-node + orchestrator paths. Cuts speculative `videoUrls` from `NodeOutput`. Net: −85 lines across 8 files + 2 new helper files. Zero regressions.)*
*Previous version: 2.18.3*

---

*Previous: 2026-05-18 (Multi-result provider support — fix for "we drop every variant after the first" bug. KIE Grok image returns up to 6 variants per task and Suno music returns 2 tracks per generation. New `ProviderResult.extraUrls?: readonly string[]` + `uploadImageVariantsMaybeWatermark` + `uploadAllSunoTracks` helpers. Backend↔frontend parity: both `NodeOutput` and `output-extractor` `DIRECT_OUTPUT_KEYS` carry `imageUrls`/`audioUrls`. Cost: KIE charges per-task not per-output, so users get N variants for 1×. Regression net: 4 new unit tests.)*
