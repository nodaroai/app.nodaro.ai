...
*Last updated: 2026-05-17 (MCP character-asset gap fix — extended `generate_character` verb with `bodyAngles` + `attach_to_*` params (was previously missing); consolidated duplicate `generate_character_asset` tool. Closes the gap reported as "i cannot generate expressions and head/body angles shots via mcp." The existing `verbs-clo.ts::generate_character` (kind="asset") tool's enum was missing `headAngles` and `bodyAngles`, and didn't expose `attach_to_character_id` / `attach_to_column` / `attach_name` — so the studio attach path was unreachable. Fixed in place rather than adding a parallel `generate_character_asset` tool: the duplicate would have created a confusing two-tool surface for the same route and the file `characters.ts` already documented that asset generation lives in verbs-clo. New shared constants `CHARACTER_ASSET_TYPES` and `CHARACTER_ATTACH_COLUMNS` in `@nodaro/shared/entity-prompts` are the single source of truth — consumed by `backend/src/routes/generate-character-asset.ts` (Zod schema) and `backend/src/lib/mcp/tools/verbs-clo.ts` (MCP input enum). Final `asset_type` enum: `expressions, poses, lighting, angles, headAngles, bodyAngles, custom`. `motions` is NOT in the enum — `generate_character_motion` is the dedicated tool (separate route, different input shape). Also tightened `generate_character_motion`'s `provider` field to `z.enum(CHARACTER_MOTION_PROVIDERS)` so the LLM-facing surface is constrained to the curated i2v providers. NOT a breaking change: callers using `lighting` still work; `headAngles`/`bodyAngles` + attach params are net-new. 4 new tests added to `verbs.test.ts`; 6 tests removed from `characters.test.ts`. Files: `backend/src/lib/mcp/tools/verbs-clo.ts` (extended), `backend/src/lib/mcp/tools/characters.ts` (removed `generate_character_asset`, tightened motion provider enum), `backend/src/routes/generate-character-asset.ts` (uses shared constants), `packages/shared/src/entity-prompts.ts` (new `CHARACTER_ASSET_TYPES` + `CHARACTER_ATTACH_COLUMNS`), `packages/shared/src/index.ts` (re-exports), `docs/mcp/tools.md` (consolidated section), `docs/character-platform.md` (updated examples).)*
*Version: 2.15.2*
