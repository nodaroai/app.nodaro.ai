...
*Last updated: 2026-05-17 (two editor fixes. (1) Node handle positioning — the outer `<div className="relative">` wrapper around `<BaseNode>` in 16 node components (character, generate-image, image-to-video, text-to-video, video-to-video, image-to-image, modify-image, edit-image, face-swap, generate-mask, lip-sync, motion-transfer, remove-background, speech-to-video, upscale-image, component) was either auto-sized to its inner content or capped at `maxWidth: '220px'`. BaseNode's `imageAspectRatio` effect bumps the React Flow node width to 320px on first load; the wrapper stayed narrower, leaving the absolutely-positioned `<HandleIcon>` siblings (left:-29px / right:-29px) floating in space outside the node body edge. Replaced with `style={{ width: "100%", height: "100%" }}` so the wrapper fills the React Flow node's outer dimensions and the icon-pill chips correctly anchor to the body edge. NodeResizeControl still drives sizing — the wrapper just tracks it. (2) ITV reference reorder — the "Reference Images" sections in `ImageToVideoConfig` (Grok / VEO ref mode) and `TextToVideoConfig` (Seedance-2 ref-only mode) rendered connected character/image upstreams as static thumbnail grids with no reorder UI, so users couldn't control which character became Image 1 vs Image 2 in the assembled prompt. Both replaced with `<ConnectedMediaList>`; new field `connectedRefImageOrder?: readonly string[]` on `ImageToVideoData` + `TextToVideoData`. New `applyOrderToReferenceUrls()` helper in `backend/src/services/workflow-engine/payload-builder.ts` re-derives `referenceImageUrls` from the workflow graph in user-defined order BEFORE `resolveVideoPromptMentions` runs (filtered by `targetHandle === "references"` for i2v, by source type for t2v). Mirror logic added to frontend `execute-node.ts` for single-node "Run" parity. Without backend support the reorder would only affect the UI — orchestrator-driven runs (published apps, webhooks, cron) would still use raw edge order.)*
*Version: 2.14.5*
