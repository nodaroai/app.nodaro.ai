...
*Last updated: 2026-05-18 (fix: `flux-2-max` reservation identifier parity. PR #2469 wired the `referenceImageCount` arg into both `/v1/generate-image` and `/v1/image-to-image` preHandlers — the credit CHECK correctly resolved to `flux-2-max:Nref` — but the handler's reservation `modelIdentifier` (built by a second call to `buildCreditModelIdentifier` further down the route) dropped the count and resolved to the bare `flux-2-max`. The credit guard then verified the user could afford 5–18 credits (correct, composite row) but `reserveCredits` debited only the bare-row 3 credits. Users were undercharged 2–15 credits per generation depending on ref count (8 refs = 15-credit underbill). Two-line fix passes `referenceImageUrls?.length ?? 0` (t2i path) and `1 + referenceImageUrls?.length` (i2i path, primary `imageUrl` is one of the up-to-8 refs) to the handler's identifier build so it mirrors the preHandler. Added a regression net in both route test files — `flux-2-max:Nref` parameterized for N=0..8 — pinning that `reserveCreditsForJob` is invoked with the same composite identifier the preHandler checked. 49 route tests + 698 billing tests + 143 shared credit-identifier tests all green.)*
*Version: 2.22.2*

---

*Previous: 2026-05-18 (Suno Voice Persona node + KIE `/voice/*` proxy. New setup-time node that runs KIE's 2-stage `/voice/validate` + `/voice/generate` flow via a 3-step modal: source-audio + segment → returned validation phrase → user records phrase → custom `voiceId`. Stored on node `data` and emitted at workflow runtime — no API call at run-time. The 3 Suno music nodes (`suno-generate` / `suno-cover` / `suno-extend`) gained `personaId` + `personaModel` body fields plumbed through Zod → worker → KIE so any music generated downstream sings in the custom voice. 5 new backend routes under `/v1/suno/voice/*`. `POST /voice/generate` reserves 20 credits via `creditGuard("suno-voice-create")` against a `jobs` row; `GET /voice/record-info` doubles as commit/refund site (idempotent CAS on `status='reserved'`). Migration 131 seeds `model_pricing.suno-voice-create=20`. New node registered in 18 places — `SOURCE_NODE_TYPES` in `execution-graph.ts` so the orchestrator skips it at runtime; `output-extractor.ts` and both resolvers route `voiceId` → `personaId` only when downstream is one of the 3 music nodes. **3 audit fixes in this PR:** (1) `getPrimaryOutput` now has a `case "suno-voice"` returning `output.voiceId` — without this the orchestrator's `if (!output) continue` skipped the entire suno-voice → personaId routing, so the feature silently no-op'd in workflow runs; (2) IDOR scoping — every `/voice/*` route that accepts a `taskId` from the user now verifies ownership via `userOwnsVoiceTask` (`jobs` rows tagged `suno-voice-validate` / `suno-voice-create`), 404 on mismatch; (3) `sweepStaleVoiceJobs` cron at `45 * * * *` refunds `suno-voice-create` rows stuck >2h to close the abandoned-modal credit-leak hole. New shared `backend/src/lib/credits-job-lifecycle.ts` (commit + refund helpers — de-duplicates the same pattern from `character-lora.ts`). KIE does not publish pricing for `/voice/*` — 20cr is a conservative one-time default.)*
*Version: 2.22.1*

---

*Previous: 2026-05-18 (Story-to-Video Phase 1B.3: §6.11 Scene-Context helper buttons. 7 active helpers ship on the SceneNode config panel — 🔍 Audit Prompt (Haiku, 1cr), ✨ Improve Prompt (Sonnet, 2cr), 🎬 Generate Motion (Haiku, 1cr), 🎯 Optimize for Model (Sonnet, 3cr), 🎞️ Add B-Roll (Sonnet, 2cr), 🌉 Bridge to Next Scene (Sonnet, 2cr — v4.1 Method 5 bridge_image_prompt), 🎨 Anchor Scene Style (Sonnet + image gen, 5cr — v4.1 Method 6 master keyframe). Helpers are user-triggered only, return structured suggestions to a Accept/Reject modal that patches SceneNodeData. 3 vision-keyframe helpers (Audit Images / Fix Continuity / Validate Match Cut) render as disabled buttons with "Pending Phase 1C" tooltip — they need Stage 6 keyframes. New `backend/src/routes/scene-helpers.ts` with 7 POST endpoints under /v1/pipelines/:id/entities/:sceneId/helpers/:name (edition+scope+ownership gated; credits reserved + refunded-on-error). New `backend/src/ee/pipelines/scene-helper-credits.ts` calls `reserve_credits` RPC directly with p_job_id: null + helper-specific `scene-helper:<name>` identifier (does NOT touch `pipelines.reservation_usage_log_id` — that's pipeline-level). Migration 130 seeds 7 `model_pricing` rows; STATIC_CREDIT_COSTS fallback added. New `packages/shared/src/scene-helper-types.ts` with Zod schemas + SceneHelperName enum + HELPERS_SHIPPED_IN_1B3 set. Frontend: new `pipelinesApi.runSceneHelper<N>` generic typed by helper name; new `<SceneHelperButtons>` row component (10 buttons, 7 active + 3 disabled); new `useSceneHelper` state hook (idle/loading/ready/error); new `<SceneHelperModal>` with per-helper accept-patch logic (audit=read-only, improve/motion/optimize=merge shots by shot_id, broll=splice candidates at before_first/after, bridge=set bridge_image_prompt on target shot, anchor=set scene_anchor_keyframe). `pipeline_id` now plumbed into SceneNode data shape via canvas-materializer so the helper modal resolves the pipeline without a separate store. Spec §6.11.8 audit-via-pipeline_stage_attempts deferred to Phase 1B.4 (requires migration extending trigger CHECK); 1B.3 audits via existing llm_calls.scene_id from 1B.2.)*
*Previous version: 2.22.0*

---

*Previous: 2026-05-18 (Story-to-Video Phase 1B.2: Stage 5 + SceneNode. New Scene Director LLM (Sonnet, per-scene parallel via settledWithLimit at concurrency=5) emits SceneNodeData per scene; Shot List Critic (Sonnet, always-on) validates each. New capability registry VIDEO_MODEL_CAPS + modelsForInputMode in @nodaro/shared/model-constants.ts (single source of truth for video models, consumed by Scene Director / route validators / SceneNode config panel). New provider-directive-defaults.ts maps shot_intent → provider directives. New SceneNode 18-step registered (replaces legacy scene node) with three view modes (default / storyboard / scripting) + video placeholder (Phase 1C). Stage 5 orchestrator parallel-fans-out Scene Director with bounded concurrency, retries per scene on critic fail (≤2). Pipeline advances script → characters → objects → locations → shot_list → completed in Phase 1B.2 (Stages 6-8 land in Phase 1C). SceneGrid + SceneCard panel components. Spec v4.1 Methods 2 (paired keyframes) + 6 (scene anchor) field plumbing in place; execution lands in Phase 1C.)*
*Previous version: 2.21.0*

---

*Previous: 2026-05-18 (Flux 2 Pro + Flux 2 Max (Replicate, Safety Tolerance) + multi-upload fix. **Flux 2 Pro** (`black-forest-labs/flux-2-pro`) and **Flux 2 Max** (`black-forest-labs/flux-2-max`) are BFL's flagship Flux 2 models exposed via Replicate with `safety_tolerance` pinned to 5 (the family max — KIE's safety filter never sees the request). Pro accepts up to 4 `image_prompt_1..4` refs; Max scales to 8 (`image_prompt_1..8`). Both available in Generate Image + Image-to-Image + Modify Image (auto-inherited via `MODIFY_IMAGE_PROVIDERS = [...IMAGE_I2I_PROVIDERS, "nano-banana-edit"]`). **Flux 2 Pro pricing**: flat 4 credits. **Flux 2 Max variable pricing**: $0.04 base + $0.03 per ref image — encoded as composite identifiers (`flux-2-max` / `flux-2-max:1ref` / … / `flux-2-max:8ref`, 3-18 user-facing credits). Migrations 128 + 129 seed all 10 model_pricing rows.)*
*Previous version: 2.20.0*

---

*Previous: 2026-05-18 (Character LoRA training (Phase 1, Cloud only). Migration 126 adds 7 `lora_*` columns to characters + partial index on in-flight statuses. Migration 127 seeds `model_pricing` for `flux-lora-character` (3cr/image inference) + `character-lora-training` (150cr/training). New helpers in `backend/src/lib/character-lora.ts` — image aggregator (7 buckets), buffer-zip-to-R2, `TOK_<slug>_<6hex>` trigger word, refund helper. LoRA routing decision lives in `packages/shared/src/lora-routing.ts` (single source of truth between orchestrator + single-node Run). New Replicate training wrapper at `backend/src/providers/replicate/training.ts`. New synthetic `flux-lora-character` model with dynamic version resolution from `extraParams.lora_version`. New routes: `POST /v1/characters/:id/train`, `GET /v1/characters/:id/training`, `DELETE /v1/characters/:id/lora`. New webhook at `POST /v1/webhooks/replicate-training`. Frontend: `CharacterNodeData` gains 3 LoRA fields with mount-time DB backfill; modal training section with 8s polling; character node corner badge via shared `<TrainedPill>`. New `REPLICATE_WEBHOOK_SECRET` env var.)*
*Previous version: 2.19.0*

---

*Previous: 2026-05-18 (Pinterest integration for upload-image — `normalizePinterestUrl(url)` helper in `@nodaro/shared` auto-rewrites `i.pinimg.com/{236x|474x|564x|736x|1200x}/...` paths to `/originals/`. New "or browse on Pinterest" discovery button in `UploadPicker`. 12 unit tests.)*

---

*Previous: 2026-05-18 (Multi-result refactor pass — pure cleanup of the multi-result PR (#2464), no behavior change. Net: −85 lines across 8 files + 2 new helper files. Zero regressions.)*
*Previous version: 2.18.3*

---

*Previous: 2026-05-18 (Multi-result provider support — fix for "we drop every variant after the first" bug. KIE Grok image returns up to 6 variants per task and Suno music returns 2 tracks per generation.)*
*Previous version: 2.18.2*

---

*Previous: 2026-05-18 (Upload-image node UI modernization. Unified the URL-paste and file-upload paths through the shared `generatedResults` array, so both sources now drive aspect-ratio detection, flush layout, hover affordances, and version switching. Previously the URL-paste path went into a separate `mode === "url"` branch that wrapped the image in a fixed `aspect-video` 16:9 frame and skipped the result-stack entirely — pasting a portrait Pinterest URL rendered letterboxed/cropped with no hover buttons. New `UploadPicker` sub-component is shared between the empty state and the new "+" hover overlay, so users can add another image (file or URL) without losing the current one. Hover button layout matches generate-image exactly: top-right `+` / `X`, bottom-left Edit / Fullscreen / Download / Copy URL / Save-to-Library, versions pill top-left. Legacy migration `useEffect` hoists pre-existing `externalUrl`/`r2Url`/`url` into `results[0]` on mount (idempotent guard prevents loops), and re-fires when `useUpstreamUrl` connects an upstream post-mount so aspect-ratio detection kicks in even for upstream-driven URLs. Drops the broken `legacyRatio` local-state fallback and the `max-w-[220px]` clamp so the node fills space like generate-image. Swaps `ImageLightbox` → `MediaPreviewModal` and adds `DeleteConfirmationDialog` for full parity with generate-image's fullscreen / delete UX. File: `frontend/src/components/nodes/upload-image-node.tsx`.)*
*Previous version: 2.18.1*

---

*Previous: 2026-05-18 (G3+G4 skill-loader MCP tools — Phase A of the skill architecture build. Two new ungated MCP tools: `start_workflow_editor` (returns `backend/skills/workflow-editor.md` — canonical workflow JSON / edge / handle / `update_workflow_json` / result-field-contract teaching) and `get_node_skill(node_type)` (returns `backend/skills/nodes/<type>.md`). 8 hand-written per-node skill files for the cinematic-flow whitelist (text-prompt, loop, generate-image, image-to-video, generate-music, trim-video, combine-videos, merge-video-audio). `.dockerignore` whitelist `!backend/skills/**/*.md` + Dockerfile runner-stage `COPY --from=backend-build /app/backend/skills` so skills survive into production. Directory-traversal protected at the Zod boundary (kebab-case regex `/^[a-z][a-z0-9-]*$/`) + path-prefix check inside `loadNodeSkillContent`. `WORKFLOW_EDITOR_FALLBACK_CONTENT` embedded constant for defense-in-depth. 12 unit tests + 3 integration tests; `start_film_director` no-regression test explicit. Phase B (`feat/skill-autogen-g5`) wires the gen-skills auto-pipeline so the AUTO-GEN blocks in each skill file get refreshed from `frontend/src/types/nodes.ts` NODE_DEFINITIONS + MCP schemas at gen-time; Phase C refactors Film Director SKILL.md to v2.0 thin orchestrator. Spec: `specs/features/2026-05-18-skill-architecture-design.md`, plan: `specs/features/2026-05-18-skill-architecture-implementation-plan.md`.)*
