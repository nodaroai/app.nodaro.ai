...
*Last updated: 2026-05-18 (Pinterest integration for upload-image. New `normalizePinterestUrl(url)` helper in `@nodaro/shared` (`packages/shared/src/pinterest-url.ts`) auto-rewrites `i.pinimg.com/{236x|474x|564x|736x|1200x}/...` paths to `/originals/` so downstream `describe-image` / `image-to-image` / face-swap get the source-resolution input — Pinterest's CDN serves the SAME image at multiple sizes via a path-prefix segment, and the low-res variants are far worse for AI input. Helper is a no-op when the URL doesn't parse, isn't `i.pinimg.com`, or already has a non-size path prefix (e.g. `/avatars/`). Wired into `upload-image-node.tsx` at two sites: `handleAddUrl` normalizes any pasted URL before pushing to `generatedResults`, and the legacy migration `useEffect` normalizes any pre-existing `externalUrl`/`r2Url`/`url` when hoisting to `results[0]` (so old workflows with low-res Pinterest URLs upgrade silently on next render). New "or browse on Pinterest" discovery button in `UploadPicker` (shared by the empty state AND the "+" add overlay) opens `pinterest.com/search/pins/` in a new tab with `noopener,noreferrer` — user finds an image, right-clicks → Copy Image Address, pastes back into the URL field. Pinterest icon styled with Pinterest red (#E60023) on hover for affordance. 12 unit tests for `normalizePinterestUrl` cover every documented size segment, query-string/fragment preservation, the already-`/originals/` no-op, and the non-Pinterest pass-through cases. Files: `packages/shared/src/{pinterest-url.ts,__tests__/pinterest-url.test.ts,index.ts}` (new helper + tests), `frontend/src/components/nodes/{upload-image-node.tsx,__tests__/upload-nodes.test.tsx}` (wire-in + Pinterest button + mock).)*
*Version: 2.18.4*

---

*Previous: 2026-05-18 (Multi-result refactor pass — pure cleanup of the multi-result PR (#2464), no behavior change. Extracts shared helpers so the `${jobId}-v${i}` suffix scheme, the image output_data assembly, and the variant-fan-out logic live in ONE place each. New `variantJobId(base, i)` in `@nodaro/shared` is the single source of truth for the variant-suffix R2-key + result-ID convention (was inlined in 4 spots). New `buildImageOutputData(result, r2Urls)` in `workers/shared.ts` next to `buildProviderMeta` — collapses the `output_data: { imageUrl, ...(imageUrls?...), ...buildProviderMeta(result) }` block that 3 image handlers each carried. New `finalizeSunoJob(job, ctx, result, label)` in `workers/handlers/suno.ts` collapses the identical 7-line tail (progress 50→100 / upload / shouldSaveJobResult / markJobCompleted / commitJobCredits / log) that 8 Suno handlers each carried — net 56 lines → 8 calls. New `buildVariantResults(urls, baseJobId, opts)` in `frontend/.../workflow-editor/variant-results.ts` shared by `poll-job.ts` (single-node path) AND `run-handlers.ts` (orchestrator path), so the two paths can no longer drift in how they fan a variant array out into `GeneratedResult` entries — accepts optional `existingUrls` Set so the orchestrator-poll dedup path passes its already-loaded URLs through. Simplifies `uploadAllSunoTracks` return shape: `Record<string, unknown> | null` instead of `{outputData, primaryR2Url, trackCount} | null` (the latter two were redundant — already on `outputData`). Cuts speculative `videoUrls` from `NodeOutput` / `DIRECT_OUTPUT_KEYS` / frontend `NodeExecutionState` / `OUTPUT_URLS_KEY` — no provider emits video variants today, easy to re-add when a multi-output video model surfaces. Flattens the nested-conditional `if listResult else if variantResult else single` in `run-handlers.ts` into a flat cascade. Trims 5-line "Multi-variant fan-out: …" WHAT-narrating comments in `poll-job.ts` and `run-handlers.ts`. Net: −85 lines across 8 files + 2 new helper files. Zero regressions (48/48 multi-result tests, 6015/0 backend, 3002/0 frontend). Files: `packages/shared/src/{variant-job-id.ts,index.ts}` (new), `frontend/src/components/editor/workflow-editor/variant-results.ts` (new), `backend/src/services/workflow-engine/{types.ts,output-extractor.ts}`, `backend/src/workers/{shared.ts,handlers/image-ai.ts,handlers/suno.ts}`, `frontend/src/components/editor/workflow-editor/{poll-job.ts,run-handlers.ts}`.)*
*Previous version: 2.18.3*

---

*Previous: 2026-05-18 (Multi-result provider support — fix for "we drop every variant after the first" bug. KIE Grok image returns up to 6 variants per task in `resultJson.resultUrls` and Suno music returns 2 tracks per generation; pre-fix the worker took `resultUrls[0]` / `tracks[0]` and silently discarded the rest. Now: `ProviderResult.extraUrls?: readonly string[]` carries variant URLs alongside the primary `url` (provider.interface.ts), `RouteResult` forwards them through `routeAndExecute`, and KIE image (kie/image.ts T2I + edit) populates `extraUrls = resultUrls.slice(1)`. The worker (`workers/handlers/image-ai.ts`) calls the new `uploadImageVariantsMaybeWatermark` helper (in `workers/shared.ts`) which uploads primary + extras to R2 in parallel under suffixed keys (`${jobId}`, `${jobId}-v1`, `${jobId}-v2`, …), and persists `output_data.imageUrls: string[]` alongside the primary `imageUrl` for backwards compat. Suno worker (`workers/handlers/suno.ts`) extracted `uploadAllSunoTracks` helper that 8 handlers (generate, cover, extend, mashup, replace-section, add-instrumental, add-vocals, upload-extend) now share — uploads every track in parallel and writes `audioUrl` (primary), `audioUrls` (array), and `sunoTracks: Array<{id,title,duration,imageUrl,audioUrl}>` to output_data. **Backend↔frontend parity**: both `NodeOutput` (orchestrator) and the orchestrator's `output-extractor` `DIRECT_OUTPUT_KEYS` now carry `imageUrls`/`audioUrls`/`videoUrls`, and both result-consumption paths fan out — single-node `poll-job.ts handleJobCompleted` and orchestrator `run-handlers.ts` poll loop both produce identical N-entry `generatedResults` for the same multi-variant job, so users see every variant in the version pill regardless of whether they ran one node or a whole workflow. Cost: KIE charges per-task not per-output, so users get N variants for the price of 1 — only the R2 upload bandwidth/storage scales. KIE video / audio TTS / SFX / Replicate are single-result by design and deferred (infrastructure-ready when a multi-output model surfaces). Backwards compat: legacy single-result jobs continue to work (no `imageUrls` field → frontend falls back to single `imageUrl` entry). Regression net: 4 new unit tests across `image-ai.test.ts` and `suno.test.ts` covering Grok 6-variant fan-out, Suno 2-track upload + suffix-keyed R2 routing, and the inverse single-result case must-not-emit-array. Files: `backend/src/providers/{provider.interface.ts,router.ts,kie/image.ts}`, `backend/src/workers/{shared.ts,handlers/image-ai.ts,handlers/suno.ts}`, `backend/src/services/workflow-engine/{types.ts,output-extractor.ts}`, `frontend/src/components/editor/workflow-editor/{poll-job.ts,run-handlers.ts}`.)*
