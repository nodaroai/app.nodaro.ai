...
*Last updated: 2026-05-17 (character usage modes: added two new minimal-intervention modes — `name` and `none` — to `USAGE_MODES` (now 8 total). `name` keeps the labeled subject (`- Image N (Name)`) but DROPS the trailing directive sentence — model is told who the character is so it can correlate the slot with a named entity, without being told how to consume the image. `none` is fully silent: attaches the image, emits NO bullet, NO name in any label, and excludes the character from the "Use these characters:" header when ALL its mentions are `none`. The inline replacement for an `@kira:1:smile:none` token becomes the bare positional reference (`Image 1`) instead of the character name, so the user's sentence still parses. `usageModeDirective` now returns `string | null` (null for `name`/`none`); new helper `usageModeIncludesName(mode)` returns false only for `none`. `resolveCharacterMentions` tracks "first non-`none` bullet" per character (not "first mention overall") so a `[none, face]` mention pair correctly emits the `face` bullet — `none` mentions never claim the first-bullet slot. URL merge in `buildImagePrompt` no longer gates extras URLs on `directiveLines.length > 0` (a `none`-mode extra attaches its URL with no bullet). Parallel mode-aware treatment applied to backend `payload-builder.ts` `resolveVideoPromptMentions` and frontend `execute-node.ts` `resolveVideoPromptMentions` (both canonical-fallback + extras branches). Pill UI (CharacterRefView), autocomplete drills (suggestion-list, tag-textarea), and the character-node `defaultUsageMode` dropdown all pick up the new modes automatically — they iterate over `USAGE_MODES` generically. Pill renderText round-trips `none`/`name` modes via `:none`/`:name` slug suffix. Files: `packages/shared/src/character-usage-mode.ts` (USAGE_MODES tuple + label/directive/`usageModeIncludesName`), `packages/shared/src/prompt-builder.ts` (`resolveCharacterMentions` + `buildCanonicalFallback` + `buildExtraRefDirectives` + the extras URL merge gate), `packages/shared/src/index.ts` (export `usageModeIncludesName`), `backend/src/services/workflow-engine/payload-builder.ts` (video resolver), `frontend/src/components/editor/workflow-editor/execute-node.ts` (video resolver), `packages/shared/src/__tests__/character-usage-mode.test.ts` + `build-image-prompt-mentions.test.ts` (new test cases), `docs/character-platform.md` (Mention usage modes table + worked example). prompt-editor character mention pills: @-mentions like `@kira:1:smile:face` now render as inline violet pills with character thumbnail in the TipTap `<PromptEditor>` (generate-image / image-to-image / modify-image). New `CharacterRefExtension` is an atomic inline node; its `renderText` serializes the pill back to the exact literal slug so `editor.getText()` is byte-identical to the user-typed form and the shared `findCharacterMentionTokens` / `parseCharacterMentionToken` parser keeps working unchanged. Autocomplete `command` now inserts a `characterRef` node (instead of plain text); typing/pasting a slug literal auto-promotes to a pill via `nodeInputRule` + `nodePasteRule` (regex lowercase-only — matches the shared parser's `[a-z][a-z0-9-]*` constraint so a typo'd `@Kira:1` is left as text rather than promoted to a pill the prompt-builder would silently drop). `valueToDoc` now scans both `{image:N}` and `@<slug>:N(:variant)(:mode)` patterns in initial-load text so previously-typed slugs in saved workflows show up as pills. Pill exposes a click-to-pick usage-mode menu (Default-from-character + the 8 modes from `USAGE_MODES`); when mode=null the slug round-trips as 2/3-part (no mode suffix) — matches the autocomplete's clean-default behavior. Broken pills (character no longer wired upstream) get a dashed rose outline + `?` placeholder thumb. Storage indexed by `(characterSlug, variantSlug)` so pills survive reference-slot reordering on edge insert/remove (image-ref pills can't — they're positional). Files: `frontend/src/components/editor/config-panels/prompt-editor/{character-ref-extension.ts,character-ref-view.tsx}` (new); `prompt-editor/index.tsx` (registers ext, switches autocomplete command, scans slugs in valueToDoc); `frontend/src/globals.css` (violet `.character-ref-pill` + selected/broken states).)*
*Version: 2.14.3*

---

*Previous: 2026-05-15 (empty-prompt allowed when assembled prompt has content: frontend `execute-node.ts` no longer rejects empty user input before `buildImagePrompt` runs — `generate-image` / `image-to-image` / `modify-image` / `text-to-video` now check the FINAL assembled prompt (post mention resolution, identity directives, style, cinematography), so an empty user prompt with a wired Character (canonical fallback) or only `@kira:1:smile` still runs. Both `resolveVideoPromptMentions` functions (frontend + backend `payload-builder.ts`) updated to treat empty/undefined prompt as `""` so canonical fallback applies for character-only inputs. Backend Zod schemas unchanged — frontend sends the assembled prompt which is non-empty when assembly produced content. Negative case still rejects: empty user prompt + no character + no style + no cinematography → still throws "no prompt". character-mentions fix: frontend single-node runs of `image-to-image` and `modify-image` now build a full `connectedReferences` array (canonical + per-variant entries for every wired upstream Character node) before calling `buildImagePrompt`, so `@kira:N:variant` tokens in the prompt resolve to the variant URL via Phase 0 — same behavior the orchestrator already had via `expandWiredCharacterRefs` in `payload-builder.ts`. Bug: before this, the i2i/modify-image branches only passed flat URLs as `referenceImageUrls` to `buildImagePrompt`, skipping Phase 0 entirely, so `@kira:1:smile` survived as literal text in the prompt and ONLY the character's canonical sourceImageUrl attached (as the main `imageUrl`). Shared helper `buildConnectedRefsForI2I` in `execute-node.ts` keys character expansion by-source-node (NOT by chainRefs index) so the variants get added even when the character's URL is consumed as the main image. Reference: `expandCharacterNodeIntoRefs` mirrors the backend `expandWiredCharacterRefs` (parity for single-node ↔ orchestrator). Generate-image already had its inline expansion since the original PR #2407 and is unchanged. Frontend i2v/t2v/v2v single-node paths now apply `resolveVideoPromptMentions` (and the empty-prompt-allowed fix flows through). prompt-editor: TipTap `<PromptEditor>` (generate-image / image-to-image / modify-image) now mirrors `TagTextarea`'s hierarchical @-mention UX — one row per character at root, drill-in for variants, back row; character refs insert as PLAIN TEXT in the `@<slug>:N(:variant)?` format with N computed by scanning existing tokens (max+1); non-character refs keep the atomic TipTap `imageRef` node so `{image:N:label}` round-trips. Drill-in clears the typed filter via a parent-supplied `clearFilter` callback that `deleteRange`s chars after `@`; Backspace in drill-in with empty filter pops back to root. Closes the regression where image-to-video (TagTextarea) inserted `@kira:1:smile` but generate-image still inserted `@image:N:label`. Files: `frontend/src/components/editor/config-panels/prompt-editor/{index,suggestion-list}.tsx`. character-studio UX: AssetCard motion playback on hover (videos autoplay/pause on mouse-enter/leave with `loop`+`preload=metadata`; the prior decorative ▶ overlay was removed); hover overlay (Copy URL + Enlarge) now also renders for video cards; `MultiImageLightbox` extended with `kind: "image" | "video"` — videos render as controlled `<video>` with autoPlay+loop+muted+playsInline, `key={url}` forces a fresh element on arrow-nav; AssetCard gains `onInjectToCanvas` (+ icon) → creates `upload-image`/`upload-video` node 320px to the right of the source character node; AssetCard gains `onSetAsDefault`/`isDefault` (★ icon) → toggles per-canvas-node `defaultAssetUrl`+`defaultAssetName` on `CharacterNodeData` (frontend-only, NOT in `saveCharacter` payload); character node canvas thumbnail now uses `defaultAssetUrl` over `sourceImageUrl` when set, rendering motion defaults as `<video autoplay loop muted>`; studio state exposes `nodeId` so tabs avoid prop-drilling; shared helpers in `inject-helpers.ts`. Per-canvas-node defaults — two character nodes referencing the same DB row can show different defaults.)*
