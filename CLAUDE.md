...
- [x] List utilities: Filter List / Deduplicate / Merge Lists / Sort List (inline 0 CR, operate on `listResults` from upstream; Filter List supports AND/OR conditions with `{{now}}` / `{{trigger.last_triggered_at}}` variables + node-wide `caseSensitive` text-match toggle defaulting off for new nodes; Sort List uses dot-path field with Auto/Text/Number/Date comparison, asc/desc direction, missing-last)
...
- [x] Setting parameter node — "where it takes place" cinematography dimension (0 CR). 55-entry catalog in `packages/shared/src/setting.ts` across 4 categories: Indoor (coffee shop, kitchen, hotel room, laboratory, courtroom, taxi, cathedral, …), Urban (city street, backyard, highway, bridge, train station, airport, …), Nature (forest, beach, waterfall, cave, riverbank, western canyon, …), Fantastical (alien planet, spaceship interior, haunted mansion, cyberpunk city, medieval village, …). Static CSS preview tiles per setting. `SettingPicker` grouped with search; FE + BE cinematography-hint dispatch mirrors the Style pattern. Location entity node gains a `cinematography` target handle so Setting (and all other cinematography params) can inform the reference-image generation. New `STILL_IMAGE_EXCLUDE_TYPES` option on `collectCinematographyHints` strips video-only dims (camera-motion, temporal) for generate-image / edit-image / image-to-image / modify-image / Location entity.
- [x] Object top-level category + 4 parameter nodes (0 CR each): **Material** — 56-entry catalog (`packages/shared/src/materials.ts`) across 7 categories (Fabric / Metal / Stone / Wood / Glass-Ceramic / Natural / Exotic) with per-material CSS gradient swatches; universal `"made of {material}"` grammar works on subjects, objects, clothing alike. **Animal / Vehicle / Weapon** — reuse existing `animals.ts` / `vehicles.ts` / `weapons.ts` catalogs (115/101/78 entries) that previously powered only the Object entity's sub-category pickers; parameter nodes emit `"featuring a {label}, {description}"` / `"featuring a {label}, …"` / `"with a {label}, …"` respectively. Also: **Fabric dim added to Styling** (15 clothing-focused fabrics with `"wearing X"` grammar, intentional vocabulary overlap with universal Material). New "OBJECT" category in add-node-popup + sidebar. `ParameterNodeShell` pattern mirrors Setting; shared dispatchers in `parameter-prompt-hint.ts` and `parameter-node-value.ts` extended with 4 new cases.
- [x] Picker i18n — multi-language picker labels/descriptions for all 28 parameter-node pickers across **11 locales**: English (canonical) + Spanish (es), French (fr), German (de), Brazilian Portuguese (pt-BR), Russian (ru), Hindi (hi), Japanese (ja), Korean (ko), Simplified Chinese (zh-CN), Hebrew (he, **RTL**), Arabic (ar, **RTL**). Architecture: canonical English stays in catalog files (`packages/shared/src/<catalog>.ts`); non-English translations live in sidecar files at `packages/shared/src/i18n/<catalog>.<locale>.ts` exporting `Record<id, { label?, description? }>` (omitted fields fall back to English). 29 catalogs × 10 non-English = **319 sidecar files** lazy-loaded via dynamic `import()` so users only download the locales they use. Resolver in `packages/shared/src/i18n/index.ts` (`ensureLocaleCatalogLoaded`, `resolveLabel`, `resolveDescription`, `entryMatchesQuery`); search predicate filters across union of current-locale + English so a French user typing "soie" finds Silk AND typing "silk" still finds it. Frontend Zustand store `frontend/src/lib/locale-store.ts` (`useUserLocale`, `useLocaleDir`) hydrates from `profiles.preferred_locale` (new TEXT column, migration 090) on app load; falls back to `navigator.language` then `"en"`. `<LocalePicker>` (globe + short-code button) lives in-place at the top of each parameter-node config panel — no global app-wide language switcher; locale changes apply to ALL pickers in the canvas. RTL languages (he, ar) get `dir="rtl"` on the picker container; Latin-script proper nouns embedded in RTL text render correctly via Unicode bidi (no control characters needed); Hebrew has no niqqud, Arabic has no tashkeel. Per-entry translation rules: proper names (Tim Walker, Annie Leibovitz), brand names (Dior, Vogue, Sony A7III, Canon K35, Helios 44), film stocks (Kodak Portra), render engines (Unreal Engine), microtrends (Y2K, dadcore, mote-kei), Latin/Italian/Japanese cinematography jargon (chiaroscuro, contrapposto, ukiyo-e, gongbi, bokeh, vignette), and technical units (f/2.8, 5600K) are KEPT in their canonical form; common nouns/adjectives are translated. PATCH `/v1/user/settings { preferredLocale }` extends the existing user-settings route with a Zod-validated locale enum. Translations were AI-authored across 1,700+ entries per locale via 11 parallel translation agents, then audited by 11 fresh agents that fixed proper-noun violations, redundant labels, typos, and stripped tashkeel from Arabic.
- [x] Multi-pick (max 2) for ethnicity / mood / aesthetic — `pickIds` + `togglePick` helpers in `packages/shared/src/multi-pick.ts`; `DimensionTileGrid.maxSelected` prop with numbered tile badges (1, 2) + FIFO replace when full; `AestheticPicker.maxSelected` mirrors the same pattern; `PersonPicker` reads `MAX_SELECTED_BY_DIMENSION = { ethnicity: 2 }` and threads `selectedIds` + `multi` through `DimensionSection` / `GroupedEntryGrid` / `EntryChip`. Catalog hint builders generate proper grammar: `buildPersonHints` ethnicity 2-pick → `"of mixed Slavic and Mediterranean heritage"` (uses `shortLabel`); `buildMoodHints` 2-pick → `"with a smirking and aloof expression"`; `buildAestheticHints` 2-pick → `"styled in a Y2K + dadcore aesthetic blend"` (single-pick falls through to existing `getAestheticPromptHint`). `MoodNode` / `AestheticNode` display nodes show `"X + Y"` headline when 2 are picked. Storage: single-pick stays `string` (back-compat), multi-pick stores `string[]`; `pickIds` normalizes both at every read site, `parameter-node-value.trim()` returns the first non-empty array entry so single-string field-mapping resolvers keep working. Pose-activity and styling-wardrobe-state sub-dims excluded from this pass — they need picker-shell refactoring.
- [x] Scandinavian split — Nordic umbrella relabeled "Nordic (any)" + 5 sub-entries: Swedish / Norwegian / Danish / Finnish / Icelandic. Same group ("European"), translated across all 11 i18n sidecars.
- [x] GPT Image 2 (KIE) — added `gpt-image-2` (T2I) and `gpt-image-2-i2i` (I2I) providers using KIE.ai's `gpt-image-2-text-to-image` / `gpt-image-2-image-to-image` model identifiers. Resolution-tiered pricing (1K/2K/4K composite identifiers `gpt-image-2:2K`, `gpt-image-2:4K`); aspect ratio set `auto/1:1/16:9/9:16/4:3/3:4` (constraints: 1:1 cannot use 4K, `auto` limited to 1K); no `quality` param (replaced by resolution). I2I uses `input_urls` array (max 16). Distinct from gpt-image-1.5 which keeps quality-based pricing. New `RESOLUTION_2K_4K_TIERED_PROVIDERS` Set in shared model-constants consolidates resolution-composite logic across nano-banana-2 + gpt-image-2 family.
- [x] PromptHero vocabulary gap-closure — research-driven catalog expansion based on 134 sampled prompts (specs/2026-04-25-prompthero-vocabulary-gaps.md). **+~686 catalog entries**, **10 new full param nodes**, **9 new sub-dims on existing nodes**. New nodes (0 CR each): **Photographer / Artist Style** (49), **Photo Genre / Intent** (36, includes selfie sub-types: mirror / gym-mirror / front-cam / BeReal / accidental-flip + brand-editorial entries Vogue / Dior / Jil Sander / Vivienne Tam / Jacquemus / Helmut Newton / Harper's Bazaar), **Aesthetic / Microtrend** (35), **Era / Period** (25), **Backdrop** (32, distinct from Setting — studio backdrop with halo-glow / gradient / brick / fabric / mirror), **Held Prop** (53, with cigarette / smartwatch / wine / instrument / etc.), **Exposure Settings** (multi-cat aperture / shutter / iso, 20 entries), **Render Engine / Quality** (16, Unreal / Octane / Blender / raytracing / 8k / masterpiece), **Composition Effects** (14, bursting-through-frame / smoke-sculpture / pixel-disintegration), **Post-Process Effects** (14, vignette / dodge-burn / halation / bloom / light-leak / chromatic-aberration). New sub-dims: **Wardrobe** extends Styling with 7 new dims (outfit / top / bottom / outerwear / legwear / footwear / wardrobe-state, +117 entries) — `outfit` is a complete-look override, `wardrobe-state` is a modifier; **Hair State** sub-dim on Styling (wet / messy / windswept / voluminous / sleek / tousled), **Lip State** + **Eye State** sub-dims on Person (chapped / glossy / parted / pursed; closed / lidded / wide-eyed / glassy), **Hand Position** + **Body Lean** + **Head Tilt** sub-dims on Pose (hands-in-pockets / hand-on-hip / contrapposto / leaning-back / chin-up / etc.), **Activity / Behavior** sub-dim on Pose (smoking / drinking / reading / typing / cooking / driving), **Lighting Ratio** (1:1 → 1:16) + **Color Temperature** (2700K → 9000K Kelvin) sub-dims on Lighting. Existing catalog expansions: lighting +19 (on-camera flash / mirror-bounce flash / softbox / fresnel / silk-diffusion / candlelight / kicker / underwater caustics), pose +35 (incl. mid-fall / mid-spin / kissing / riding / driving / wading + activity), mood +20 (smirking / aloof / vulnerable / dazed / sultry / unbothered / smoldering / sinister / wiccan-mystical / lazy-shy), person +40 (skin-texture: porcelain / sun-kissed / freckled / blemished / shower-fresh-wet; distinctive-features: birthmark / aegyo-sal / heterochromia / vitiligo / sleeve-tattoo / face-tattoo / piercings / collarbone-visible / midriff-visible / elongated-neck), atmosphere +12 (chalk-dust / falling-petals / confetti / sparks / lens-flare / heat-haze / steam / bubbles / rain-on-glass / water-droplets / pollen / falling-ash), color-look +11 (selective-color / split-toning / faded-matte / log-flat / two-strip-Technicolor / hand-tinted / Y2K-saturated / MTV90s-VHS / Polaroid-faded / lifestyle-warm-magazine), framing +19 (cowboy-shot / head-to-knees / head-to-hip / half-body / mirror-selfie / through-glass / 3x3-collage / multi-frame-mosaic / contact-sheet / magazine-spread / side-back-angle / cutaway-cross-section), camera-format +14 (Sony A7III mirrorless / Canon R5 / Hasselblad medium-format / Leica M / Voigtlander / Fuji XT4 / drone-aerial / GoPro / webcam-FaceTime / disposable / Fuji-Instax / tintype / daguerreotype / toy-camera + revised security-cam), styling.jewelry +9 (smart-watch / leather-velvet-chain choker / stud-hoop earrings / ear-cuff / septum-ring / body-chain), lens +4 vintage cinematic primes (Canon K35 / Cooke S4 / Helios 44 / Petzval), style +1 (caricature). **Identity Lock** toggle on Character node — `identityLock?: "off" | "soft" | "strict"` field with `collectIdentityLockClause` helper that propagates through `PASSTHROUGH_TYPES` (ai-writer / loop / combine-text), injected at 9 generation call sites in both `frontend/src/components/editor/workflow-editor/execute-node.ts` and `backend/src/services/workflow-engine/payload-builder.ts` (generate-image / edit-image / image-to-image / modify-image / image-to-video / text-to-video / video-to-video / speech-to-video / extend-video). **JSON Wizard output mode** — `outputFormat: "natural" | "json"` segmented toggle in Phase-3 of `prompt-helper-dialog.tsx`. `CATEGORY_TO_JSON_KEY` map (29 entries) drives `buildJsonPrompt()` serializer that emits prompthero-style structured dict (subject / scene / camera / lighting / composition / mood / attire / accessories / makeup / hair / lips / eyes / post_processing / image_quality / color_grade / focus_point / identity_lock / lens_type / render_quality / lighting_ratio / references[]). Reference-image roles collapse into `references[{slot, role}]`. Empty fields omitted. Read-only JSON preview in JSON mode.
