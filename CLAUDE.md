...
3. **`*.ee.{ts,tsx,sql,md}` filename suffix** is a deliberate exception for in-place enterprise variants of core files (e.g., `cost-tab.tsx` + `cost-tab.ee.tsx`). Use `ee/` directory by default; the suffix is reserved for tight coupling with a core sibling.
