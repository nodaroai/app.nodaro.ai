...
*Last updated: 2026-05-15 (character mention usage mode: `@kira:1:smile:face` 4-part slug — modes are `identical` (default, current behavior) / `face` / `face-pose` / `emotion` / `style`. New shared module `packages/shared/src/character-usage-mode.ts` (5-mode enum + `usageModeDirective(mode)` per-mode natural-language directive + label helper). `parseCharacterMentionToken` extended to 2/3/4-part form: 3-part disambiguates by checking 3rd segment against `USAGE_MODES` (`@kira:1:face` → canonical+mode; `@kira:1:smile` → variant+default-mode); 4-part requires variant + valid mode (`@kira:1:smile:invalid` returns null). `ConnectedReference.defaultUsageMode` propagates the character node's `defaultUsageMode` field through every entry, sourced from `CharacterNodeData.defaultUsageMode` in three places: frontend `execute-node.ts` (single-node + `expandCharacterNodeIntoRefs`), frontend `image-configs.tsx` (FinalPromptPreview connectedReferences for generate-image + modify-image), backend `payload-builder.ts` (orchestrator `expandWiredCharacterRefs`). Mode resolution at prompt-build time: `token.usageMode ?? ref.defaultUsageMode ?? "identical"`. `resolveCharacterMentions` + `buildCanonicalFallback` (in `prompt-builder.ts`) emit `- Image N (Name). <directive>` using `usageModeDirective(effectiveMode)` and drop the canonical description for `face`/`emotion`/`style` modes (kept for `identical`/`face-pose` where identity context matters). Backend + frontend video mention resolvers (`resolveVideoPromptMentions`) updated to the same mode-aware directive. Character node UI: small inline `<select>` dropdown in the title row (next to character name), stops drag/click propagation. Autocomplete insertion (PromptEditor + tag-textarea) appends trailing `:<mode>` only when the character node has a non-default `defaultUsageMode` — keeps the common case on the existing 2/3-part form so casual users never see the 4-part syntax. Default mode = "identical" = legacy "Match exactly. Maintain perfect likeness" behavior, so backward compatibility is preserved for every existing workflow.)*
*Version: 2.13.5*
