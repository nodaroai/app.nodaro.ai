...
*Last updated: 2026-05-16 (character aspect-ratio smart defaults: per-asset-type table replaces 16:9 default — portrait/headAngles/lighting=3:4, expressions=1:1, poses/bodyAngles/motions=9:16, legacy `angles`=3:4 (same as headAngles). New shared module `packages/shared/src/character-aspect-defaults.ts` (4-value `CHARACTER_ASPECT_OPTIONS` `[1:1, 3:4, 16:9, 9:16]` + `CHARACTER_ASPECT_DEFAULTS` table + `resolveCharacterAspectRatio({explicit, nodeOverride, assetType})` helper with precedence: explicit > nodeOverride > per-asset-type default; invalid values silently fall through). Routes `generate-character` / `generate-character-asset` / `generate-character-motion` now accept `aspectRatio` (explicit) + `characterNodeAspectRatio` (node toggle) on the body, resolve via the shared helper, and pass the resolved ratio to the worker. Worker `entity.ts`: per-job `aspectRatio` from job data wins over the handler's static `opts.aspectRatio` (faces stay pinned at 1:1 via opts because they're always square crops); motion handler now passes `{aspectRatio}` through `imageToVideo`'s `options` slot. Character node UI: 4-pill toggle (was 2-pill 1:1/16:9), drives BOTH thumbnail crop AND the override slot — `defaultAssetAspectRatio` type extended to `CharacterAspectRatio` (4-value union from @nodaro/shared). Frontend wiring: all 9 studio + canvas call sites pass `state.staged.defaultAssetAspectRatio` (or `data.defaultAssetAspectRatio` from the canvas) as `characterNodeAspectRatio` — appearance/expressions/motions tabs, character-page-modal generate-all + custom, workflow asset-executors character + asset paths. SDK `@nodaro/client/resources/characters`: `GenerateCharacterInput`/`GenerateAssetInput`/`GenerateMotionInput` extended with `aspectRatio?: CharacterAspectRatio` + `characterNodeAspectRatio?: CharacterAspectRatio`; index exports `CharacterAspectRatio` type + `CHARACTER_ASPECT_OPTIONS`/`CHARACTER_ASPECT_DEFAULTS` runtime constants. Tests: 10 new tests in `packages/shared/src/__tests__/character-aspect-defaults.test.ts` covering registry shape, resolver precedence, null/invalid value handling, and the legacy `angles` alias; 11 backend route tests (asset 11, motion 4, character 5) confirming the default flows from request body through Zod through `resolveCharacterAspectRatio` to the worker payload's `aspectRatio` field; 1 worker test in `entity.test.ts` confirming `aspectRatio` from job.data flows to `generateImage` via `extraParams.aspect_ratio` and to `imageToVideo` via `options.aspectRatio`. Backward compat: when no `aspectRatio` or `characterNodeAspectRatio` is set, the resolver falls back to the per-asset-type default — workflows that explicitly pass `aspectRatio` are unaffected.)*
*Version: 2.14.0*
