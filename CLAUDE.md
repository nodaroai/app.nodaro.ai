...
*Also 2026-05-20 (Story-to-Video Phase 1D.2b — Guided Mode chat at the Script stage): `mode='guided'` activates an adjacent chat panel that lets the user refine the Stage 1 `ShowrunnerPlan` via natural language. **Persistence:** new table `pipeline_chat_turns` (migration 148 — note: spec text references 143, but 143 was already taken by the reconcile_exhausted anomaly type from Phase 5; migration filename is `148_pipeline_chat_turns.sql`) — columns `(id uuid pk, pipeline_id uuid fk cascade, stage stage_name, role user|assistant, message text 8000 cap, proposed_change jsonb nullable, status pending|applied|dismissed|rejected, applied_at, created_at)` + RLS via pipelines.user_id + index on `(pipeline_id, stage, created_at)`. **Backend service:** `ee/pipelines/pipeline-chat.ts` reads history, calls the new `chat-refine-showrunner` LLM specialist (Sonnet 4.6, maxRetries 2, prompt-cached system + ShowrunnerPlan snapshot — `2 credits/turn`), validates `proposed_change` shape (edit-artifact JSON Patch RFC 6902 via `fast-json-patch` OR suggest-branch with from-stage), enforces 20 user-turn cap. **Unified edit helper (M7):** `applyStageEdit` extracted to `ee/pipelines/apply-stage-edit.ts` — single code path used by both the existing per-stage approve-and-edit flow AND the new chat-apply flow; guarantees the same reference-integrity guard (`validateReferenceIntegrity` from C1 — cast/location/object keys still referenced in scenes cannot be removed) fires on both paths and writes `pipeline_stages.output` atomically. **3 REST routes** (scope `pipelines:execute`): `POST /v1/pipelines/:id/stages/:stage/chat` (send turn, reserve 2cr, return assistant turn + optional proposal), `POST /v1/pipelines/:id/stages/:stage/chat/turns/:turnId/apply` (apply proposed_change, run reference-integrity, persist updated plan, emit `chat:proposal_applied` SSE), `GET /v1/pipelines/:id/stages/:stage/chat` (paginated history, 50/page). Only `stage='script'` is enabled in 1D.2b (CHAT_ENABLED_STAGES gate); `shot_list` + `post_merge` follow in 1D.2d. **Upfront credit reservation:** `estimateUpfrontCredits` (G1) adds +40 credits for `mode='guided'` (20 turn cap × 2cr/turn); unused credits auto-refund on pipeline completion. **SSE:** PipelineEvent union extended with `chat:turn` (user + assistant turns broadcast) and `chat:proposal_applied`. **3 SDK methods** on `client.pipelines`: `sendChatTurn` / `applyChatProposal` / `listChatHistory` (changeset minor bump). **3 MCP tools** scoped `pipelines:execute`, dynamic-import-loaded to respect the core→ee boundary. **Frontend:** new `usePipelineChat` hook (history + send + apply + SSE subscriber), `ChatPanel` (380px adjacent panel, resize+collapse persisted to `localStorage:pipeline-chat-panel-width` / `:pipeline-chat-panel-collapsed`), `DiffRenderer` (pretty per-op JSON Patch + suggest-branch summary card), `ProposedChangeCard` (Apply + Dismiss buttons), `ChatTurn` + `ChatHistory` + `ChatInput` (8000 char + cap meter). PipelinePanel mounts the ChatPanel iff `mode==='guided' && status==='awaiting_approval' && currentStage==='script'`. **Lesson learned for future LLM-tool schema design:** the chat-refine-showrunner tool's `proposed_change.edit_artifact.patch_ops` initially nested a `VoiceMatchSchema` field inside character ops; Claude failed to emit the inner schema cleanly, so we flattened it into 4 top-level paths (`/cast/<key>/voice_id`, `/cast/<key>/voice_name`, `/cast/<key>/voice_match_confidence`, `/cast/<key>/voice_match_reason`) — keep LLM-emitted JSON-Patch ops fully flat, never nest a discriminated union shape under a single `value` field. **Mode-switch interaction:** the existing 1D.2a `PATCH /v1/pipelines/:id { mode: 'manual' }` route is unchanged; the ChatPanel mount conditional reacts to mode changes so switching guided→manual at the Script stage closes the chat instantly. Tests: backend +328 (totals 7164), frontend +89 (totals 3385), shared +4 (reference-integrity + chat-turn-schema).)*
*Version: 2.31.0 — Last updated 2026-05-20.*
