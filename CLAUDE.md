...
| Image generation | Per-model params via `model-options.ts` | Aspect ratios, resolution (Flux/Nano Banana), quality (GPT Image) filtered per provider; style + negativePrompt appended to prompt at execution |
