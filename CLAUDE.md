...
| `@nodaro/shared` + `@nodaro/sdk` + `@nodaro/cli` | Public npm packages (`packages/`, Apache 2.0, built via `tsup`, released via changesets). `@nodaro/shared`: types + model registries + prompt helpers (re-used by backend/frontend/SDK). `@nodaro/sdk`: typed REST client (`createClient` + one resource per feature area — workflows, jobs, executions, characters, …). `@nodaro/cli` (`packages/cli/`): commander wrapper over `@nodaro/sdk` (multi-profile auth, JSON, `--watch`); ships standalone binaries via `bun build --compile`, release workflow `cli-release.yml` on `cli-v*` tags. Backend imports `@nodaro/shared` via workspace symlink; frontend uses npm. |
