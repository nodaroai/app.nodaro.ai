...
*Last updated: 2026-05-16 (character mention usage modes: added 6th mode `pose` (body pose only, no face transfer) — `USAGE_MODES = [identical, face, face-pose, pose, emotion, style]`. `usageModeDirective("pose")` emits "Take only the body pose and posture — do not copy facial identity, features, or skin. Adopt face, clothing, hair, and all other details from the rest of the prompt." `usageModeLabel("pose")` = "Pose only". Pose mode falls outside the canonical-description inclusion guard (kept only for `identical`/`face-pose`), matching face/emotion/style — desired since pose explicitly suppresses identity transfer. Also strengthened identity-preservation language across the existing 5 directives: `identical` now starts with "The subject must remain exactly the same person — preserve 100% facial identity, bone structure, skin tone, proportions…", `face`/`face-pose` add "preserve 100% facial identity, bone structure, skin tone, and all unique features. Do not alter eyes, nose, mouth, or facial shape", `emotion` reframes as "Adopt face shape, clothing, body, and pose from the rest of the prompt; transfer only the emotional cue", `style` adds "Do not copy the subject's identity, clothing, or pose". Test updates: 5 shared + 1 backend + 8 frontend assertion strings refreshed (`/Match exactly/` → `"The subject must remain exactly the same person"`, `"Take only the facial features and expression"` → `"Take ONLY the facial features"`, etc.). UI auto-renders the new pose option via `{USAGE_MODES.map(...)}` in both character-node.tsx and extra-refs-section.tsx — no UI code change needed.)*
*Version: 2.13.6*
