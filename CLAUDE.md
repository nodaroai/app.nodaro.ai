...
*Last updated: 2026-05-18 (hard-fail policy for missing model prices: `getModelCreditBaseCost` (`backend/src/ee/billing/credits.ts`) now throws a new `PriceNotConfiguredError` when a credit identifier resolves to neither a `model_pricing` DB row nor a `STATIC_CREDIT_COSTS` fallback entry — replacing the prior silent `creditCost: 1` default that leaked revenue on missing composite ids like `seedance-2:8s:1080p-ref`. Caught at two tripwires in `backend/src/ee/lib/credit-guard-impl.ts`: (a) `creditGuardImpl` translates the error to HTTP 503 `price_not_configured` (with `{ code, message, identifier }`) in the `computeCredits` path and the `checkCreditsWithProfile` path, BEFORE any job row is created; (b) `reserveCreditsForJobImpl` does the same, but also `DELETE`s the orphan job row that the route handler already inserted, so a missing-price job leaves zero state behind. Hot/cold paths covered: AI provider routes go through the standard preHandler check; computeCredits hooks (loop-video / trim-video / combine-videos / generate-video / generate-character) surface the error at the same point. Workers never see the error — reservation must succeed first. Two unit-test gates added: `backend/src/ee/billing/__tests__/credits.test.ts` asserts the throw path (5 new tests, including reservation propagation), and `backend/src/middleware/__tests__/credit-guard.test.ts` asserts the 503 translation (3 new tests, including orphan-row cleanup). Plus a permanent regression test `backend/src/ee/billing/__tests__/hard-fail-coverage.test.ts` walks every (provider × duration × sound × resolution × hasVideoRef × quality × renderingSpeed × LLM model) the four runtime builders (`buildVideoCreditModelIdentifier` / `buildMotionCreditModelIdentifier` / `buildCreditModelIdentifier` / `buildLlmCreditIdentifier`) can emit — 214 distinct identifiers, all in STATIC_CREDIT_COSTS today. The pre-existing `composite-credit-coverage.test.ts` only covered the image builder; this new file extends to video + motion + LLM. Docs updated: `backend/CLAUDE.md` gets a new "Hard-Fail Policy for Missing Prices" section above the credit-cost-per-node table; root CLAUDE.md "Credit pricing" row notes the policy + regression-net file. Frontend error handling: existing handlers display `error.message` from the response body via toast — no new UI work needed.)*
*Version: 2.16.1*

---

*Previous: 2026-05-18 (Story-to-Video Phase 1A: Foundation. New tables pipelines/pipeline_stages/pipeline_stage_attempts/pipeline_entities/pipeline_entity_variants/pipeline_entity_nodes/llm_calls/llm_prompt_versions (migration 121) + pipelines.reservation_usage_log_id (migration 122) + model_pricing seed rows for `pipeline-orchestration` and `pipeline-orchestration:stage_1_only` (migration 123). New `pipeline-orchestration` BullMQ queue + dedicated worker process (`backend/src/pipeline-worker.ts`). New node `generative-pipeline` (Cloud-only, 18-step registered). REST routes `/v1/pipelines/*` + 3 new scopes `pipelines:{read,execute,approve}`. Stage 1 (Script) end-to-end Manual mode: Detection → Showrunner → Script Critic + Cast Coverage Critic. Frontend PipelinePanel + StageRow + `usePipelineStream` hook.)*
