...
-- All created CONCURRENTLY for zero-downtime on production
--
-- Note: CREATE INDEX CONCURRENTLY cannot run inside a transaction.
-- supabase db push handles this automatically. If running manually,
-- execute each statement separately outside a transaction block.
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_assets_user_id_created_at
  ON assets (user_id, created_at DESC);

-- 4. Assets: duplicate URL check (library.ts:378)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_assets_user_id_r2_url
  ON assets (user_id, r2_url);

-- 5. Assets: cleanup service batch query (cleanup-service.ts:118)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_assets_cleanup
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_credit_transactions_user_id_created_at
  ON credit_transactions (user_id, created_at DESC);

-- 8. Usage logs: admin usage history (supersedes separate single-column indexes)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_usage_logs_user_id_created_at
