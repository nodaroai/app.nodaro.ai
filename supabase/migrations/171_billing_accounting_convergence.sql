-- 171: Converge reserve_credits / refund_credits across environments.
--
-- Why this exists
-- ---------------
-- The billing-accounting fixes (8-arg reserve_credits + pool/allowance/daily-cap
-- aware refund_credits) shipped to staging as migration 169
-- (169_billing_accounting_fixes.sql). But PRODUCTION had already recorded
-- version 169 for a DIFFERENT migration: the IDOR lock-down, which deployed at
-- 169 (169_lock_down_append_asset_rpcs.sql) via an earlier dev→main PR, before
-- it was renumbered to 170 on dev. Supabase keys migrations by version number,
-- so on production `169_billing_accounting_fixes.sql` is treated as
-- already-applied (version 169 present) and SKIPPED — production would never get
-- the 8-arg reserve_credits.
--
-- That is an outage risk: the application code calls reserve_credits with the
-- 8th arg `p_daily_limit`; against the old 7-arg function PostgREST errors
-- ("no function matching …"), failing every credit reservation.
--
-- This migration re-applies the billing-accounting definitions at a fresh
-- version (171) that NO environment has used, so production gets them and
-- staging (which already has them via 169) simply re-applies harmlessly. The
-- DDL is identical to 169_billing_accounting_fixes.sql and fully idempotent
-- (DROP IF EXISTS the 7-arg signature + CREATE OR REPLACE). See migration 166
-- for the analogous convergence pattern after the 165 number collision.
