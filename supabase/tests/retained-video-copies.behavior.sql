...
INSERT INTO public.retained_videos(id,user_id,workflow_id,sha256,byte_length,width,height,duration_ms,content_type,charged)
 SELECT pg_temp.copy_uuid('a',n),pg_temp.copy_uuid('0',1),pg_temp.copy_uuid('d',least(n,3)),
   CASE WHEN n = 4 THEN repeat('b',64) ELSE repeat('a',64) END,10,1,1,1200,'video/mp4',false FROM generate_series(1,4) n;
SELECT public.complete_retained_video(pg_temp.copy_uuid('a',n),CASE WHEN n = 4 THEN repeat('b',64) ELSE repeat('a',64) END)
 FROM generate_series(1,4) n;
INSERT INTO public.jobs(id,user_id,workflow_id,job_type,status,credits,output_data,submission_context) VALUES
 (pg_temp.copy_uuid('f',1),pg_temp.copy_uuid('0',1),pg_temp.copy_uuid('d',1),'image-to-video','completed',0,
 '{"videoUrl":"https://media.test/original.png"}','{"kind":"original","input":"immutable"}');
SELECT public.record_retained_job_video(pg_temp.copy_uuid('0',1),pg_temp.copy_uuid('d',1),pg_temp.copy_uuid('f',1),
 pg_temp.copy_uuid('a',1),'https://media.test/original.png');
DELETE FROM public.jobs WHERE id = pg_temp.copy_uuid('f',1);

SELECT pg_temp.assert_true('unattested job cannot become a copy proof', public.record_retained_video_copy(
...
SELECT pg_temp.assert_true('wrong source workflow cannot use the original proof', public.record_retained_video_copy(
 pg_temp.copy_uuid('b',1),pg_temp.copy_uuid('0',1),pg_temp.copy_uuid('d',2),pg_temp.copy_uuid('a',2),
 pg_temp.copy_uuid('d',3),pg_temp.copy_uuid('f',1),NULL,'{"frame":"B"}') IS NULL);
SELECT pg_temp.assert_true('destination video must belong to the destination workflow', public.record_retained_video_copy(
...
SELECT pg_temp.assert_true('changed bytes cannot claim source provenance', public.record_retained_video_copy(
 pg_temp.copy_uuid('b',1),pg_temp.copy_uuid('0',1),pg_temp.copy_uuid('d',3),pg_temp.copy_uuid('a',4),
 pg_temp.copy_uuid('d',1),pg_temp.copy_uuid('f',1),NULL,'{"frame":"B"}') IS NULL);
SELECT pg_temp.assert_true('destination capture belongs to the copier', public.record_retained_video_copy(
...
SELECT pg_temp.assert_true('copy records its own identity and server-copied original provenance',
 public.record_retained_video_copy(pg_temp.copy_uuid('b',1),pg_temp.copy_uuid('0',1),pg_temp.copy_uuid('d',2),pg_temp.copy_uuid('a',2),
...
SELECT pg_temp.assert_true('one original can support multiple copied frames without fake jobs',
 public.record_retained_video_copy(pg_temp.copy_uuid('b',3),pg_temp.copy_uuid('0',1),pg_temp.copy_uuid('d',2),pg_temp.copy_uuid('a',2),
...
SELECT pg_temp.assert_true('copy of copy preserves the original job and immediate mapped source',
 public.record_retained_video_copy(pg_temp.copy_uuid('b',2),pg_temp.copy_uuid('0',1),pg_temp.copy_uuid('d',3),pg_temp.copy_uuid('a',3),
...
DO $$ BEGIN
  BEGIN
    UPDATE public.retained_video_copies SET context = '{}' WHERE id = pg_temp.copy_uuid('b',1);
  ...
  BEGIN
    DELETE FROM public.retained_video_copies WHERE id = pg_temp.copy_uuid('b',1);
  ...
  BEGIN
    PERFORM public.record_retained_video_copy(pg_temp.copy_uuid('b',1),pg_temp.copy_uuid('0',1),pg_temp.copy_uuid('d',2),pg_temp.copy_uuid('a',2),
  ...
  BEGIN
    PERFORM public.record_retained_video_copy(pg_temp.copy_uuid('b',1),pg_temp.copy_uuid('0',2),pg_temp.copy_uuid('d',2),pg_temp.copy_uuid('a',2),
      pg_temp.copy_uuid('d',1),pg_temp.copy_uuid('f',1),NULL,'{"frame":"B"}');
...
SELECT pg_temp.assert_true('copy proof survives original and immediate source deletion', EXISTS (
 SELECT 1 FROM public.retained_video_copies WHERE id = pg_temp.copy_uuid('b',2)
 AND origin_submission_context = '{"kind":"original","input":"immutable"}' AND source_context = '{"frame":"B"}'));
SELECT pg_temp.assert_true('exact retry survives deletion of source workflow', public.record_retained_video_copy(
...
RESET ROLE;
SELECT pg_temp.assert_true('anonymous access is revoked', NOT has_table_privilege('anon','public.retained_video_copies','SELECT'));
SELECT pg_temp.assert_true('anonymous proof writer is revoked', NOT has_function_privilege('anon',
 'public.record_retained_video_copy(uuid,uuid,uuid,uuid,uuid,uuid,uuid,jsonb)','EXECUTE'));
SET LOCAL ROLE authenticated;
DO $$ BEGIN
  BEGIN
    PERFORM * FROM public.retained_video_copies;
    RAISE EXCEPTION 'ASSERT FAIL: client can read copy proofs';
  EXCEPTION WHEN insufficient_privilege THEN NULL; END;
  BEGIN
    PERFORM public.record_retained_video_copy(gen_random_uuid(),gen_random_uuid(),gen_random_uuid(),gen_random_uuid(),
...
SELECT pg_temp.assert_true('destination deletion cleans up proof and queues its bytes',
 NOT EXISTS (SELECT 1 FROM public.retained_video_copies WHERE id = pg_temp.copy_uuid('b',2))
 AND EXISTS (SELECT 1 FROM public.retained_video_gc WHERE id = pg_temp.copy_uuid('a',3)));
RESET ROLE;
